...
fn test_expert_review_new_wide_surface_symbols() {
    use expanse::blobmap::{
        expanse_blob_map_contains, expanse_blob_map_free, expanse_blob_map_insert,
        expanse_blob_map_new,
    };
    use expanse::modern::{
        expanse_bytesmap_contains, expanse_bytesmap_for_each, expanse_bytesmap_free,
        expanse_bytesmap_insert, expanse_bytesmap_new, expanse_map_contains, expanse_map_free,
        expanse_map_insert, expanse_map_new, expanse_map_validate, expanse_map_validate_explain,
        expanse_set_free, expanse_set_insert, expanse_set_new, expanse_set_validate,
        expanse_strmap_contains, expanse_strmap_free, expanse_strmap_insert, expanse_strmap_new,
    };

    // SAFETY: every handle below is created by its `_new` and freed once;
    // null handles are passed only to entry points documented to accept them.
    unsafe {
        ...
        assert!(expanse_map_validate_explain(
            core::ptr::null(),
            core::ptr::null_mut(),
            0
        ));
        ...
        assert!(!expanse_bytesmap_contains(
            core::ptr::null(),
            core::ptr::null(),
            0
        ));
        assert_eq!(
            expanse_bytesmap_for_each(core::ptr::null(), None, core::ptr::null_mut()),
            0
        );
        ...
        unsafe extern "C" fn count_cb(
            ...
            // SAFETY: `user_data` is the test's `usize` counter, alive and
            // unaliased for the duration of the iteration.
        ...
        let break_visited =
            expanse_bytesmap_for_each(bmap, Some(break_cb), (&raw mut break_count).cast());
        ...
        assert!(!expanse_strmap_contains(
            core::ptr::null(),
            core::ptr::null()
        ));
