...
enum Root {
    ...
    /// `ExpanseSet::tree_pop` (the JPM role) so the enum stays a plain
    /// `Copy` word pair — an atomic inside it would make every read of the
    /// root reload through memory.
    Tree {
        top: Edge,
    },
}
...
/// holding the tree word for the whole operation (the string, bytes and
/// blob wrappers' inner trees: nested brackets, the protocol their readers
...
/// scratch word — never the tree word (a nested `begin` would make it even
/// mid-write), never a node's own (an upgrade inside the build marks that
/// node obsolete, which needs it even).
#[inline(always)]
fn promote_leaf<const OCC: bool, const NESTED: bool>(
    ...
    for &k in slice {
        ...
        let ins = unsafe { mutate::insert::<OCC, NESTED>(alloc, &mut top, k, 8, cover) };
        debug_assert!(ins);
    }
...
impl ExpanseSet {
    ...
    pub fn insert_c_int(&mut self, key: Key) -> core::ffi::c_int {
        self.insert_plain(key) as core::ffi::c_int
    }
    ...
    /// root leaf while this stores to it (#1086): [`Self::insert_inner_shared`].
    #[cfg(feature = "std")]
    #[inline(always)]
    ...
    /// [`Self::insert_inner`] for a shared set: the stores into a live root
    ...
    #[cfg(feature = "std")]
    #[inline(always)]
    fn insert_inner_shared(&mut self, key: Key) -> bool {
        match &mut self.root {
            Root::Empty => {
                let keys = self.alloc.alloc_bytes(root_leaf_size(1));
                // SAFETY: fresh 8-byte allocation, cache-line aligned.
            ...
            Root::Leaf { keys, pop } => {
                ...
                if pop < ROOT_LEAF_CAP {
                    ...
                    unsafe {
                        ...
                        dst.add(at + 1)
                            .copy_from_nonoverlapping(slice.as_ptr().add(at), pop - at);
                        self.alloc.free_bytes_plain(keys, root_leaf_size(pop));
                    }
                    self.root = Root::Leaf {
                        keys: new,
                        pop: pop + 1,
                    ...
                    // SAFETY: old root leaf no longer referenced.
                    unsafe { self.alloc.free_bytes_plain(keys, root_leaf_size(pop)) };
                    self.tree_pop = pop as u64 + 1;
                    self.root = Root::Tree { top };
                }
                true
    ...
    ///
    /// The blocks a removal frees stay with the set for reuse, including
    ...
    /// [`Self::remove_inner`] for a shared set; as
        ...
        match &mut self.root {
            ...
            Root::Leaf { keys, pop } => {
                ...
                if pop == 1 {
                    ...
                    unsafe {
                        let ptr = keys.as_ptr().cast::<u64>();
                        shared_word::shift_down::<true>(ptr, at, pop - 1 - at);
                    }
                    self.root = Root::Leaf { keys, pop: pop - 1 };
                } else {
                    let new = self.alloc.alloc_bytes(root_leaf_size(pop - 1));
        ...
        match &mut self.root {
            ...
            Root::Leaf { keys, pop } => {
                ...
                if pop == 1 {
                    ...
                    unsafe {
                        self.alloc
                            .free_bytes_dispatch::<OCC>(keys, root_leaf_size(1))
                    };
                    self.root = Root::Empty;
                } else if crate::leaf::cap_class(pop - 1) == crate::leaf::cap_class(pop) {
                    // Fast path: capacity class unchanged — shift surviving keys in-place.
                    ...
                    let new = self
                        .alloc
                        .alloc_bytes_dispatch::<OCC>(root_leaf_size(pop - 1));
                    // SAFETY: copy the surviving keys into the smaller
                    // allocation.
                    unsafe {
                        let dst = new.as_ptr().cast::<u64>();
                        ...
                        dst.add(at)
                            .copy_from_nonoverlapping(slice.as_ptr().add(at + 1), pop - 1 - at);
                        self.alloc
                            .free_bytes_dispatch::<OCC>(keys, root_leaf_size(pop));
                    }
                    self.root = Root::Leaf {
                        keys: new,
                        pop: pop - 1,
            ...
            Root::Tree { top } => {
                ...
                // always sat (see `insert_inner`).
                let (removed, now) =
                    tree_remove::<OCC, NESTED>(&self.alloc, &mut self.tree_pop, top, key);
                if removed {
                    if now == 0 {
                        debug_assert!(top.is_null());
                        // A root-state change: on a shared tree whose engine
                        ...
                        // both forms). A root-state change, as above.
                        crate::occ::tree_begin_if::<OCC>(&self.alloc);
                        self.condense_to_root_leaf::<OCC>();
                        crate::occ::tree_end_if::<OCC>(&self.alloc);
                    }
                }
                removed
    ...
    /// Removes every key, and returns the blocks the set's allocator kept
    /// for reuse to the system allocator, so the set holds what a new one
    /// does ([`Self::mem_held`] is 0). It keeps no capacity: a set refilled
    ...
    fn clear_dispatch<const OCC: bool>(&mut self) {
        ...
        match &mut self.root {
            ...
            Root::Tree { top, .. } => {
                ...
                unsafe { mutate::free_subtree::<OCC, false>(&self.alloc, top) };
            }
        }
        self.root = Root::Empty;
    }
...
impl ExpanseSet {
    ...
    fn condense_to_root_leaf<const OCC: bool>(&mut self) {
        self.path.get_mut().clear();
        ...
        let leaf = self.alloc.alloc_bytes_dispatch::<OCC>(root_leaf_size(n));
...
impl ExpanseSet {
    ...
    fn condense_built_tree(&mut self, top: Edge, n: usize) {
        ...
        if n == 0 {
            ...
            unsafe { mutate::free_subtree::<false, false>(&self.alloc, &mut t) };
            self.root = Root::Empty;
            return;
        }
        let leaf = self.alloc.alloc_bytes_plain(root_leaf_size(n));
        let mut written = 0usize;
        let mut from = Some(0u64);
...
mod tests {
    ...
    fn occ_engine_drive(engine_covers_root: bool) {
        ...
        if engine_covers_root {
            ...
            s.occ_root().1.bracket_enter_any();
        }
    ...
    /// The insert-path cache records the root edge slot among a warm
    /// terminal's ancestors, and that slot lives in the set, not in a trie
    /// node: once the set moves, the entry names memory the set has left. It
    /// is never dereferenced, because the root is always a level-8 branch (so
    /// the entry is recorded at level 8) and `bump_pop0` ignores levels above
    /// 7. The cache is warmed in a helper whose frame then ends, and the set
    /// is moved into a `Box` and later out of it, which frees the box. Flushes
    /// then run with the stale entry recorded — select and the bitmap leaf's
    /// conversion to a full expanse inside the bypass after the first move, a
    /// removal after the second — and population, select and the validator
    /// are checked against a model. Miri reports a dereference of the stale
    /// entry as a use-after-free; natively the test pins the recorded levels
    /// and the results. Sized for Miri.
    #[test]
    fn warm_insert_path_across_a_move() {
        /// Address of the root edge slot while the root is a tree.
        fn root_slot(s: &ExpanseSet) -> *const Edge {
            match &s.root {
        ...
        /// level in `2..=7`.
        fn root_entry(p: &crate::mutate::InsertPath) -> *const Edge {
            assert!(
                p.depth >= 2,
        ...
        fn check(s: &ExpanseSet, model: &BTreeSet<u64>) {
            for (n, &k) in model.iter().enumerate() {
                assert_eq!(s.by_count(n as u64), Some(k), "by_count({n})");
            }
            assert_eq!(s.by_count(model.len() as u64), None);
            assert_eq!(s.len(), model.len() as u64);
            s.validate();
        }
        ...
        fn insert_warm(
            ...
            for k in keys {
                let pending = s.path_mut().pending_pop;
                assert!(s.insert(k));
                model.insert(k);
                assert_eq!(
                    s.path_mut().pending_pop,
                    pending + 1,
                    "insert({k:#x}) left the warm path"
                );
            }
        }
        fn warmed() -> (ExpanseSet, BTreeSet<u64>, *const Edge) {
            let mut s = ExpanseSet::new();
            let mut model = BTreeSet::new();
        ...
        // the set again.
        let (s, mut model, recorded) = warmed();
        let mut s = Box::new(s);
        assert!(!core::ptr::eq(root_slot(&s), recorded));
        assert!(core::ptr::eq(root_entry(s.path_mut()), recorded));
        insert_warm(&mut s, &mut model, (0x201..0x240).step_by(2));
        check(&s, &model);
        assert_eq!(s.path_mut().pending_pop, 0, "select must flush the cache");
        assert!(core::ptr::eq(root_entry(s.path_mut()), recorded));
        // The rest of block 2 through the bypass; the key that completes it
        // turns the bitmap leaf into a full expanse, which flushes the cache
        // before freeing the leaf and clears it after.
        insert_warm(&mut s, &mut model, 0x240..0x2FF);
        assert!(s.insert(0x2FF));
        model.insert(0x2FF);
        assert_eq!(
            s.path_mut().prefix,
            u64::MAX,
            "the full expanse must clear the cache"
        );
        check(&s, &model);

        // A cold descent records the root slot inside the box; moving the set
        // out frees the box with the entry still naming it.
        for k in (0x300..0x340u64).step_by(2) {
            assert!(s.insert(k));
            model.insert(k);
        }
        let recorded = root_slot(&s);
        assert!(core::ptr::eq(root_entry(s.path_mut()), recorded));
        let mut s = {
            let boxed = s;
            *boxed
        ...
        assert!(!core::ptr::eq(root_slot(&s), recorded));
        assert!(core::ptr::eq(root_entry(s.path_mut()), recorded));
        insert_warm(&mut s, &mut model, (0x301..0x340).step_by(2));
        // A removal clears the cache, flushing it first.
        assert!(s.remove(0x301));
        model.remove(&0x301);
        assert_eq!(s.path_mut().prefix, u64::MAX, "remove must clear the cache");
        check(&s, &model);
    }
