...
///
/// `remove_range` cannot express a set whose keys share no interval — the
/// retired half of a hash-keyed index, say — and removing them one at a time
/// pays a fresh root descent per key. This partitions the sorted slice by
/// digit at every branch, so each node on the path is visited once no matter
/// how many of the keys pass through it, and the leaf-local work is delegated
/// to `map_remove` unchanged.
///
/// `f(key, value)` is called once per key actually present, in ascending
/// order. Keys absent from the map are skipped silently; the return is the
/// number removed, not the number asked for.
///
/// The slice must be sorted. An unsorted one removes only what it happens to
/// find in the runs it forms, which `debug_assert` catches in test builds.
pub(crate) fn map_remove_many<F: FnMut(u32, u32)>(
    a: &mut Arena,
    e: &mut Edge32,
    kb: u8,
    prefix: u32,
    ...
    match kind(e) {
        Kind::BranchL2 | Kind::BranchL6 | Kind::BranchB | Kind::BranchU => {
            ...
            let mut i = 0usize;
            while i < keys.len() {
                // The slice is sorted, so every key with this digit is one
                // contiguous run and the branch is entered once for it.
                let d = digit_at(keys[i] & rem_mask(kb), kb);
                let mut j = i + 1;
                while j < keys.len() && digit_at(keys[j] & rem_mask(kb), kb) == d {
                    j += 1;
                }
                let run = &keys[i..j];
                i = j;
                ...
                let cprefix = prefix | (u32::from(d) << ((kb - 1) as u32 * 8));
                let before = child;
                let n = map_remove_many(a, &mut child, kb - 1, cprefix, run, f);
                if n == 0 {
                    continue;
                }
                total += n;
        ...
        _ => {
            // A terminal: every remaining key lands in this leaf, so the
            // descent is already paid and the per-key work is leaf-local.
            let mut total = 0usize;
            for &k in keys {
                let rem = k & rem_mask(kb);
                if let Some(v) = map_remove(a, e, kb, rem) {
                    f(prefix | rem, v);
                    total += 1;
                }
                if e.is_null() {
