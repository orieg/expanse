...
impl<T: SharedTree> Shared<T> {
    ...
    /// Promoted architecture (Refs #929, AGENTS.md §2.7): [`Self::with_locked`]
...
pub(crate) enum CapExpansionKind {
    ...
    #[allow(dead_code)]
    MapBitmapSub,
    /// Remove-side capacity adjustments (shrink/demote) (Phase 4F eliminated; kept for partition completeness).
    #[allow(dead_code)]
    Remove,
}
        ...
        loop {
            ...
            match tag {
                ...
                EdgeTag::Immed(im) => {
                    ...
                    // SAFETY: payload holds n packed keys of kb bytes per ImmedType invariant, and i < n.
                    let ins_pos = (0..n)
                        .find(|&i| unsafe {
                            crate::mutate::read_packed(payload.as_ptr(), i, kb_usize) > k
                        })
                        .unwrap_or(n);
                    if n < crate::types::ImmedType::max_count(kb) as usize {
                        let Ok((old_v, lock_t0)) =
                            version_try_lock_expect_timed(p_cell, parent.version_snap)
                        else {
                            return OlcOutcome::Retry;
                        ...
                        unsafe {
                            ...
                            if ins_pos < n {
                                core::ptr::copy_nonoverlapping(
                                    payload.as_ptr().add(ins_pos * kb_usize),
                                    new_payload.as_mut_ptr().add((ins_pos + 1) * kb_usize),
                                    (n - ins_pos) * kb_usize,
                                );
                            }
                        }
                        ...
                        let new_im =
                            crate::types::ImmedType::new(kb, (n + 1) as u8).expect("capacity");
                    ...
                    // SAFETY: new_buf is freshly allocated with size for n + 1 keys; payload has n keys.
                    unsafe {
                        if ins_pos > 0 {
                            core::ptr::copy_nonoverlapping(
                                payload.as_ptr(),
                        ...
                        crate::mutate::write_packed(new_buf, ins_pos, kb_usize, k);
                        if ins_pos < n {
                            core::ptr::copy_nonoverlapping(
                                payload.as_ptr().add(ins_pos * kb_usize),
                                new_buf.add((ins_pos + 1) * kb_usize),
                                (n - ins_pos) * kb_usize,
                            );
                        }
                        let saved_aux = *edge.aux_bytes();
            ...
            match tag {
                ...
                EdgeTag::Structural(EdgeType::LeafB1) => {
                    ...
                    if pop0 < crate::types::LEAFB1_DOWN {
                        ...
                        if keys.len <= immed_max {
                            ...
                            // SAFETY: node, edge, and version pointers are valid and EBR-live under the OLC protocol.
                            unsafe {
                                let mut full = crate::mutate::StackKeys32::new();
                            ...
                            unsafe {
                                for (i, &k) in keys.as_slice().iter().enumerate() {
                                    *new_buf.add(i) = k as u8;
                                }
                                let saved_aux = *edge.aux_bytes();
                ...
                EdgeTag::Structural(
                    ...
                    if pop - 1 > immed_max {
                        ...
                        // SAFETY: node, edge, and new_buf pointers are valid and EBR-live under the OLC protocol.
                        unsafe {
                            crate::leaf::set_realloc_remove(keys_ptr, new_buf, kb as u8, pop, pos);
                            let saved_aux = *edge.aux_bytes();
                            let mut new_edge = Edge::new_node(new_buf, edge.tag_byte());
                            new_edge.set_aux_bytes(saved_aux);
                ...
                EdgeTag::Immed(im) => {
                    ...
                    let payload = edge.imm_payload();
                    if n == 1 {
                        // SAFETY: payload pointer is valid for 1 immediate key of kb bytes.
                        let existing_k =
                            unsafe { crate::mutate::read_packed(payload.as_ptr(), 0, kb as usize) };
                        if existing_k == k {
                            if parent.edge_type != EdgeType::BranchU {
                                return branch_split(BranchSplitKind::Remove);
                            }
                    ...
                    // SAFETY: payload pointer is valid for n immediate keys of kb bytes.
                    let pos = match unsafe { crate::leaf::locate(payload.as_ptr(), n, kb, k) } {
                        Ok(p) => p,
                        Err(_) => {
                            if !crate::occ::node_validate(p_cell, parent.version_snap) {
                                return OlcOutcome::Retry;
                    ...
                    // SAFETY: edge_ptr and payload are valid and EBR-live under the OLC protocol.
                    unsafe {
...
/// Promoted architecture (Refs #929, AGENTS.md §2.7): one writer slot's
...
struct BlobWriterArenas {
    ...
    has_writer_deltas: core::sync::atomic::AtomicBool,
}
...
impl SyncExpanseBlobMap {
    ...
    /// Promoted architecture (Refs #929, AGENTS.md §2.7): folds every writer slot's
    ...
    /// Promoted architecture (Refs #929, AGENTS.md §2.7): forgets every slot's private
    ...
    fn alloc_private(
        ...
        let chunk_size = self.writer_arenas.chunk_size;
        let st = owner.state();
...
mod tests {
    ...
    /// Phase 4B verification: concurrent writers trigger immediate growths and
    /// immediate-to-leaf conversions across both set and map while concurrent
    /// readers validate consistency under OLC (W=4, R=2).
    #[test]
    fn test_concurrent_immediate_conversion() {
        use std::sync::Barrier;

        let set = Arc::new(SyncExpanseSet::new());
        let map = Arc::new(SyncExpanseMap::new());

        // Prepopulate enough prefixes so root is a tree and each prefix has 1 key (an immediate child).
        for i in 0..64u64 {
            let base_k = (i << 32) | 1;
            set.insert(base_k);
            map.insert(base_k, i);
        }
        // Also prepopulate 1 key in prefix 0x02_0000 for kb=1 immediate tests
        set.insert(0x02_0000 | 1);
        map.insert(0x02_0000 | 1, 100);
        ...
        for _ in 0..num_readers {
            ...
            reader_handles.push(std::thread::spawn(move || {
                ...
                while !stop_clone.load(Ordering::Relaxed) {
                    ...
                    assert!(s_reader.contains(0x02_0000 | 1));
                    assert_eq!(m_reader.get(0x02_0000 | 1), Some(100));
                }
            }));
        }
        ...
        for w in 0..num_writers {
            ...
            writer_handles.push(std::thread::spawn(move || {
                ...
                for step in 0..4 {
                    ...
                    // 2. kb=1 keys in prefix 0x02_0000: j in 2..=15
                    if j <= 15 {
                        let k_kb1 = 0x02_0000 | j;
                        assert!(set_clone.insert(k_kb1));
                        assert_eq!(map_clone.insert(k_kb1, j * 7), None);
                    }
                }
            }));
        }
        ...
        for i in 0..64u64 {
            let base_k = (i << 32) | 1;
            assert!(set.contains(base_k));
            assert_eq!(map.get(base_k), Some(i));
            for j in 2..18u64 {
                let k = (i << 32) | j;
                assert!(set.contains(k));
                assert_eq!(map.get(k), Some(i * 1000 + j));
        ...
        assert!(set.contains(0x02_0000 | 1));
        assert_eq!(map.get(0x02_0000 | 1), Some(100));
        for j in 2..=15u64 {
            let k_kb1 = 0x02_0000 | j;
    ...
    /// Phase 4F verification: concurrent removals across linear leaves, LeafB1 bitmap leaves,
    /// and immediate nodes under concurrent readers and writers (W=4, R=2).
    #[test]
    fn test_concurrent_removals() {
        ...
        // Prepopulate:
        // 1. Anchor keys so root is firmly a tree throughout removals:
        for p in 0..64u64 {
            let anchor = (p << 48) | 0xFFFF_0000;
            set.insert(anchor);
            map.insert(anchor, p * 100);
        }

        // 2. LeafB1 nodes: prefix 0x0100 has 100 keys (0..100) -> converts to LeafB1 (> 25)
        for j in 0..100u64 {
            let k = 0x0100_0000 | j;
            set.insert(k);
            map.insert(k, j * 7);
        ...
        // 3. Linear leaves across 16 prefixes, each populated with 20 keys (capacity class 20)
        for p in 0..16u64 {
            for j in 1..=20u64 {
                let k = ((0x0200 + p) << 16) | j;
                set.insert(k);
                map.insert(k, p * 1000 + j);
            }
        }

        // 4. Immediates across 16 prefixes, each with 3 keys
        for p in 0..16u64 {
            for j in 1..=3u64 {
                let k = ((0x0300 + p) << 32) | j;
                set.insert(k);
                map.insert(k, p * 500 + j);
            }
        }
        ...
        // Spawn readers checking anchor keys and sampling dynamic keys
        let mut reader_handles = Vec::with_capacity(num_readers);
        for _ in 0..num_readers {
            let set_clone = Arc::clone(&set);
            let map_clone = Arc::clone(&map);
            ...
            reader_handles.push(std::thread::spawn(move || {
                ...
                while !stop_clone.load(Ordering::Relaxed) {
                    for p in 0..64u64 {
                        let anchor = (p << 48) | 0xFFFF_0000;
                        assert!(s_reader.contains(anchor));
                        assert_eq!(m_reader.get(anchor), Some(p * 100));
                    }
                    for j in 0..100u64 {
                        let k = 0x0100_0000 | j;
                        if let Some(v) = m_reader.get(k) {
                            assert_eq!(v, j * 7);
                        }
                    }
                }
        ...
        // Spawn 4 concurrent writers removing keys from the three groups
        let mut writer_handles = Vec::with_capacity(num_writers);
        for w in 0..num_writers {
            let set_clone = Arc::clone(&set);
            let map_clone = Arc::clone(&map);
            ...
            writer_handles.push(std::thread::spawn(move || {
                b_clone.wait();
                // (a) Remove from LeafB1: keys 0..80 partitioned by writer
                for j in (w as u64..80).step_by(num_writers) {
                    let k = 0x0100_0000 | j;
                    assert!(set_clone.remove(k));
                    assert_eq!(map_clone.remove(k), Some(j * 7));
                }
                // (b) Remove from linear leaves: keys 5..20 in prefixes partitioned by writer
                for p in (w as u64..16).step_by(num_writers) {
                    for j in 5..=20u64 {
                        let k = ((0x0200 + p) << 16) | j;
                        assert!(set_clone.remove(k));
                        assert_eq!(map_clone.remove(k), Some(p * 1000 + j));
                    }
                }
                // (c) Remove from immediates: remove key 3 from each immediate
                for p in (w as u64..16).step_by(num_writers) {
                    let k = ((0x0300 + p) << 32) | 3;
                    assert!(set_clone.remove(k));
                    assert_eq!(map_clone.remove(k), Some(p * 500 + 3));
                }
            }));
        }
        ...
        // Verify surviving keys are present
        for p in 0..64u64 {
            let anchor = (p << 48) | 0xFFFF_0000;
            assert!(set.contains(anchor));
            assert_eq!(map.get(anchor), Some(p * 100));
        }
        for j in 80..100u64 {
            let k = 0x0100_0000 | j;
            assert!(set.contains(k));
            assert_eq!(map.get(k), Some(j * 7));
        }
        for p in 0..16u64 {
            for j in 1..=4u64 {
                let k = ((0x0200 + p) << 16) | j;
                assert!(set.contains(k));
                assert_eq!(map.get(k), Some(p * 1000 + j));
            }
            for j in 1..=2u64 {
                let k = ((0x0300 + p) << 32) | j;
                assert!(set.contains(k));
                assert_eq!(map.get(k), Some(p * 500 + j));
            }
        }
        ...
        // Deep validation of tree invariants
        set.with_locked(crate::set::ExpanseSet::validate);
        map.with_locked(crate::map::ExpanseMap::validate);
    }

    /// Verifies deterministic immediate growth and immediate-to-leaf conversion across
    /// all key_bytes widths kb in 1..=7 for both set and map engines, ensuring high-width
    /// upgrade paths (kb in 4..=7) and single-key-to-leaf conversions execute with zero
    /// FallbackImmediateConversion (Kent Beck / Bryan Cantrill expert review).
    #[test]
    fn test_immediate_conversions_all_widths_deterministic() {
        let set = SyncExpanseSet::new();
        let map = SyncExpanseMap::new();

        // 1. Prefill scattered prefixes so the root is unconditionally in tree state.
        for p in 0..64u64 {
            let k = (p << 56) | 1;
            set.insert(k);
            map.insert(k, p);
        ...
        #[cfg(feature = "occ-stats")]
        let before_stats = crate::occ_stats::snapshot();

        // 2. Deterministically test each key_bytes width kb in 1..=7
        for kb in 1..=7u8 {
            let prefix = (0x80u64 + kb as u64) << (kb * 8);
            let set_max = crate::types::ImmedType::max_count(kb) as usize;
            let map_max = crate::mutate::map_immed_max(kb);

            // Step through set inserts: 1 -> set_max -> set_max + 1 (conversion to leaf)
            let total_set_keys = set_max + 2;
            for j in 1..=total_set_keys {
                let k = prefix | (j as u64);
                assert!(set.insert(k), "set insert kb={kb} j={j}");
            }

            // Step through map inserts: 1 -> map_max -> map_max + 1 (conversion to leaf)
            let total_map_keys = map_max.max(1) + 2;
            for j in 1..=total_map_keys {
                let k = prefix | (j as u64);
                let v = (kb as u64) * 1000 + (j as u64);
                assert_eq!(map.insert(k, v), None, "map insert kb={kb} j={j}");
            }
        }

        #[cfg(feature = "occ-stats")]
        {
            let after_stats = crate::occ_stats::snapshot();
            let imm_fallbacks = after_stats
                [crate::occ_stats::Stat::FallbackImmediateConversion as usize]
                - before_stats[crate::occ_stats::Stat::FallbackImmediateConversion as usize];
            assert_eq!(
                imm_fallbacks, 0,
                "all-widths immediate expansions and conversions must have 0 FallbackImmediateConversion"
            );
        }

        // Verify content integrity across all widths
        for kb in 1..=7u8 {
            let prefix = (0x80u64 + kb as u64) << (kb * 8);
            let set_max = crate::types::ImmedType::max_count(kb) as usize;
            let map_max = crate::mutate::map_immed_max(kb);
            ...
            for j in 1..=total_set_keys {
                let k = prefix | (j as u64);
                assert!(set.contains(k), "set contains kb={kb} j={j}");
            }
            ...
            for j in 1..=total_map_keys {
                let k = prefix | (j as u64);
                let v = (kb as u64) * 1000 + (j as u64);
                assert_eq!(map.get(k), Some(v), "map get kb={kb} j={j}");
            }
        }
