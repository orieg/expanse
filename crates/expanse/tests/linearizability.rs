...
fn is_valid_transition(state: &Option<u64>, op: &Op, ret: &Ret) -> (bool, Option<u64>) {
    match (op, ret) {
        (Op::Insert(_, v), Ret::Insert(old)) => {
            (old == state, Some(*v))
        }
        (Op::Remove(_), Ret::Remove(old)) => {
            (old == state, None)
        }
        (Op::Get(_), Ret::Get(val)) => {
            (val == state, *state)
        }
        _ => (false, None),
    }
...
fn test_sync_map_linearizability() {
    ...
    for (key, events) in by_key {
        ...
        assert!(check_linearizability_for_key(&events), "Linearizability violation for key {}", key);
    }
}
