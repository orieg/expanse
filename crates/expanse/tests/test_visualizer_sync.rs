...
fn test_visualizer_constants_sync() {
    ...
    assert!(html_path.exists(), "docs/architecture_visualizer.html must exist at {:?}", html_path);
    assert!(json_path.exists(), "docs/visualizer_data.json must exist at {:?}", json_path);

    let html_content = fs::read_to_string(&html_path)
        .expect("failed to read docs/architecture_visualizer.html");
    let json_content = fs::read_to_string(&json_path)
        .expect("failed to read docs/visualizer_data.json");
    ...
    assert_eq!(BITMAP_TO_UNCOMPRESSED_THRESHOLD, 192, "BITMAP_TO_UNCOMPRESSED_THRESHOLD should be 192");
    ...
    assert!(
        html_content.contains("ROOT_LEAF_CAP: 31") || html_content.contains("ROOT_LEAF_CAP = 31") || html_content.contains("pop <= 31") || html_content.contains("Pop ≤ 31"),
        "HTML visualizer must reference ROOT_LEAF_CAP (31)"
    );
    assert!(
        html_content.contains("BRANCH_L3_CAP") || html_content.contains("Branch L3/L7") || html_content.contains("Fanout 1..7"),
    ...
    assert!(json_content.contains(r#""ROOT_LEAF_CAP": 31"#), "JSON must contain ROOT_LEAF_CAP: 31");
    assert!(json_content.contains(r#""BRANCH_L3_CAP": 3"#), "JSON must contain BRANCH_L3_CAP: 3");
    assert!(json_content.contains(r#""BRANCH_L7_CAP": 7"#), "JSON must contain BRANCH_L7_CAP: 7");
    assert!(json_content.contains(r#""BRANCHB_UP": 192"#), "JSON must contain BRANCHB_UP: 192");
    assert!(json_content.contains(r#""MAX_LEVEL": 8"#), "JSON must contain MAX_LEVEL: 8");
}
...
fn test_benchmark_coverage_sync() {
    ...
    let benches_content = fs::read_to_string(&benches_path)
