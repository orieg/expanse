...
2. **Striping the bins and the freelists is the production standard; sharded allocator counters remain rejected**:
   ...
   - **Still rejected on the current engine, now for a measured reason rather than the one above (2026-09-16, Refs #930).** The first bullet blamed a thread-local read; that explanation does not survive. What the cells measure is that **the per-writer accounting itself is the floor**: with neither the deferred gate nor the boxing indirection, 13 of the 16 arms the best variant fails are already above 0.5 % (`docs/benchmarks/concurrency/README.md` §11.10.2). Every way of rearranging it has now been measured and none reaches the §2.7 bound — inline (4,864 B), boxed (696 B), one merged `OnceLock` (712 B), padded back to 728 B, with the slot hoisted, at 64 shards and at 16, gated and ungated. Three hypotheses were tested with controls built to falsify them and all three were refuted: the footprint confound (`c5b`), the field-offset shift (`c12`, which matches its twin to 16 Ir in 10.6 M), and the smaller array (S = 16, which leaves the same 27 plain arms above 0.5 % as S = 64). The gate is not waste — it is what holds the §2.1 zero-overhead invariant, cutting plain-arm regressions from 27–28 to 2. **Two of the three counters (`live_allocs`, `total_allocs`) have no non-test callers**, so the open question is no longer how to shard the accounting but whether it belongs on the hot path at all. Nothing is promoted.
