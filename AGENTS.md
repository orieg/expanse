...
2. **Striping the bins is the production standard; sharded allocator counters remain rejected**:
   ...
   - **Per-stripe collector freelists measured a minor secondary effect**: the per-class freelist mutex (`Collector::pop_freelist`), ablated as arm (c), accounted for only 9–11% at $W=8$ vs 62–73% for the bins, and remains a diagnostic ablation.
