...
| `allow-regression:` citation is **fresh** — the cited run concluded (not `cancelled`/`failure`) at a commit reachable from the PR head, and a cited data artifact post-dates the branch's newest `crates/`/`include/` change | **CI (fatal)** | `scripts/perf_report.py` (and `scripts/wasm_fuel.py`, which shares the check) — every citation in the reason is checked, not the first; an undecidable one degrades to a named `::notice::` (§8.11.5). Pinned by `--self-test`, which carries #822's, #826's and #827's lines verbatim |
