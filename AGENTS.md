...
| Wall-clock runners take the hybrid-host core pin (§8.4; the P/E exposure is 1.576×) | **CI** | `lint` job → `scripts/check_bench_pin.py`; the pin itself is `scripts/bench_pin.sh`, sourced by every `docs/benchmarks/*/run.sh` and by the bare-metal and AVX-512 bench steps |
