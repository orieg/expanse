...
- ❌ **Global Locking or Coarse Mutexes**: Concurrency must adhere strictly to optimistic concurrency control (OCC seqlock version bracketing + Epoch-Based Reclamation), preserving single-threaded zero-overhead performance. **The protocol is blocking, not lock-free** — writers serialize on one mutex, `SeqVersion::sample` spins unbounded while a bracket is open, and readers fall back to the writer mutex after `MAX_RETRIES`. That is optimistic lock coupling (Leis et al., DaMoN 2016) and is the intended design; what this rule forbids is a *coarse lock on the common read path*, not the bounded fallback. Never describe the protocol as lock-free or obstruction-free.
