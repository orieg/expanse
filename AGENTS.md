...
2. **Striping the bins and the freelists, the sharded allocator counters and the padded writer state are all production standard**:
   ...
   - **Superseded on the current engine (2026-09-15, Refs #930).** The rejection above was measured on the pre-Phase-4C engine and stays as that engine's record (§8.20.6). Re-run at `726b01fc` with one thread per physical core, the same feature — its shard index still read through a thread-local (`occ::writer_slot`) — measured `SINGLE_RUN_PASS` on `map` and `set` at every W ∈ {2, 4, 8} in both of two runs; at W = 8 the paired C(W) ratio is 1.1753 [1.1472, 1.2024] and 1.2069 [1.1577, 1.2459] on `map`, 1.1770 [1.1363, 1.2387] and 1.2073 [1.1466, 1.2757] on `set`. The cost explanation in the bullet above therefore does not describe the current engine. Source: `docs/benchmarks/concurrency/README.md` §11.7. The same builds, and a build combining the two, were re-measured with one harness process per timed cell at `bad1bd3d` (`docs/benchmarks/concurrency/README.md` §11.8, which supersedes §11.7's multi-cell verdicts). The combination was then re-measured at `3fa6a2f2` and **promoted**; see the bullet below and §11.10. The `set` W = 1 loss under the feature alone stays unexplained.
   ...
   - **The sharded allocator counters and the padded writer state were promoted together (Refs #568, #930)**: measured as one configuration at `3fa6a2f2`, one thread per physical P-core, 8 rounds, one harness process per timed cell, two runs. At $W=8$ the paired C(W) ratio, variant ÷ default, is 2.0497 [2.0083, 2.0911] and 2.0608 [2.0104, 2.1000] on `map`, and 2.5182 [2.4459, 2.5998] and 2.5324 [2.4828, 2.5739] on `set` — `SINGLE_RUN_PASS` in both runs at $W \in \{2, 4, 8\}$. **Three arms are not helped, and are published beside it**: `str` is `INCONCLUSIVE` because its two runs disagree, and `bytes` and `blob` — measured on this configuration for the first time — are `INCONCLUSIVE` at every $W$, with one `bytes` $W=8$ cell `REJECTED` in run 2 on an interval wide enough that it is weak evidence of harm rather than established harm. No mechanism is attributed for any arm (§8.9). The inverses are `ablation-unsharded-alloc` and `ablation-unpadded-lock`; the retired names `ablation-sharded-alloc`, `lock-padded` and `ablation-striped-epoch` are `compile_error!`, never no-ops (§2.7). Source: `docs/benchmarks/concurrency/README.md` §11.10.
