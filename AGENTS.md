...
2. **Striping the bins and sharding the counters are measured, and neither is a design to reach for on the contention argument alone**:
   ...
   - **Per-stripe collector freelists measured nothing either**: the per-class freelist mutex (`Collector::pop_freelist`), taken on every size-class allocation under OCC, was ablated as arm (c) and came back `INCONCLUSIVE` at every cell across two runs.
   - That exhausts the three mechanisms this hypothesis named. The multi-writer plateau is **not** explained by shared allocator or reclamation state, which is a result about those mechanisms and not a measurement of what does explain it. Shared state no arm touched — the system allocator behind a `pop_freelist` miss, the reader registry, the `epoch` and `op_count` words — and the gate convoy that §10 does measure are where the next arm belongs.
