...
2. **Striping the bins and the freelists is the production standard; sharded allocator counters remain rejected**:
   ...
   - **Promoted to the production default in its deferred-gated form (2026-09-16, Refs #568, #930).** The rejection above described the *inline* layout and its stated cause — "a per-writer stripe costs a thread-local read" — did not survive re-measurement; both are superseded here in place (§8.20.6). What ships: `NodeAlloc` keeps its three inline atomics and allocates one cache-line-aligned shard per writer slot **only in `defer_to`**, so a plain tree carries one word rather than 4 KiB and `size_of::<ExpanseMap>()` moves 712 → 728 B, not 712 → 4,864 B. Paired with the padded writer state (`occ::Line<X>`), it measures `map` **24.07 / 23.95** and `set` **32.55 / 31.67** M ops/s at W = 8 against a default of 11.5 and 13.4, clearing #930's ≥ 20 M gate on the BCa lower bound across two runs — `set` at both pins, `map` at the per-core pin, with `map` at `0-15` still `INTERMEDIATE`. `perf c2c` puts cross-core HITM down 12× on `map` and 18× on `set`. **It is promoted over a known, permanent single-threaded cost across three targets**, carried by an `allow-regression:` line rather than hidden: x86 Callgrind 16 arms (all `sync_*`, worst +2.50 %), x86 smoke 2 arms, **aarch64 4 arms — all of them plain, worst `judyl_insert/sequential` +2.57 %** — and **wasm fuel 4 arms, all plain, worst +4.63 %** on a target where `writer_slot()` is a constant and the shards can never be exercised. §2.1 invariant 5 is met on x86 and is **not** met on aarch64 or wasm; removing that cost rather than excusing it is a follow-up. That bound is not reachable — every way of reducing the accounting without per-writer sharding was measured and the best reaches 15.68 M ops/s against a floor of 20, and with neither gate nor indirection present 13 of the 16 arms already fail. The inverses are real: `ablation-unsharded-alloc` never allocates the shards, `ablation-unpadded-lock` unpads `Line<X>`, and the retired names are `compile_error!`s (§2.7). What remains unexplained is why sharding removes ~92 % of a *diffuse* HITM profile; no mechanism is attributed (§8.9). Source: `docs/benchmarks/concurrency/README.md` §11.10.
