...
static void report_json(const char *benchmark, const char *arm, size_t n, double cycles_per_op, size_t heap_used, double fragmentation) {
    printf("{\"benchmark\": \"%s\", \"arm\": \"%s\", \"n\": %zu, \"cycles_per_op\": %.2f, \"heap_used_bytes\": %zu, \"frag_ratio\": %.4f}\n",
           benchmark, arm, n, cycles_per_op, heap_used, fragmentation);
}
...
void bench_tsdb_hardware(size_t n) {
    ...
    report_json("esp32_tsdb_ingest", "expanse_memtable", n, cycles_per_op, heap_used, frag);
    ...
    report_json("esp32_tsdb_aggregate_500", "expanse_memtable", 500, (double)elapsed / 500.0, heap_used, frag);
...
void bench_ble_tracker_hardware(size_t n) {
    ...
    report_json("esp32_ble_sighting_record", "expanse_slab", n, cycles_per_op, heap_used, frag);
    ...
    elapsed = GET_CYCLES() - start;
    report_json("esp32_ble_point_lookup", "expanse_slab", n, (double)elapsed / (double)n, heap_used, frag);
    ...
    elapsed = GET_CYCLES() - start;
    report_json("esp32_ble_ttl_eviction", "expanse_slab", expired, (double)elapsed / (double)(expired > 0 ? expired : 1), heap_used, frag);
