...
- **CI concurrency**: one in-flight run per PR (`concurrency` group keyed on the PR number); a new commit cancels the previous run, so only the branch tip is fully verified. Pushes to `main` are never cancelled — a cancelled required check is not a green one, and those runs are the record that the protected branch was verified. Nightly uses a single queued group so a manual dispatch never runs alongside the cron.
