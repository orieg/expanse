...
__weak HAL_StatusTypeDef MX_LTDC_Init(LTDC_HandleTypeDef *hltdc, uint32_t Width, uint32_t Height)
{
  ...
  /* Use ST's default (non-inverted) pixel clock polarity. The previous
   * inverted setting (IIPC) can cause sampling on the wrong edge and
   * visible horizontal shifts/clipping with some DSI timings. */
...
__weak HAL_StatusTypeDef MX_LTDC_ClockConfig(LTDC_HandleTypeDef *hltdc)
{
  ...
  PeriphClkInitStruct.PLL3.PLL3N = 132U; /* 5 * 132 = 660 MHz VCO */
  PeriphClkInitStruct.PLL3.PLL3P = 2U;
  PeriphClkInitStruct.PLL3.PLL3Q = 2U;
  PeriphClkInitStruct.PLL3.PLL3R = 24U; /* 660 / 24 ≈ 27.5 MHz LTDC clock */
