...
# program: prologue 6 s, six steps of 20 s, summary; grab at 4 s into the prologue, 14 s into each step, and in the summary
python3 "$REPO/integrations/stm32h747/capture.py" "$PORT" "$OUT/transcript.txt" 150 > "$OUT/capture.log" &
CAP=$!
sleep 2
