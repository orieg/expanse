...
3. **Faster Sequential Iteration** (vs the reference SkipList) — **3.1426× [3.1061, 3.2269] and 3.0744× [3.0480, 3.1095]** across two runs:
   - Forward range scans traverse contiguous leaf blocks via intrusive sibling leaf chaining: **176.62 Mops/s [159.08, 182.81] against 56.29 Mops/s [49.27, 58.67]** in run 1, and **182.45 [181.21, 184.70] against 59.35 [58.33, 60.17]** in run 2 *(workload: rocksdb_memtable_single_threaded)*. The previously published ~10× (160.8 against 16.4 Mops/s at commit `7d87dff7`) was measured against the retracted fat-node layout, whose worse cache locality inflated the ratio; against the fair variable-height baseline the edge is **3.1×, not 10×**, and the intermediate `6cb64b45` figure of `3.331×` is superseded too. An unordered `VectorRep` append-vector scans faster still (630.21 Mops/s [625.30, 633.72]) but cannot serve ordered seeks.
...
**In one paragraph.** ExpanseMemTable achieves **1.42× higher key density in RAM** than a fair variable-height skiplist baseline (13.2 against 18.7 B/entry, deterministic byte accounting) and, across two runs, **3.1426× / 3.0744× faster sequential scans** ([3.1061, 3.2269] and [3.0480, 3.1095]), **1.4915× / 1.4985× faster point lookups** ([1.4901, 1.4939] and [1.4913, 1.5073]), and **1.5318× / 1.5348× faster range seeks** ([1.5225, 1.5377] and [1.5268, 1.5414]) *(measured: reference host — Intel i9-12900F, pin `0-15`, commit `7cd5140e`, 5 rounds per run, runs [35547165132](https://github.com/orieg/expanse/actions/runs/35547165132) and [35547235205](https://github.com/orieg/expanse/actions/runs/35547235205); (workload: rocksdb_memtable_single_threaded); artifacts [`docs/benchmarks/rocksdb_memtable/results/baseline_rocksdb.json`](../../docs/benchmarks/rocksdb_memtable/results/baseline_rocksdb.json) and [`…_run2.json`](../../docs/benchmarks/rocksdb_memtable/results/baseline_rocksdb_run2.json))*. `VectorRep` (unordered append vector) scans faster and is denser still (10.5 B/entry) but cannot serve ordered seeks. The earlier `6cb64b45` figures are superseded, and batch scan is a loss against them (`2.524×` → 2.13× in both runs); the suite's `METHODOLOGY.md` §6 reads every row against rule 18.
...
The optimistic reader protocol in `ExpanseMemTableRep` is safe under concurrent writes. The earlier race — `SplitLeafBlock` nulling source entries before lowering the block count (stale-count `nullptr` deref in `Get`), and `Insert`'s in-place shift leaving the leaf array transiently unsorted (spurious binary-search miss) — was resolved by a **per-leaf seqlock with publication ordering** (#234) and **release/acquire happens-before on the reader path** (#236): each `LeafBlock` carries a `version` counter and its `entries`/`count` are published with `memory_order_release` and read with `memory_order_acquire`, so a reader either observes a consistent snapshot or retries. A `Get` that continues into the next block starts after the last entry it already passed to the callback, not at the lookup key again, so a split between a block's validation and its `next_leaf` load cannot deliver an entry twice; `tests/test_memtable_park_points.cc` forces that interleaving under every seek lock scope. This is exercised in CI by the RocksDB memtable matrix under ThreadSanitizer (`test-rocksdb-memtable`, `sanitizer: tsan`) and a differential test. Issue [#229](https://github.com/orieg/expanse/issues/229) is closed.

The seqlock is not the whole read path, and the distinction matters for anyone reasoning about read scaling. `Contains`, `Get` and `Iterator::Seek` all locate their leaf through `FindLeafBlockForSeek`, which takes the same `mutex_` that `Insert` holds for its entire body — the binary search, the entry shift, the trie insert and `SplitLeafBlock`. Readers therefore serialise against writers *before* they reach the per-leaf bracket, and what the bracket buys is a consistent in-block scan, not contention-free reads. Nothing here is lock-free, and AGENTS.md §2.2 forbids describing it that way. Making the locate phase genuinely optimistic is tracked in [#802](https://github.com/orieg/expanse/issues/802), which also records that this integration has no concurrent benchmark arm to measure such a change with.
