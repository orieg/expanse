...
- **Ordered range scans are the systematic loss.** Against ART (`blart` 0.5.0), short scans at k=10 go to ART at **1.96×–2.09×** on every distribution — an unpredicted loss with the mechanism unmeasured *(measured: reference host; workload: `art_scan`)*. Against HOT the loss is wider than pre-registered: 27 of HOT's 30 integer-key wins are scan cells, still losing at k=1000 (HOT ÷ Expanse 0.41–0.50 on random keys at 100k), and on string keys HOT wins **72 of 72** scan cells, down to 0.019 of Expanse's time *(measured: reference host — Intel i9-12900F, HOT `96bf6fb`; workloads: `hot_latency`, `hot_string_latency`)*. The string-key scan measures the shipped `ExpanseStrMap` navigation surface, which re-descends from the root and allocates a key per element; a cursor iterator is the open engine item.
- **Point lookup and insertion mostly go to Expanse.** ART: point lookups 1.54×–3.21× and sequential insert 4.84× in Expanse's favour at 1M keys. HOT integer keys: 109 of 144 latency cells to Expanse, 30 to HOT, 5 within the interval of parity. HOT string keys: `skewed` point lookup 1.17×–1.97× to Expanse at 1M and `prefixed` insert 1.20×–1.29×, while `prefixed` point lookup goes to HOT (0.76×) — HOT's design regime *(measured: reference host; workloads: `hot_latency`, `hot_string_latency`)*.
