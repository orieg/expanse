...
- **Against Masstree the integer-key regimes split by operation, and insertion order decides two of them.** Point lookup goes to Expanse by **3.2×–12.4×** at 1M keys on every distribution. Ordered scan goes to Expanse on every structured distribution and on `random` at 1M (1.1×–2.5×) — the pre-registered scan loss refuted on 20 of 24 registered cells — and to Masstree on `random` at 10k and 100k (0.59×–0.84×, one boundary). Insertion in the sorted order every suite builds in goes to Masstree on `random`, `sparse` and `clustered` keys (0.66×–0.97×), an unpredicted loss; on a shuffled permutation of the same keys the `random` cell is 1.88× to Expanse, because sorted insertion is a B+-tree's best case — and the shuffled order moves both arms' insertion cost and Expanse's allocator footprint as well, so every latency verdict here is a sorted-order verdict. Memory: Masstree holds a flat 22.8 bytes/key of nodes at every density in that sorted build (33.1 at random-order leaf fill); Expanse is below it for λ ∈ [8, 30] (16.3–19.4) and above it past the `LEAF_CAP` cascade (23.8–24.7) *(measured: reference host — Intel i9-12900F, Masstree `1119842`, harness commit `82966aae`; workload: `masstree_map_64bit`)*.
- **On string keys Masstree wins insertion and scans, Expanse wins lookups, and the long-shared-prefix loss the issue expected did not land.** Insertion 0.42×–0.88× and every scan cell 0.04×–0.56× to Masstree; point lookup 1.25× (`short`), 1.40× (`skewed`) and 1.053× (`prefixed`, the registered loss refuted narrowly) to Expanse; `short`-key memory 33.9 bytes/key against Expanse's 69.2 to Masstree *(measured: reference host; workload: `masstree_str_map`)*. Masstree's declared key limit is 255 bytes; keys beyond it are refused at the call and the `beyond` shape publishes Expanse alone, as a predicate on the competitor rather than a restriction on the arm.
