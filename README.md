...
- **Against Masstree the integer-key regimes split by operation, and insertion order decides two of them.** Point lookup goes to Expanse by **3.2×–12.4×** at 1M keys on every distribution, and ordered scan goes to Expanse in every cell (1.1×–2.5×) — the pre-registered scan loss refuted in Expanse's favour. Insertion in the sorted order every suite builds in goes to Masstree on `random`, `sparse` and `clustered` keys (0.66×–0.97×), an unpredicted loss; on a shuffled permutation of the same keys the `random` cell is 1.88× to Expanse, because sorted insertion is a B+-tree's best case. Memory: Masstree holds a flat 22.8 bytes/key of nodes at every density; Expanse is below it for λ ∈ [8, 30] (16.3–19.4) and above it past the `LEAF_CAP` cascade (23.8–24.7) *(measured: reference host — Intel i9-12900F, Masstree `1119842`, harness commit `82966aae`; workload: `masstree_map_64bit`)*.
