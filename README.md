...
**Phase 7 — OCC concurrent reads (tree-level).** Done: type-tag encoding, geometry constants, digit extraction (Phase 1); SIMD byte-vector search (SSE2/NEON with portable parity-tested fallbacks) and the 256-bit bitmap with popcount rank/select and ordered navigation (Phase 2); cache-line-native node layouts — the 16-byte tagged `Edge` (the literature's "Judy Pointer") with level-split pop0/decode aux field, 64/128-byte linear and bitmap branches, bitmap leaves, all layout-proven by compile-time asserts (Phase 3; variable-length linear leaves land with the Phase 5 allocator); the read-only lookup engine — set-flavor `test` and map-flavor `get` over branches, bitmap leaves, immediates, full-expanse and narrow pointers, differentially tested against `BTreeSet`/`BTreeMap` models on hand-built trees and Miri-clean (Phase 4); cache-line-aligned allocation with byte-exact accounting (the future `MemUsed`/bytes-per-key source) plus the header-less linear-leaf layout and its lookup integration, leak-checked under Miri (Phase 5); the set-flavor mutation engine — `ExpanseSet` with insert/remove over the full compression ladder (immediate → linear leaf → bitmap leaf → full expanse; branch cascades L3 → L7 → bitmap → uncompressed) with 1-index delete hysteresis, a root-leaf small-population form, a structural invariant validator with a CI negative control, and model-based differential tests against `BTreeSet` across the TESTING.md key distributions (Phase 6); the map-flavor engine and `ExpanseMap` — insert-returns-old/get/remove-returns-old semantics, map immediates (value in word 0 or value-array pointer), `[values][keys]` map leaves, `LeafBitmapL` value subarrays, differentially tested against `BTreeMap` (Phase 6b); ordered navigation — first/last/next/prev, ascending iterators, O(depth) rank (`count_below`/`count_range`) and select (`by_count`) on both types, differentially tested against the `BTree` models (the engine behind the compat `First/Next/Last/Prev/Count/ByCount` surface). Phase 8 in progress: `libexpanse` now **exports the full Judy1 and JudyL families** (30 entry points incl. the `*Empty` searches) with the shipped clean-room `Judy.h`, and the COMPAT.md gate G1 **differential oracle runs in CI** — randomized op sequences through libexpanse and a dlopen'd stock libjudy must agree exactly (they do). The bench harness is in (criterion grids + deterministic bytes/key + a dlopen'd head-to-head vs stock libjudy), and **leaf-targeted narrow-pointer synthesis** landed with measured wins: clustered keys dropped from 1.34 to 0.35 B/key and clustered lookup went from 5.3× slower than stock libjudy to parity (BENCHMARKING.md). `JudySL*` is exported too, backed by `ExpanseStrMap` (byte-lexicographic string map as a meta-trie of word maps) and oracle-validated byte-for-byte against stock libjudy; the insert path is capacity-classed with in-place shifts and a fused single-walk `JudyLIns` (insert ratios vs stock: clustered 1.7×, random 1.35×). **Branch-targeted narrow pointers** landed next (branches carry their own level in the header; cascades branch at the keys' true divergence level; divergence inside a skipped prefix splits at the highest diverging level — wide 4096-key clusters dropped 0.19 → 0.12 B/key), followed by **Phase 7 concurrency**: `SyncExpanseSet`/`SyncExpanseMap` give one serialized writer plus lock-free validated readers over a tree-level seqlock with epoch-based reclamation, loom-model-checked (the model caught two real ordering bugs pre-merge) and thread-stress-tested in CI. **JudyHS** completed the compat surface: all four families (Judy1/JudyL/JudySL/JudyHS) are exported and oracle-validated against stock libjudy, JudyHS backed by `ExpanseBytesMap` (an unordered byte-string map: 64-bit-hash-keyed word trie over byte-exact collision buckets). **Gate G2 is green**: the php-judy extension builds unmodified against libexpanse (a `--with-judy` prefix of our clean-room `Judy.h` + `libJudy` alias of libexpanse) and its full test suite passes — 221/221 (macOS AArch64, PHP 8.5) and in the Linux `php-judy-compat` CI job. **Gate G4** (preload smoke) is also green: an unmodified consumer built against stock libjudy runs identically under `LD_PRELOAD` of libexpanse. **Gate G3** followed: the Windows CI job builds php-judy against `expanse.dll` (no bundled C libjudy) and its suite passes — **all four COMPAT.md acceptance gates are green as standing CI jobs**, and the per-node OCC refinement landed with a measured ~700× recovery of read throughput under writer churn. The **modern `expanse.h` API** now ships alongside the compat surface: typed handles, rank/select on ordered types, byte-exact memory accounting, and lock-free concurrent readers — the capabilities that make switching worthwhile rather than merely safe (a C smoke consumer exercises all of them in CI). Remaining: a first lookup/insert profiling pass and quiet-host headline numbers (BENCHMARKING.md), Testing now spans all six layers: **fuzzing** (three cargo-fuzz targets, 60s per push and 20 min per target nightly) and **property tests with shrinking** landed with the Phase 8 hardening pass, joining the model harness, differential oracle, Miri, and loom.
