...
- **OCC Reads, Optimistic Writers (optimistic lock coupling)**: Optimistic concurrency control — **validated readers take no lock on the common path**; `SyncExpanseSet` and `SyncExpanseMap` admit concurrent writers through optimistic lock coupling (`docs/ARCHITECTURE.md` §4.2), while the blob, string and bytes wrappers serialise writers on one mutex. At 100% read on bounded-keyspace workloads at 16 threads, `SyncExpanseMap` reaches **361 M ops/s, C(16) = 9.44 [9.43, 9.46]** in both of two runs, `SyncExpanseBytesMap` 126 M / 11.03–11.08× and `SyncExpanseStrMap` 73.3–75.6 M / 10.63–10.99×, against `DashMap` at 129–130 M / 8.39–8.46×; `SyncExpanseSet` measured 562 and 415 M ops/s in the two runs, a level they do not agree on. **Honest limit:** at 50/50 the two multi-writer arms scale to C(16) = 1.93–2.23 while `DashMap` reaches 7.77–7.80, and the single-writer arms lose throughput as threads are added (0.27–0.65) *(measured: reference host — Intel i9-12900F, runs [34881026495](https://github.com/orieg/expanse/actions/runs/34881026495) and [34882381735](https://github.com/orieg/expanse/actions/runs/34882381735), commit `76432c5c`, `docs/benchmarks/concurrency/results/baseline_concurrent_mixed.json`; workload: `core_concurrency`)*. See the concurrency section below.
- **Dense Memory Packing**: Down to **0.07–0.36 bytes/key** on 64-bit sets *(measured: Apple M1, `bytes_per_key` example, commit 6c63826a)* and **~0.31 bytes/key** on clustered 32-bit embedded sets *(measured: `bytes_per_key_32`, commit `27019b23`)*. Those are the dense and clustered distributions the target names; on **uniform random keys** per-key cost is a sawtooth in expanse occupancy λ = N / 2¹⁶, and the same `ExpanseSet` spans **7.64–21.02 bytes/key** under density alone *(measured: deterministic byte accounting, `keyspace_density` example, commit 16003bc0; [docs/ARCHITECTURE.md §3.5](docs/ARCHITECTURE.md#35-per-key-memory-is-a-sawtooth-in-expanse-occupancy-and-leaf_cap-sets-the-tooth))*.
...
| **RocksDB Pluggable MemTable** | [`integrations/rocksdb`](integrations/rocksdb) (`rocksdb-expanse`) | Official RocksDB `MemTableRep` / `MemTableRepFactory` implementation. **1.42× higher key density in RAM** than a fair variable-height skiplist baseline (13.2 vs 18.7 B/entry, deterministic accounting). Fewer L0 flushes is inferred (target). Against that same fair baseline, point lookup is **1.457× (BCa 95% CI [1.444, 1.470])**, range seek **1.512× [1.492, 1.546]** and sequential scan **3.331× [3.198, 3.486]** *(measured: reference host — Intel i9-12900F, run [33398474866](https://github.com/orieg/expanse/actions/runs/33398474866), commit `6cb64b45`, 5 rounds; artifact [`docs/benchmarks/rocksdb_memtable/results/baseline_rocksdb.json`](docs/benchmarks/rocksdb_memtable/results/baseline_rocksdb.json))*. See [`docs/benchmarks/rocksdb_memtable/`](docs/benchmarks/rocksdb_memtable/README.md) and [`integrations/rocksdb/`](integrations/rocksdb/README.md) |
...
- **Ordered range scans are the systematic loss against HOT, and no longer against ART.** The ART short-scan loss published here before #745 is **formally retracted**: it was measured from a single start key on every round, so a k = 10 window held ten element visits between two clock reads. Re-measured with starts that scale as `max(1000, 10^6/k)`, every ART scan cell at 1M keys goes to Expanse, k = 10 by **1.50×–1.53×** on structured keys and 2.67× on random, and the full-iteration cells are unchanged *(measured: reference host, `b447dbc0`; workload: `art_scan`; the k = 0 control reproduces the superseded run within 2%)*. Against HOT the loss is wider than pre-registered: 28 of HOT's 29 integer-key wins are scan cells, still losing at k=1000 (HOT ÷ Expanse 0.41–0.53 on random keys at 100k), and on string keys HOT wins **72 of 72** scan cells, down to 0.017 of Expanse's time *(measured: reference host — Intel i9-12900F, HOT `96bf6fb`; workloads: `hot_latency`, `hot_string_latency`)*. The string-key scan measures the shipped `ExpanseStrMap` navigation surface, which re-descends from the root and allocates a key per element; a cursor iterator is the open engine item.
- **Point lookup and insertion mostly go to Expanse.** ART: point lookups 1.54×–3.21× and sequential insert 4.84× in Expanse's favour at 1M keys. HOT integer keys: 111 of 144 latency cells to Expanse, 30 to HOT, 3 within the interval of parity — including uniform-random map lookup at 1M, which was an Expanse win at 1.399 before the arm timed first was alternated per round and claims no winner after it, 0.992 [0.977, 1.007]; two independent runs of the alternating harness put that cell at 0.993 [0.977, 1.009] and 0.992 [0.977, 1.007], so the boundary is not one run's accident. HOT string keys, at 1M and by arm: `skewed` point lookup 1.15×–1.95× to Expanse on the two `ExpanseStrMap` arms, while the `ExpanseBytesMap` arm splits (0.99× to HOT on the hit path, 1.16× to Expanse at 50/50); `prefixed` insert 1.21×–1.29× to Expanse on those same two arms and 0.74× to HOT on the bytes arm; `prefixed` point lookup goes to HOT on the shipped string arm (0.77×) — HOT's design regime *(measured: reference host — Intel i9-12900F, HOT `96bf6fb`, harness commit `0f4fd40c`; workloads: `hot_latency`, `hot_string_latency`)*.
- **Memory depends on which side of the density cascade the cell sits.** HOT's set arm holds a flat 11.7–12.1 bytes/key across a 2.7× swing in Expanse's footprint, and its map arm is flat too at 35.7–35.9 where the heap-allocated value pair dominates; Expanse wins the integer-set census only for λ ∈ [8, 23] and loses outside it (§3.5 above). On short string keys HOT's ownership footprint is **36.2 bytes/key against Expanse's 69.2** — `ExpanseStrMap` allocates twice per key not resolved in a terminal chunk *(measured: reference host — Intel i9-12900F, harness commit `0f4fd40c`; workload: `hot_str_ptr`)*. HOT truncates string keys at 255 bytes and silently drops longer ones (1 of 1,000 held on the `beyond` shape); that is reported as a capability finding about HOT, and the Expanse arm is not restricted to match it.
- **Against Masstree the integer-key regimes split by operation, and insertion order decides two of them.** Point lookup goes to Expanse by **3.2×–13.6×** at 1M keys on every distribution. Ordered scan goes to Expanse on every structured distribution and on `random` at 1M (1.1×–2.6×) — the pre-registered scan loss refuted on 21 of 24 registered cells — and to Masstree on `random` at 10k (0.60×–0.95×) and at 100k from k = 100 (0.61×). Insertion in the sorted order every suite builds in goes to Masstree on `random`, `sparse` and `clustered` keys (0.67×–0.97×), an unpredicted loss; on a shuffled permutation of the same keys the `random` cell is 1.88× to Expanse, because sorted insertion is a B+-tree's best case — and the shuffled order moves both arms' insertion cost and Expanse's allocator footprint as well, so every latency verdict here is a sorted-order verdict. Memory: Masstree holds a flat 22.8 bytes/key of nodes at every density in that sorted build (33.1 at random-order leaf fill); Expanse is below it for λ ∈ [8, 30] (16.3–19.4) and above it past the `LEAF_CAP` cascade (23.8–24.7) *(measured: reference host — Intel i9-12900F, Masstree `1119842`, harness commit `2ce92b7f`; workload: `masstree_map_64bit`)*.
- **On string keys Masstree wins insertion and scans, Expanse wins lookups, and the long-shared-prefix loss the issue expected did not land.** Insertion 0.42×–0.88× and every scan cell 0.04×–0.55× to Masstree; point lookup 1.25× (`short`), 1.41× (`skewed`) and 1.06× (`prefixed`, the registered loss refuted narrowly) to Expanse; `short`-key memory 33.9 bytes/key against Expanse's 69.2 to Masstree *(measured: reference host — Intel i9-12900F, Masstree `1119842`, harness commit `2ce92b7f`; workload: `masstree_str_map`)*. Masstree's declared key limit is 255 bytes; keys beyond it are refused at the call and the `beyond` shape publishes Expanse alone, as a predicate on the competitor rather than a restriction on the arm.
- **Write concurrency is a measured loss against HOT-ROWEX and against Masstree**; see the next section.
...
How often the fallback fires has been measured on dedicated-writer workloads, and the answer carries a caveat that matters more than the number. In both FFI suites' health cells — one to eight writers with eight readers, on the diagnostic `occ-stats` build, two runs each — **zero reads took the writer mutex at every writer count**, and the restart share sat in a 5–15% band that the two runs order differently *(measured: reference host — Intel i9-12900F, tree at `a1982ff2`, [`docs/benchmarks/hot_comparison/README.md` §7.3](docs/benchmarks/hot_comparison/README.md) and [`docs/benchmarks/masstree_comparison/README.md` §7](docs/benchmarks/masstree_comparison/README.md); workloads: `hot_rowex_set_63bit`, `hot_rowex_map_64bit`, `masstree_conc_map_64bit`)*. **That zero is true by construction, not by measurement.** A fallback needs 64 consecutive failed optimistic walks, and at the bracket lengths a single writer holds, 64 in a row is negligible by construction: the falsifier would have read zero whatever the engine did. It is labelled `PASS_categorical_by_design` and it licenses no claim that the fallback is rare in general — only that it did not fire where it could not. The `read_fallbacks` and `locked_reads` counters remain compiled out by default (`--features occ-stats`) and are not exercised by CI; `locked_reads` also counts the unconditional `with_locked` / `len` / `mem_used` paths that never attempt an optimistic walk, and read zero at these cells.

What the readers *are* doing under one writer has now been measured rather than inferred. With one writer and eight readers, the readers spend **53–60% of their time spinning on the odd tree-level version** — the pre-registered floor was 50% — which is 64–76% of their per-probe slowdown against the same readers alone; restarts are 2–4% of it and 20–34% is unattributed. The writer, for its part, retires almost the same instructions per insert with readers present and takes 2.6× the cycles, with RFO misses per insert rising from about 2 to about 12; `perf c2c` puts about half of all contended-line samples on the one line holding the tree version and the writer mutex, and an eighth on the root-snapshot line *(measured: reference host — Intel i9-12900F, tree at `a1982ff2`, two runs of every sweep cell, seven rounds of every per-thread counter cell, the pre-registration and every table in [`docs/benchmarks/concurrency/`](docs/benchmarks/concurrency/README.md); workloads: `hot_rowex_set_63bit`, `hot_rowex_map_64bit`, `masstree_conc_map_64bit`)*.

**Against a trie that admits concurrent writers, the single writer is a measured loss.** HOT's ROWEX variant (concurrent insert and lookup, no deletion) against `SyncExpanseSet` / `SyncExpanseMap` on the reference host: Expanse wins with one writer (**1.56×** set, **1.76×** map) and loses from two writers (0.57× / 0.75×) down to **0.084× / 0.130×** at sixteen; one writer alongside eight readers collapses Expanse's set-arm readers to roughly a seventh of their reader-only rate while ROWEX's hold most of theirs — the ratio is 0.140–0.142 across the suite's two runs of that cell, and the arm is cited as a direction and a range rather than a level because four of the ten C2 reader cells moved past their own intervals between two runs taken at the same commit *(measured: reference host — Intel i9-12900F, 8 P-cores, harness commit `d3bc49c0` for both runs, 15 interleaved rounds, both runs and the between-run spread in [docs/benchmarks/hot_comparison/README.md §7](docs/benchmarks/hot_comparison/README.md); workload: `hot_rowex_set_63bit`)*. Masstree, the second route to the same claim, measures it wider: on the identical cells Expanse keeps the single integer writer (**1.13×**) and loses from two (0.41×) to **0.075×** at sixteen, and on `short` string keys loses from one writer (0.88×) to **0.019×** at sixteen; one writer alongside eight readers takes Expanse's integer readers from 142.8 to 18.7 M ops/s while Masstree's keep 43.2 of 58.4, and that writer itself falls from 5.66 to 1.86 M inserts/s; the health counters show the readers spending most of their time waiting on the writer's open tree-level bracket (the paragraph above), and the arm has since been re-measured twice at `a1982ff2` with every cell's direction holding *(measured: reference host — Intel i9-12900F, 8 P-cores, harness commit `2ce92b7f` for the levels quoted, `a1982ff2` for the replication pair, 15 interleaved rounds, BCa 95% intervals in [docs/benchmarks/masstree_comparison/README.md §7](docs/benchmarks/masstree_comparison/README.md); workloads: `masstree_conc_map_64bit`, `masstree_conc_str`)*. Per-node write locks — full optimistic lock coupling — are tracked in [#568](https://github.com/orieg/expanse/issues/568), and that baseline is the one its gate is measured against. All arms use bounded keyspaces *(measured: reference host — Intel i9-12900F, runs [34881026495](https://github.com/orieg/expanse/actions/runs/34881026495) and [34882381735](https://github.com/orieg/expanse/actions/runs/34882381735), commit `76432c5c`; workload: `core_concurrency`)*. Correction history for the earlier unbounded-keyspace figures: [docs/BENCHMARKING.md](docs/BENCHMARKING.md).

| arm | 1 Thread | 16 Threads, run 1 | 16 Threads, run 2 | Scaling, run 1 / run 2 |
|---|---:|---:|---:|---:|
| `SyncExpanseSet` (100% read) | 76.4 M ops/s | 562 M ops/s | 415 M ops/s | 7.36× / 5.44× (runs disagree) |
| `SyncExpanseMap` (100% read) | 38.2 M ops/s | **361 M ops/s** | **361 M ops/s** | **9.44× / 9.44×** |
| `SyncExpanseBlobMap` (100% read) | 31.0 M ops/s | 306 M ops/s | 307 M ops/s | 9.86× / 9.87× |
| `SyncExpanseBytesMap` (100% read) | 11.4 M ops/s | 126 M ops/s | 126 M ops/s | 11.08× / 11.03× |
| `SyncExpanseStrMap` (100% read) | 6.89 M ops/s | 73.3 M ops/s | 75.6 M ops/s | 10.63× / 10.99× |
| `DashMap<Vec<u8>, u64>` (100% read) | 15.4 M ops/s | 130 M ops/s | 129 M ops/s | 8.46× / 8.39× |
| `Mutex<Expanse*>` baselines (100% read) | 8.88–41.2 M ops/s | 2.68–5.41 M ops/s | 2.67–5.27 M ops/s | 0.13×–0.30× (collapse) |
| `SyncExpanseMap` (50R/50W mixed) | 28.0 M ops/s | **62.5 M ops/s** | **62.2 M ops/s** | **2.23× / 2.23×** |
| `SyncExpanseSet` (50R/50W mixed) | 42.9 M ops/s | 82.9 M ops/s | 84.8 M ops/s | 1.93× / 1.98× |
| `SyncExpanseBlobMap` (50R/50W mixed) | 27.2 M ops/s | 7.31 M ops/s | 7.21 M ops/s | 0.27× / 0.27× |
| `DashMap` (50R/50W mixed) | 10.7 M ops/s | 83.6 M ops/s | 83.6 M ops/s | 7.80× / 7.77× |

- **Read-only OCC scaling holds to sixteen threads on bounded-keyspace workloads.** Every `Sync*` arm scales at 100% read — `SyncExpanseMap` C(16) = 9.44 [9.43, 9.46] in both runs, `SyncExpanseStrMap` and `SyncExpanseBytesMap` 10.63–11.08, against 8.39–8.46 for `DashMap` — while the `Mutex<Expanse*>` baselines fall below their single-thread throughput (0.13–0.30). `SyncExpanseSet` scales in both runs, but its level does not reproduce: 7.36 [7.25, 7.46] and 5.44 [4.37, 6.47] (`docs/benchmarks/concurrency/results/baseline_concurrent_mixed.json`; workload: `core_concurrency`).
...
- **Write-mixed scaling is the honest limit.** At 50/50 the two multi-writer arms scale — `SyncExpanseMap` C(16) = 2.23 [1.92, 2.43] and `SyncExpanseSet` 1.93 [1.78, 2.07] in run 1, with run 2's estimates inside both intervals — and the single-writer blob, string and bytes arms lose throughput as threads are added (0.27–0.65 in both runs). `DashMap` (7.77–7.80) and `SkipMap` (8.54) scale further than either Expanse arm, and what limits the Expanse arms is unmeasured (workload: `core_concurrency`).
