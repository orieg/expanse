...
- **Point lookup and insertion mostly go to Expanse.** ART: point lookups 1.54×–3.21× and sequential insert 4.84× in Expanse's favour at 1M keys. HOT integer keys: 109 of 144 latency cells to Expanse, 29 to HOT, 6 within the interval of parity — including uniform-random map lookup at 1M, which was an Expanse win at 1.399 before the arm timed first was alternated per round and is 0.993 [0.977, 1.009] after it. HOT string keys: `skewed` point lookup 1.17×–1.97× to Expanse at 1M and `prefixed` insert 1.21×–1.30×, while `prefixed` point lookup goes to HOT (0.77×) — HOT's design regime *(measured: reference host; workloads: `hot_latency`, `hot_string_latency`)*.
- **Memory depends on which side of the density cascade the cell sits.** HOT holds a flat 11.7–12.1 bytes/key across a 2.7× swing in Expanse's footprint; Expanse wins the integer-set census only for λ ∈ [8, 23] and loses outside it (§3.5 above). On short string keys HOT's ownership footprint is **36.2 bytes/key against Expanse's 69.2** — `ExpanseStrMap` allocates twice per key not resolved in a terminal chunk *(measured: reference host; workload: `hot_str_ptr`)*. HOT truncates string keys at 255 bytes and silently drops longer ones (1 of 1,000 held on the `beyond` shape); that is reported as a capability finding about HOT, and the Expanse arm is not restricted to match it.
