...
- **Sparse / clustered (<0.1% density)**: Expanse point lookups (`contains`) beat Roaring Bitmaps, on direct tagged-pointer immediate storage; on **dense** sets Roaring's bit containers win `contains`; and Roaring's specialized rank index makes its **`rank`/`select` faster** than Expanse's `count_below`/`by_count` — use Expanse for membership, Roaring for heavy rank/select. **The multipliers are withheld pending re-measurement (#758)**: they were measured at a 100% hit rate by a harness that has since been corrected to 50% hit with misses drawn from the population's own generator, and the miss half is where a trie and a bitmap diverge most. The ordering of the three claims is unaffected; the numbers are re-run, not re-estimated.
