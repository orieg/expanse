...
| `test-aarch64` | Core / AArch64 Linux Tests & Callgrind (Neoverse N2) | `ubuntu-24.04-arm` native execution — hardware capability census (`lscpu`, `/proc/cpuinfo`, `CTR_EL0` cache-line size), full workspace tests (`PROPTEST_CASES=500`), SIMD NEON parity execution (closes AGENTS.md §5 parity gap), C ABI drop-in verification, C++20 header tests, and deterministic Callgrind instruction tracking on AArch64. |
...
- **AArch64 / NEON execution & Callgrind tracking** — delivered on GitHub-hosted ARM64 Linux runner (`ubuntu-24.04-arm`, Neoverse N2) with native workspace tests, NEON SIMD parity execution, and Callgrind instruction tracking (#397).
