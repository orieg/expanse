...
| `ycsb-concurrent-selftest` | Perf / YCSB Concurrent Instrument Self-Test | `docs/benchmarks/concurrency/scripts/ycsb_concurrent.py --self-test`, testing gate formulas, artifact schemas, process isolation, role enforcement, and mutation-tested error paths (`METHODOLOGY.md` §20.12). Gated on `ycsb-concurrent-instrument`. |
