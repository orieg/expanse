...
| [`codeql.yml`](../.github/workflows/codeql.yml) | `push`, `pull_request`, `schedule`, `workflow_dispatch` | CodeQL code scanning. A pull request analyses only the languages whose files it changes, and a docs-only one analyses none; pushes to `main` and the weekly schedule analyse all nine |
