...
| `fast-lane` | Core / Fast Lane Passed | The one decision point for the slow lanes: every job other than `detect-changes`, `lint`, `docs-lint` and `ci-gate` lists it in `needs`, so when it is skipped they all skip. It runs when `lint` passed or was skipped, `docs-lint` passed, the event is not a push, and the pull request is not a draft. `scripts/check_ci_filters.py` fails when a job omits it from `needs` or when its `if:` loses one of those clauses; `scripts/check_gate_floor.py` judges its result (§3). |
