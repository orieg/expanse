...
| Job | Name | Role |
|---|---|---|
| `detect-changes` | Subsystems / Change Detection | `dorny/paths-filter@v4` computes per-subsystem outputs (`rust-src`, `tooling`, `perf-tooling`, `concurrency-instrument`, `visualizer`, `python`, `node`, `dotnet`, `java`, `ruby`, `php`, `wasm`, `go`, `integrations`, `esp-component`) that downstream jobs gate on. |
...
| Job | Name | Role |
|---|---|---|
| `ci-gate` | CI Gate / All Checks Passed | Runs `if: always()`, `needs:` all 42 other jobs, treats cleanly-skipped jobs as passing, and runs the completeness self-check. The **only** required branch-protection context. |
