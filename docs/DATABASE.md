...
**Measured host-side ingest loss, reported per §8.7:** the `ExpanseMap32` ingest+flush cycle at $N=2\text{k}$ measures **219.1 µs** [218.5, 220.8] against `BTreeMap` **62.2 µs** [62.1, 62.2] and reserved `HashMap` **6.2 µs** [6.19, 6.20] — 3.5× and 35.4× slower respectively *(measured: reference host — Intel i9-12900F, 24 threads, 30 MiB L3, Linux 6.8, run [33515865311](https://github.com/orieg/expanse/actions/runs/33515865311), commit `12013487`; BCa 95% CIs, artifact `docs/benchmarks/embedded/results.json`)*. The point-lookup concession to a reserved hash map was pre-registered (#556 §3.2); the ingest gap to `BTreeMap` was **not**, and stands as an open finding for the 32-bit write path. The winning regimes remain memory density (panels 1–2) and $O(\text{expired})$ TTL eviction; whether this wall-clock ordering holds on a cache-light 160 MHz target is exactly what the pending hardware harvest measures.
