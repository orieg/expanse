...
**Architectural Benefits in RocksDB LSM Storage** *(memory density: deterministic seeded byte accounting against the fair variable-height skiplist baseline, at the [#372](https://github.com/orieg/expanse/issues/372) fix commit — Apple M1, 8 cores, Apple clang 21, `-O3`, load-immune, reproduced twice. Throughput cells: **re-measured** against the fair variable-height skiplist baseline at commit `7cd5140e`, pin `0-15`, five rounds with BCa 95% intervals and paired per-round ratio intervals, in **two independent runs** — reference host, runs [35547165132](https://github.com/orieg/expanse/actions/runs/35547165132) and [35547235205](https://github.com/orieg/expanse/actions/runs/35547235205), artifacts [`docs/benchmarks/rocksdb_memtable/results/baseline_rocksdb.json`](benchmarks/rocksdb_memtable/results/baseline_rocksdb.json) and [`…_run2.json`](benchmarks/rocksdb_memtable/results/baseline_rocksdb_run2.json). The baseline reports 18.7 B/entry, not the retracted 146.7 B/entry fat-node strawman, so these cells stand on the same footing as the density figure. The earlier `6cb64b45` figures are superseded)*:
...
The Yahoo! Cloud Serving Benchmark evaluates engine behaviour under real-world access distributions across analytical caches, session stores, time-series appends, and read-modify-write transactional workloads *(measured: reference host — Intel i9-12900F, 24 threads, 30 MiB L3, Ubuntu 22.04 / kernel 6.8, commit `21a382f3`, runs [35419627251](https://github.com/orieg/expanse/actions/runs/35419627251) and [35419631571](https://github.com/orieg/expanse/actions/runs/35419631571), pinned CPUs `0,2,4,6,8,10,12,14`, 8 paired rounds with BCa 95% bootstrap intervals, Refs #1005)*:

```
Workload Comparison (Throughput in Mops/sec, N = 100,000, 128B Blobs, Shuffled):
────────────────────────────────────────────────────────────────────────────────────────────────────
Workload                     ExpanseMap (u64)    ExpanseBlobMap (128B)    BTreeMap (128B)    SkipMap (RocksDB)
────────────────────────────────────────────────────────────────────────────────────────────────────
A (50% Read, 50% Update)         38.73 Mops/s         17.94 Mops/s          14.04 Mops/s        3.09 Mops/s
B (95% Read, 5% Update)          47.41 Mops/s         30.27 Mops/s          15.63 Mops/s        5.15 Mops/s
C (100% Read)                    48.94 Mops/s         31.84 Mops/s          15.86 Mops/s        5.68 Mops/s
D (95% Read Latest, 5% Ins)      65.83 Mops/s         34.79 Mops/s          17.79 Mops/s        7.29 Mops/s
E (95% Range Scan, 5% Ins)        0.87 Mops/s          0.70 Mops/s           1.58 Mops/s        0.33 Mops/s
F (50% Read, 50% RMW)            33.45 Mops/s         15.48 Mops/s          15.05 Mops/s        2.75 Mops/s
────────────────────────────────────────────────────────────────────────────────────────────────────
```

Under sorted ascending insertion order, throughputs remain consistent: Workload A 37.77 / 17.05 / 13.27 / 3.28; Workload B 47.45 / 29.22 / 14.62 / 5.69; Workload C 48.67 / 31.43 / 14.90 / 6.42; Workload D 66.07 / 35.39 / 15.58 / 7.83; Workload E 0.94 / 0.78 / 1.87 / 0.40; Workload F 33.36 / 15.42 / 13.79 / 2.89.
...
>
> On dense clustered keys (`benches/ycsb_dense.rs`, workload: `workload_ycsb_dense`), key clustering amortizes leaf transitions (~322 records per leaf, 0.003 transitions/record), **inverting the result in Expanse's favor**:
...
> **Per-operation latency percentiles** are measured under 64-op window means (calibrated bracket ~23.5 ns, residual overhead <0.37 ns/op).
> The historical p95–p99.9 tail of ~38–41 µs (e.g. 37,755 ns on A and 38,224 ns on F from commit `43b46f38`) was an artifact of uncalibrated single-op brackets and unmasked write errors. Under window-mean sampling with valid writes, `ExpanseBlobMap` p95 is **64.5 ns** on Workload A and **72.4 ns** on Workload F, with slow windows (>8× median) counting only ~7–8 out of 3,125 windows per round. The unmeasured slab allocation stall hypothesis is formally retracted.
