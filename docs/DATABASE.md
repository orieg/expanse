...
- **Read-only** OCC access scales at 16 threads in both runs: `SyncExpanseMap` **361 M ops/s, C(16) = 9.44 [9.43, 9.46]**, `SyncExpanseBytesMap` **126 M ops/s (11.03–11.08×)**, `SyncExpanseStrMap` **73.3–75.6 M ops/s (10.63–10.99×)**; `SyncExpanseSet` measured 562 and 415 M ops/s, a level the two runs do not agree on. Coarse-mutex baselines fall to **0.13×–0.30×**.
- **Write-mixed is the weak regime.** At 50/50, `SyncExpanseMap` and `SyncExpanseSet`, which admit concurrent writers (`docs/ARCHITECTURE.md` §4.2), scale to C(16) = **2.23** and **1.93–1.98**, while the single-writer blob, string and bytes wrappers fall to **0.27×–0.65×** and `DashMap` (**7.77×–7.80×**) and `SkipMap` (**8.54×**) scale further than either (workload: `core_concurrency`). The 50/50 figure is a **mixed-operation rate** — every thread picks a read or a write per operation in one loop — not a read-scaling result.
...
- **MVCC Engine Visibility**: Reader validation without a lock on the common path, scaling on bounded-keyspace read workloads at 16 threads — `SyncExpanseMap` 361 M ops/s (C(16) = 9.44 [9.43, 9.46]), `SyncExpanseBytesMap` 126 M ops/s (11.03–11.08×), `SyncExpanseStrMap` 73.3–75.6 M ops/s (10.63–10.99×) (workload: `core_concurrency`) — vs coarse-mutex baselines falling to 0.13×–0.30× *(measured: reference host — Intel i9-12900F, runs [34881026495](https://github.com/orieg/expanse/actions/runs/34881026495) and [34882381735](https://github.com/orieg/expanse/actions/runs/34882381735), commit `76432c5c`, `docs/benchmarks/concurrency/results/baseline_concurrent_mixed.json`)*. Write-mixed workloads are the weak regime (0.27×–2.23× at 50/50 across the `Sync*` arms) — see §3.2.
