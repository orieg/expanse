...
**Measured host-side wall-clock outcomes, reported per §8.7** *(measured: reference host — Intel i9-12900F, 24 threads, 30 MiB L3, Linux 6.8, run [33550404527](https://github.com/orieg/expanse/actions/runs/33550404527), commit `1402a606`; BCa 95% CIs, artifact `docs/benchmarks/embedded/results.json`)*:

- **Ingest** (`ExpanseMap32`, per event incl. flush share, $N=2\text{k}$): **41.3 ns** vs `BTreeMap` 28.3 ns and reserved `HashMap` 3.1 ns — 1.46× and 13.1× slower. The hash-map concession was pre-registered (#556 §3.2); the `BTreeMap` gap was not pre-registered and remains an open finding, though the in-place leaf mutation fix ([#584](https://github.com/orieg/expanse/pull/584), #577 candidate 1) cut it from the 3.5× first measured at commit `a191bae0` — a 2.63× ingest improvement whose conservative cross-run CI bound is [2.61, 2.65].
- **CAN dispatch lookup** ($N=500$, per op): **12.8 ns** vs `HashMap` 1.4 ns and `BTreeMap` 6.9 ns — the pre-registered lookup concession, plus an unregistered 1.8× gap to the ordered competitor. Unchanged by #584 (the read path was untouched).
- **TTL eviction — the pre-registered winning regime still does not win on this host in either shape, though #584 halved both losses.** Bulk (600 of 2k expired, symmetric cutoffs): dual-trie **87.5 µs** vs full sweep **7.3 µs** (11.9× slower; was 25.8×). Steady state (25 of 2k expired — the regime the $O(\text{expired})$ claim is about): dual-trie **3.9 µs** vs sweep **3.0 µs** (1.30× slower; was 2.5×). The sweep walks a ~68 KiB flat table that is L2-resident on this host at ~1.6 ns/entry; its cost scales with $N$ while the dual-trie's scales with the expired count, so the projected crossover sits at larger populations — and a cache-light 160 MHz target changes the constants entirely. Whether the asymptotic story survives on-device is exactly what the pending hardware harvest measures.
