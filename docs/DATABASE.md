...
- **BLE point lookup: 1.67× faster than the hash** (2,127 vs 3,548) and 52×
...
4. **Zero-Copy Batch Scan Extraction**: `ScanBatch` extracts keys and values at **111.8 Mops/s** with zero redundant varint re-parsing *(measured: reference host, commit 7d87dff7; no skiplist comparison involved)*. Pending re-measurement ([#868](https://github.com/orieg/expanse/issues/868)): `ScanBatch` has changed since `7d87dff7`.
...
> ⚠️ **Harness methodology disclosure — every figure and ratio in this subsection is pending re-run (#1005).** The run predates three corrections on the measured path: 255 of every 256 `ExpanseBlobMap` writes in workloads A, B, D and E were rejected with `MetaOverflow` and the `Result` discarded; every arm's teardown of 100,000 entries was inside the criterion timed region, by an amount that is unmeasured and not symmetric by construction; and workload D never read a key the run had inserted. `docs/BENCHMARKING.md` ("Standardized YCSB Workload Suite", §2) carries the evidence for each. The earlier statement here that the comparative ratios were sound because a payload-dereference omission was symmetric (#470) covered that one defect only and does not extend to these; the ratios below are derived from the table, carry no interval, and are deferred to the re-run rather than defended.
...
> **Workload E is a loss.** `BTreeMap` leads `ExpanseMap` **1.55×** on range scans (1.289 / 0.830, `u64` values against 128 B blobs) and leads `ExpanseBlobMap` 1.96× (1.289 / 0.657, blob against blob; workload: `workload_ycsb`; criterion medians with no interval, pending re-run (#1005)). The earlier 4.33× and 3.08× figures, both in Expanse's favour, were withdrawn once [#385](https://github.com/orieg/expanse/pull/385) fixed a scan bound that had every arm traversing one record per scan against a mean requested length of 54.9. The cause is structural: uniform-random 64-bit keys give ~1.43 records per leaf, so a 55-record scan pays ~27.6 leaf transitions where a B-tree walks contiguous arrays. Dense and clustered distributions yield ~322 records/leaf and do not share this behaviour — **this is a sparse-key result, not a general scan result**, though no dense YCSB workload E cell exists yet: `benches/ycsb_dense.rs` (workload `workload_ycsb_dense`) is that cell, pending its first run (#1005). Choose `BTreeMap` for scan-dominated workloads over sparse random keys.

> **Per-operation latency percentiles** are pending re-run (#1005). The published table (`docs/BENCHMARKING.md`, "Standardized YCSB Workload Suite", §2) was measured at commit `43b46f38`, before the #385 and #470 changes to its measured path, with an `Instant` bracket around every op that costs about as much as the point lookups it timed. It shows an `ExpanseBlobMap` tail of ~38–41 µs at p95–p99.9 on the 50%-write mixes (A, F); those cells are pending re-run (#1005) and **their cause is unmeasured** — the arena slab-chunk allocation stall previously named here had no counter, census or profile behind it and is withdrawn as an explanation. The cells are also inconsistent with the throughput table above: 5% of ops at ≥ 37,755 ns puts the mean at ≥ ~1,888 ns per op, at most ~0.53 Mops/s against the 20.38 Mops/s in the table (workloads differ: `workload_ycsb` at `43b46f38` under a per-op bracket vs `workload_ycsb` at `6462c4ee` under criterion batches). Do not select or reject an engine on these percentiles until the re-run lands.

**Key Architectural Insights for Database Engineers:**
1. **Workload D**: `ExpanseBlobMap` measures 5.02× `BTreeMap`'s throughput (21.50 / 4.28 Mops/s; workload: `workload_ycsb`) — a criterion median with no interval, pending re-run (#1005), on a D cell that was not read-latest and whose blob inserts were rejected (disclosure above). The mechanism previously stated for it (trie leaf appends avoiding page-split stalls) was never measured and is withdrawn (§8.9.1).
2. **Against the RocksDB `SkipMap` model**: `ExpanseBlobMap` measures 8.14× to 11.14× `crossbeam_skiplist::SkipMap`'s throughput across Workloads B, C, D and F (B 8.35×, C 10.74×, D 11.14×, F 8.14×, each the quotient of its two table cells; workload: `workload_ycsb`) — criterion medians with no interval, pending re-run (#1005). The margin is host- and payload-dependent.
