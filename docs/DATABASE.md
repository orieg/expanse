...
- **TTL eviction — the pre-registered $O(\text{expired})$ regime now wins on this host, through the batched path.** Steady state (25 of 2k expired — the regime the claim is about): `remove_range` ([#589](https://github.com/orieg/expanse/pull/589), #578) evicts in **1.06 µs** vs the full sweep's **3.2 µs** — **3.0× faster**, conservative CI-bounded ratio [3.03, 3.04]; the per-key `first`/`remove` loop, kept as its own arm, is at 3.0 µs (0.94× of the sweep, from 1.16× slower before its `first()` stopped paying a second descent). Bulk (600 of 2k expired, symmetric cutoffs) is still a loss: `remove_range` **23.6 µs**, per-key loop 68 µs, vs sweep **7.1 µs** (3.35× slower batched; 11.3× before #578). The sweep walks a ~68 KiB flat table that is L2-resident on this host at ~1.6 ns/entry; its cost scales with $N$ while the dual-trie's scales with the expired count — and with batching that constant is now small enough to win the steady regime even here, while the bulk regime's per-record hash-keyed `by_mac` removal (inherently random access) keeps it behind the sweep. Whether the bulk story flips on cache-light silicon is what the pending hardware harvest measures.
