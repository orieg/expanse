...
> **Model scope.** The table above is a *pure DRAM-traffic* model: it assumes the only per-entry cost is bytes moved, so its speedups (up to $46\times$ at $\sigma=0.1\%$) are an **upper bound that ignores index traversal**. Measured reality (§10.3, RESOLVED): a correct in-slot filter that walks the trie per entry lands at **~10.3× at σ=0.05 / ~22× at σ=0.001** cold — below the idealized $46\times$ (the trie walk is not free) but *clearing the RFC ≥10× target*, because in the cold regime the ~1.5 ns/entry walk is small against a ~34 ns cold-payload fetch. (An earlier draft of this note predicted ~4–5×; that under-counted the payload/traversal ratio in the cold regime and was corrected by measurement.)

### 5.5 Configurable Metadata Layout — `CompactInSlot` (default) & `BlobLeafVector` — *proposed (#282)*

**Status:** Phase 1 (`CompactInSlot` / `ArenaMeta`) **implemented (#287) and meets the RFC `>10×` target** (measured ~10.3× at σ=0.05, ~22× at σ=0.001 — §10.3, RESOLVED). Phase 2 (`BlobLeafVector`) is **not needed** (go/no-go = NO-GO, §5.5.10) — retained below as a deferred, documented option only. Tracked in #282 / #285.
...
- **`CompactInSlot` (default)** — a new quantized `ValueSlot` encoding that carries 24-bit metadata *and* a 32-bit locator in the existing 64-bit slot (§5.5.7). It reuses the current leaf machinery unchanged (no new leaf type), fixes the filter, and — measured — **already achieves the RFC `>10×` target** (~10.3× at σ=0.05, ~22× at σ=0.001; §10.3 RESOLVED). This is the shipped default and covers point-lookup/write-heavy, sparse, embedded, ≤24-bit-metadata, *and* the cold-DRAM analytical scan workloads.
...
> **This section's original premise was falsified by measurement (§10.3, RESOLVED).** It argued the payload-skip win was only ~4–5× (traversal-floor bounded), so `BlobLeaf32`'s SIMD scan was needed to reach ≥10×. **Wrong.** The ~4–5× was extrapolated from the *warm*-arena 1.4× ceiling; in the *cold* regime the in-slot-meta trie walk costs ~1.5 ns/entry against a ~34 ns cold payload fetch, so skipping non-matching payloads alone yields **~22× at σ=0.001 / ~10.3× at σ=0.05 — Phase 1 (`ArenaMeta`) already, with no leaf change.** The reasoning below is retained for the record but does not describe the shipped engine.

The original argument (now known to mis-weight the traversal cost): `BlobLeaf32` does not eliminate the trie walk; it iterates leaves in key order and performs $N/\text{pop}$ inter-leaf descents, so its ceiling was expected to be **occupancy-sensitive** — "approaches ≥10× at high leaf occupancy, degrades toward ~4–5× as occupancy drops." Since the payload-skip alone already clears ≥10×, the marginal gain of the SIMD meta scan is confined to shaving the ~1.5 ns/entry traversal, which only moves the needle at high σ (where the ≥10× target does not apply). Hence the NO-GO in §5.5.10.
...
- **Phase 1 — `CompactInSlot` (default). ✅ Implemented (#287); meets the target.** The `ArenaMeta` encoding (sole arena encoding, tag `0x10`, replacing `ArenaShort`/`ArenaLong`) + 24-bit-meta in-slot read + error-at-insert overflow policy. Fixes the correctness bug (metadata survives across the whole arena) **and, measured, achieves the RFC `>10× at σ≤0.05` target on its own — ~10.3× at σ=0.05, ~22× at σ=0.001** (§10.3, RESOLVED), with no new leaf type.
- **Phase 2 — `BlobLeafVector` (opt-in). ❌ Not needed (go/no-go = NO-GO, #285).** The Phase-2 gate was: does a SIMD columnar leaf beat Phase 1 enough to justify a second leaf layout? Measurement answered no — **Phase 1 already clears ≥10× at σ≤0.05**, because in the cold regime the trie walk is ~1.5 ns/entry against a ~34 ns cold payload, so it was never the floor. `BlobLeafVector`'s only marginal gain (SIMD-scanning the meta column to shave that ~1.5 ns/entry) matters solely at *high* σ, where most payloads must be touched anyway and the ≥10× target does not apply. Building a new SoA leaf — the most `unsafe`/optimized code in the crate — for that is unjustified. **It is also an *architectural* NO-GO:** the SoA leaf is a multi-word leaf array that co-locates columnar metadata in the trie index, tripping two `AGENTS.md` §2.2 anti-patterns (Fat Slots, Complecting Index and Columnar Attributes; see the §5.5 alignment callout). The §2.2-sanctioned home for wide/multi-column metadata is a **decoupled sidecar** — pursued in #295, which also covers the >64 GiB axis. The `BlobLeafVector` design is retained here only as a *documented record of a rejected approach*; it is not on the roadmap, and a future need for wide/multi-column/key-ordered metadata should be met by the sidecar (#295), not by fattening the leaf.
...
2. **Graduation:** nothing graduates now — consistent with #295's parked status,
   ...
   in-slot metadata is faster — keep it. The POC stays feature-gated
   (`poc-meta-sidecar`, non-default, out of the public API), re-runnable when a
