...
| §9 `SeqVersion32` / OCC primitives | **Shipped** | `occ32.rs` provides the 32-bit seqlock version word; sampling is bounded (`try_sample`), never spinning, per the interrupt-handler contract. The concurrent wrapper brackets at **tree** granularity (one `SeqVersion32` per container, `sync32.rs`); there are no per-node version words, and the `version` field `BranchB32` carries is bracketed by nothing |
