...
**RISC-V parts only** — ESP32-C2/C3 (RV32IMC, built as `riscv32imc-unknown-none-elf`), ESP32-C6/H2 (RV32IMAC, built as `riscv32imac-unknown-none-elf`), and ESP32-P4 (RV32IMAFC, currently built as `riscv32imac-unknown-none-elf` — a strict instruction subset, with the float-ABI caveat in [`components/expanse/README.md`](../components/expanse/README.md)). Per-part ISA and core counts are sourced to the Espressif datasheets/TRMs in [`docs/HARDWARE.md` §4.3](HARDWARE.md#43-espressif-risc-v-per-part-core-inventory--cas-soundness--validated-567). The Xtensa ESP32/S2/S3 have no mainline rustc target; the configure step fails with a named error rather than registering a component with no engine behind it.
