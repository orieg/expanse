...
**`SyncExpanseBytesMap`** completes the family for unordered byte keys: one validated walk over the hash trie, then a byte-exact comparison against a collision bucket that is write-once after publication. Structural changes publish a replacement bucket and retire the old shell, entries and key buffers through the same collector. Only value words mutate in place, covered by the reader's final tree-version validation. Under multi-writer OLC (Refs #929 Task C), concurrent writers execute via optimistic CAS bucket replacement: writers allocate an immutable replacement bucket and publish it into the hash trie with atomic CAS under `OlcHost`, retiring superseded buckets through the epoch collector and falling back to serial writer mutex synchronization on root transitions, trie conversions, or retry exhaustion; `ablation-bytes-serial-writers` restores the single serial writer mutex.
...
- **Optimistic CAS Publication:** A writer looks up or inserts the hash bucket entry. To insert a new key or overwrite an existing value, the writer allocates a fresh immutable `Bucket` containing the updated entries, and publishes it via `olc_cas_publish_map` (which uses atomic CAS on the parent node's value slot or edge).
- **Contention and Backoff:** If another writer updates the bucket concurrently, the CAS fails; the writer discards the unpublished bucket via `free_bytes_unpublished`, re-reads the bucket state, and retries with exponential backoff up to `MAX_RETRIES`.
