...
**Who holds the tree word.** The word lives in the `Collector`, on a cache line it shares only with the epoch: the registry mutex, the reclamation bins the writer locks on every retire, the free-list atomics and the retained-bytes counter start on the next line, because every reader samples that word on every walk and a per-retire store on its line was measured as a reader collapse on the string wrapper (`docs/benchmarks/concurrency/README.md` §8; `layout_report`'s test pins the layout). `SyncExpanseSet` and `SyncExpanseMap` defer their tree with `defer_to_engine_root`: the engine brackets root-state writes itself and an ordinary insert or remove never stores to the tree word, so readers on other subtrees never wait on it. `SyncExpanseStrMap`, `SyncExpanseBytesMap` and `SyncExpanseBlobMap` keep the whole-operation tree bracket in `Shared::write` — their reads validate several sub-tries and the arena metadata against that one word (#730 is where the string wrapper's bracket is narrowed) — and their inner trees, deferred without the engine-root flag, treat `Cover::Tree` as a no-op. `clear` on every wrapper goes through the bracketed `write`, a root-state change covered by the wrapper. Writers still serialise on one mutex: this is the single-writer protocol with brief brackets, not lock coupling — the writer-count cells are unchanged by it and #568 stays open until per-node write locks land.
