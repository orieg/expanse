...
**The string wrapper (Refs #929).** `SyncExpanseStrMap` runs the OLC bodies above on each `StrNode`'s sub-map: `olc_insert_map` and `olc_remove_map` are generic over an `OlcHost` — the tree word's parity, the top edge, the allocator and the dirty mark — and a `StrNode` hosts them with its **cover word** standing where the tree word stands for the map wrapper. The word is a `u32` of the kind every branch header carries, at offset 0 of the node, so a reader or writer holding only the `*mut StrNode` it decoded from a parent's tagged entry reaches it with no field arithmetic. A writer enters the gate, pins an epoch and couples hand-over-hand down the chunk chain: at each node it samples the cover, copies the sub-map root state and looks the chunk up under that cover. Which transition follows (`benchmarks/concurrency/METHODOLOGY.md` §17.2.1) decides the store's cover:
