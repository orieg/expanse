...
**Memory-model soundness of the shared paths: the class-1 plan (#1086).** The shared paths still reach one class of undefined behaviour: plain loads that race a writer's plain stores (the seqlock pattern: sound at the protocol level, a data race under the Rust memory model; the census's `race` entries). The second class, references over shared state, is closed for point readers, optimistic writers and lock holders. It has two residuals. The covered writer's `&mut Edge` / `&mut StrNode` is pinned by `map_covered_reader_writer` / `set_covered_reader_writer` with a positive control (`occ_stats::Stat::ReadCoverOverlaps`). The map's ordered reads (`sync_nav`, #900) read `BranchB` and bitmap-leaf bitmaps through `Bitmap256`'s `&self` methods rather than `bits::shared_bitmap`; `map_ordered_tree_reader_writer` reports it under all three checks (#1142). What follows is the design for class 1, fixed before any code.
...
| `ARENA_ALIGN` | 16 | `crates/expanse/src/blobmap.rs:484` |
| `ARENA_META_CEILING` | 68719476736 | `crates/expanse/src/blobmap.rs:489` |
| `MAX_ARENA_CHUNKS` | 65536 | `crates/expanse/src/blobmap.rs:496` |
| `MAX_ARENA_CAPACITY` | 1073741824 | `crates/expanse/src/blobmap.rs:507` |
| `DEFAULT_CHUNK_SIZE` | 2097152 | `crates/expanse/src/blobmap.rs:479` |
