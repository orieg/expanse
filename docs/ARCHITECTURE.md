...
| 21.36 | 67% | 7.66 | 1.4M @64 |
| 24.41 | 76% | 8.46 · 8.49 · 8.47 | 1.6M @64 · 800k @63 · 400k @62 |
| 27.00 | 84% | 10.12 | 884,736 @63 (exact λ) |
| 27.47 | 86% | 10.51 · 10.53 | 1.8M @64 · 900k @63 — the knee |
| 30.52 | 95% | 13.60 · 13.60 | 2M @64 · 1M @63 — the `memory-budget` gate's second `random` cell, in the cascade's mixture regime |
| 33.57 | 105% | 16.83 | 2.2M @64 |
| 36.62 | 114% | 19.26 · 19.30 · 19.25 | 2.4M @64 · 1.2M @63 · 600k @62 |
| 39.67 | 124% | 20.67 | 2.6M @64 |
| 40.00 | 125% | 20.79 | 1,310,720 @63 (exact λ) |
...
| 1,953 | 61× | 8.80 | 2M @58 — every level-6 expanse is a `BranchU` over packed `Leaf5` leaves |
| 3,906 | 122× | 6.89 | 2M @57 |
| 4,688 | 146× | **6.71** | 1.2M @56 — the second trough, below the first |
| 6,641 | 208× | 8.44 | 1.7M @56 — the second knee: 10% of level-5 sub-expanses cascaded |
| 7,812 | 244× | 12.98 | 2M @56 — the second tooth: 34.9% of level-5 sub-expanses cascaded |
| 10,547 | 330× | 20.98 | 2.7M @56 — 90.6% cascaded; the second peak |
...
**Who holds the tree word.** The word heads the wrapper's `Shared` block, which is `repr(C)`, line-aligned and boxed: it shares its cache line with the root snapshot, so a reader's sample, root load and validate touch one line at fixed offsets from the pointer the reader already holds, and the writer-private words (the mutex, the holder token, the advance tick) follow the tree on a line no reader samples. The engine reaches the word through a pointer bound once at construction (`NodeAlloc::bind_tree_word`), which is why the block is boxed — the address must survive the wrapper moving (`layout_report`'s test pins the layout). The engine is monomorphized per sharing mode, decided once per operation where the runtime dispatch always sat, so the unshared path pays one load and one branch and nothing else. `SyncExpanseSet` and `SyncExpanseMap` defer their tree with `defer_to_engine_root`: the wrapper holds the tree word only while the root is a leaf (every store is then a root-state write), the engine brackets the two root-state changes a remove can make, and an ordinary tree-state insert or remove never stores to the tree word — readers on other subtrees never wait on it, and every per-node bracket is *brief*, around the frame's own stores. `SyncExpanseStrMap`, `SyncExpanseBytesMap` and `SyncExpanseBlobMap` keep the whole-operation tree bracket in `Shared::write` — their reads validate several sub-tries and the arena metadata against that one word (#730 is where the string wrapper's bracket is narrowed) — and their inner trees run the engine's *nested* mode: a node's word stays odd across the whole descent beneath it, as before #568 PR 3. That is not a leftover: with brief brackets under a whole-operation tree word, a reader that used to wait at an odd node instead restarts its whole multi-trie walk (measured as a reader loss on the string wrapper during #568 PR 3's development; the PR records it). The string wrapper's readers under a saturating writer progress only between that writer's brackets, and what that window is worth depends on the writer's own coherence stalls — the whole-operation bracket, not the layout, is the mechanism, and #730 is where it is removed. The two modes are one engine with a compile-time flag; a tree in one mode never sees the other's brackets. `clear` on every wrapper goes through the bracketed `write`, a root-state change covered by the wrapper. Writers still serialise on one mutex: this is the single-writer protocol with brief brackets, not lock coupling — the writer-count cells are unchanged by it and #568 stays open until per-node write locks land.
...
**`WriterGate` and the two exclusive paths.** A writer publishes through an in-flight slot of the tree's `WriterTable`, allocated on the thread's first write and never freed; once all `MAX_WRITER_SLOTS` are allocated, later threads hash onto taken slots, so two live writers can share one. A slot's word therefore *counts* its in-flight writers rather than flagging one. At operation entry a writer increments its slot, executes a `SeqCst` fence, and re-reads the gate; a closed gate makes it decrement and wait. A reader that exhausts `MAX_RETRIES` takes an exclusive `fallback_mutex`, closes the gate, fences, waits for every allocated slot to reach zero with an `Acquire` load (pairing with the exit's `Release`: the exclusive section takes no lock an optimistic writer released, so this is the edge that orders that writer's node stores before its reads), reads, reopens, releases — so two fallers-back never race each other on the drain, and readers still within their retries are untouched. `with_locked` takes the same mutex and gate. The fallback rate at W = 16 is measured before `MAX_RETRIES` is fixed (§10 names the cell).
...
| S9 | the writer drain never completes while a writer is in flight, including two writers sharing one slot | `loom_shared_slot_quiescence` — red when `exit_writer` or the closed-gate back-out stores 0 instead of decrementing; `quiesce_waits_for_a_writer_sharing_an_exhausted_slot` drives it through an exhausted `WriterTable` |
| S10 | a drained writer's stores happen before the exclusive section that drained it | `loom_quiesce_drain_acquires_writer_exit` — red when the load in `WriterGate::wait_drained` is `Relaxed` |
...
The models cover the gate primitives, not `Shared::enter_writer`. That function caches a thread's slot in a `std::thread_local!`, which loom's model threads share and which outlives one model iteration, and every gate has id 1 under loom. A model driving it would put every writer on one slot and, after the first iteration, skip `allocate_slot`, so the drain mask would be empty.
...
5. **Phase 7 OCC** — done, including the per-node refinement.
   ...
   - One tree-level version was the correctness landing. The measured collapse under writer churn — single-reader reads fell to 3.4 K/s against a saturating writer (BENCHMARKING.md) — then justified the **per-node refinement**: `BranchU` gained a version header (4 KiB + one line), writers bracket their stores with the version of the node containing them (§4.1's covering function; until #568 PR 3 the bracket also spanned the recursion beneath the node), and readers validate hand-over-hand, node by node, falling back to the tree version only for the root snapshot. Bracketing is skipped entirely on single-threaded trees via `NodeAlloc::occ_enabled`. Measured: single-reader churn throughput ~700× (3.4 K/s → 2.4 M/s). Under multi-writer OLC (Phase 4A, #568), insertions into unoccupied slots of an already-allocated `BranchU` execute in-place under the local node version lock with zero heap allocation, zero ancestor locking, zero EBR retirement (nothing is replaced), and zero obsolete marking (no node is unlinked) — that is why it is safe without the machinery subsequent Phase 4 variants require.
...
- **Inverted Indexes & Posting Lists (`ExpanseSet`, `ExpanseSet32`)**: Tracks document IDs at **0.07–0.36 bytes/docID** on clustered/dense sets (outperforming Roaring Bitmaps) with native pairwise and $k$-way aggregate set algebra (`intersection_len_many`, `union_len_many`, `intersection_many`, `union_many`, #610) executed directly over compressed trie edges and $O(\text{depth})$ skip-scans (`next_at_or_after`).
