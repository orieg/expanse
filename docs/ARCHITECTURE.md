...
**The string wrapper's per-`StrNode` cover word (groundwork, issue #929).** Stage B above is `SyncExpanseSet` and `SyncExpanseMap` only; `SyncExpanseStrMap` still serialises every mutation on one writer mutex (§4.1). Two structural facts blocked even reaching the OLC route for it, and both are now addressed. First, `ExpanseStrMap` implemented no `RootState`, so `write_root_covered` and every `olc_*` path could not be instantiated for it at all; it does now, answering `false` — its root is an `Option<Box<StrNode>>` naming the meta-trie's first node, never an engine `Root::Tree`, so the tree-level bracket stays open across a fallback operation and covers the root's creation and removal, which carry no bracket of their own. Second, a sub-map's root state (the `Root` variant, a root leaf, the top edge) is covered by the *tree-level* word, which the engine reaches through `NodeAlloc::bind_tree_word` — and `ExpanseStrMap` shares one `NodeAlloc` across every sub-trie, so one allocator is one bound word and every `StrNode` in the meta-trie would contend on it. A `StrNode` therefore now heads its map core with a **cover word of its own**: a plain `u32` of exactly the kind every branch header already carries, even when stable and odd while one frame stores, addressed through `occ::version_cell` and bracketed by the same `version_begin`/`version_end` pair, so the existing `Cover::Node` machinery, reader primitives and loom models apply to it unchanged. It sits at offset 0 for the reason the tree word heads `Shared`: a writer or reader holding only the `*mut StrNode` decoded from a parent's tagged continuation entry reaches the word with no field arithmetic, on the line it is about to read the map root from. The node stays inside one cache line (the map core plus one alignment prefix), which `str_node_is_the_map_core_plus_its_cover_word` pins as a derived bound rather than a literal. **Nothing bumps the word yet**: routing stores onto the per-node covers, moving the readers' cross-hop validation to them, and marking an unlinked node's word obsolete before it retires (property S3) are one behaviour change, pre-registered in [`benchmarks/concurrency/METHODOLOGY.md`](benchmarks/concurrency/METHODOLOGY.md) §17, and `deferred_str_node_cover_words_stay_even_and_unbumped` is what turns red when it arrives.
