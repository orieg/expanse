...
**Known shared-path cost: the one-digit U ↔ B band.** A `BranchU` is created above `BRANCHB_UP` = 192 digits and demoted at 191, so a workload whose digit count under one branch oscillates across 191–193 falls back on every crossing, `Upgrade` up and `DemoteU` down, each an exclusive section that quiesces the writers and rebuilds the branch. The plain engine pays the same rebuilds without the quiescence. The Callgrind arm `sync_map_branchu_thrash` holds one top branch in that band and measures the cost per crossing.
