...
3. **Narrow-pointer synthesis in mutation** — **leaf-targeted skips done** (measured: clustered bytes/key 1.34 → 0.35; vs-stock clustered get 5.31× slower → parity): cascades whose keys diverge only in the last byte build one skip-carrying bitmap leaf instead of a branch chain; inserts diverging inside a skipped prefix split the pointer by materializing one branch level per divergence step; shrink conversions absorb decode bytes back into slot-level immediates. Remaining: **branch-targeted** skips (need per-level tags or header-stored levels) for multi-byte-divergence clusters.
