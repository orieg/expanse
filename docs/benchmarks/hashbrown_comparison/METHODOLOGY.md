...
- **Harness as measured from #1005 on** (`crates/expanse/benches/hashbrown_ycsb.rs`, driven by `scripts/ycsb_bench.py --suite hashbrown`): 500,000 dense sequential keys; every cell built in both insertion orders (ascending and Fisher–Yates shuffled), because `BTreeMap` is order-sensitive; rounds with one process per round, the leading arm rotating; workload D reads are Zipfian over recency and reach in-run inserts; a workload E scan takes 10–59 records with no predicate; a per-cell work checksum is asserted equal across arms; intervals are BCa 95% and ratios are paired per round. The committed `results/baseline_ycsb.json` predates all of this and is pending re-run (#1005).
