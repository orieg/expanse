"""
docs/benchmarks/art_comparison/scripts/theme.py

Shared dual-theme SVG styling for the HOT vs Expanse benchmark comparison suite.
...
Palette:
  ...
  - HOT: #2563eb (light) / #3b82f6 (dark)  -- `.b-hot`, this suite's declared accent

The chrome selectors below are byte-identical to the other suites' copies, which
`scripts/check_chart_themes.py` enforces. `.b-hot` is the one addition and is an
...
def svg_header(width: int = 960, height: int = 340, title: str = "Benchmark Chart") -> str:
  ...
  <defs>
    <style>
      ...
      :root[data-theme="dark"] .b-hot, [data-theme="dark"] .b-hot {{ fill: #3b82f6; }}
