...
| **Pillar D: Full-Vocab Apply** | Mask Apply Latency | **Dense Bitmask** | **Confirmed Loss** (by construction), and by a wider margin than the pre-registration assumed. Measured **82.4 µs** per full-vocab apply across 128,000 tokens *(measured: reference host, run [33434306015](https://github.com/orieg/expanse/actions/runs/33434306015), artifact [`results/bench_grammar_masks.json`](results/bench_grammar_masks.json))*. The previously published <0.1 µs came from a dead-code-eliminated loop that recorded 0.0 ns; the repaired arm consumes its masked buffer via `black_box`, and the honest figure is roughly three orders of magnitude larger. The pre-registered expectation that Dense wins raw apply still holds — it is the regime, not the margin, that was mis-stated. |
...
| **Pillar D: Top-100 Intersect** | Candidate ∩ Allowed Set | **RoaringBitmap & ExpanseSet** | Fast candidate filtering via native SIMD set algebra (#339): **Roaring 0.23 µs, ExpanseSet 1.01 µs** — both sub-5 µs as pre-registered, but Roaring is 4.5× faster, so the pillar is a shared win only in the regime sense, not in ranking *(measured: reference host, run [33434306015](https://github.com/orieg/expanse/actions/runs/33434306015))*. |
