...
| **(b) Grammar Full-Vocab Apply** | Dense Bitmask: not measured (prior cell was a dead-code-eliminated loop; re-run pending [#546](https://github.com/orieg/expanse/issues/546)) | ExpanseSet: 1.9 µs (Top-100 SIMD intersect) | **Dense expected to win raw apply by construction; Roaring/Expanse win candidate filtering** | **Expected Loss (measurement pending [#546](https://github.com/orieg/expanse/issues/546))** |
...
|---|---|---|---|---|---|
| **Dense Bitmask (`[u64]` Array)** | 30.61 MB (16.0 KB/state) | 306.1 MB | 1.0× (Baseline) | not measured (prior cell was dead-code-eliminated; re-run pending [#546](https://github.com/orieg/expanse/issues/546)) | N/A (Linear scan) |
