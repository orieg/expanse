...
*(measured: reference host — Intel i9-12900F, 8P+8E / 24 threads, 30 MiB L3, Linux 6.8; commit `0ed8f5e5`; pin `0-15`, the auto `cpu_core` pin: 8 physical P-cores with both SMT siblings of each, so two hardware threads per physical core, and **not** the pre-registered one-sibling pin `0,2,4,6,8,10,12,14` (note below); `benches/bench_memtable_concurrent.cc`; 5 rounds per cell, 60 cells per run; **two independent runs**, [34720727337](https://github.com/orieg/expanse/actions/runs/34720727337) and [34721708502](https://github.com/orieg/expanse/actions/runs/34721708502); artifacts [`results/baseline_concurrent_reads.json`](results/baseline_concurrent_reads.json) and [`results/baseline_concurrent_reads_run2.json`](results/baseline_concurrent_reads_run2.json). Pre-registered in [`METHODOLOGY.md`](METHODOLOGY.md) §5 before the harness existed; outcomes in §5.7.)*

> **Pre-registration deviation: the pin.** [`METHODOLOGY.md`](METHODOLOGY.md) §5.2 pre-registers one hardware thread per physical P-core (`EXPANSE_BENCH_PIN=0,2,4,6,8,10,12,14`). Both runs used `0-15` instead, as both artifacts record. The harness sets no per-thread affinity, so under `0-15` the scheduler can place two threads on one physical core, and SMT is not excluded from the `R = 7` cell as §5.2 intended. This arm serialises every reader on one mutex, the kind of arm AGENTS.md §8.20.5 step 0 names as pin-sensitive. **The gate verdicts are deferred pending re-run** under the pre-registered pin ([#802](https://github.com/orieg/expanse/issues/802)). The figures below are not retracted: they remain what was measured under `0-15`. Full note: [`METHODOLOGY.md`](METHODOLOGY.md) §5.7.
