...
`FindLeafBlockForSeek` (`integrations/rocksdb/src/expanse_memtable.cc:136@0e32070c`) opens with `std::lock_guard<std::mutex> lock(mutex_)`, and is the locate path for `Contains`, `Get` and `IteratorImpl::Seek`. It is the same `mutex_` that `Insert` holds for its whole body. The per-leaf seqlock therefore protects the in-block scan *after* a reader has already serialised, and every cell in §2 is single-threaded, so nothing measured in this suite so far can show a read-scaling change in either direction.
...
- Multi-writer scaling. `InsertConcurrently` is `Insert` verbatim (`:319@0e32070c`), so writers are fully serialised by construction; measuring that is a separate question from the read path.
...
- **Where that address is:** the harness constructs the `ExpanseMemTableRep`, and with it its `mutex_`, on `RunCell`'s stack (`integrations/rocksdb/benches/bench_memtable_concurrent.cc:292@a6898c64`).
...
|---|---|---|---|---|
| `head_` / `tail_` loads and the `h == t` return (`:146@c09f04af`–`:150@c09f04af`, `:168@c09f04af`–`:172@c09f04af`) | locked | locked | consistency with the trie: under the lock, `h == t` means one block exists and the trie is not consulted | acquire loads, no lock. `head_` is stored once, in the constructor (`:77@c09f04af`); `tail_` only by `SplitLeafBlock` (`:302@c09f04af`), a release store after the new block's entries, count and own links are stored (`:280@c09f04af`–`:296@c09f04af`). A reader that loads `tail_` before that store returns the head, and `Get`, `Contains` and `IteratorImpl::Seek` already step forward over `next_leaf` from wherever the locate ends (`:559@c09f04af`, `:438@c09f04af`, `:890@c09f04af`) |
| `expanse_map_prev_at_or_before` on `trie_index_` (`:154@c09f04af`, `:176@c09f04af`) | locked | locked | the one thing `kTrieCall` still needs the lock for: `expanse_map_t` is not safe for a read concurrent with `expanse_map_insert`, which `Insert` (`:369@c09f04af`) and `SplitLeafBlock` (`:310@c09f04af`) call under `mutex_` (`:318@c09f04af`) | `trie_index_` is an `expanse_sync_map_t`, read with `expanse_sync_map_reader_prev_at_or_before` (`include/expanse.h:606@c09f04af`) through the calling thread's reader handle. Its contract: "the entry returned was present, and every key the search passed over was absent, at one instant" (`include/expanse.h:587@c09f04af`–`:596@c09f04af`) |
| the leaf walk, `SettleSeekCandidate` (`:210@c09f04af`–`:253@c09f04af`) | locked | not locked | — | not locked, unchanged |

- **Why the walk's argument carries over.** The four invariants beside `SettleSeekCandidate` (`:191@c09f04af`–`:201@c09f04af`) need the candidate to be a block in the chain, not a candidate taken from a writer-quiescent trie. Every value the trie can hold at any instant names a linked block: `SplitLeafBlock` inserts the new block's prefix (`:310@c09f04af`) after the release store that links it (`:305@c09f04af`), and `Insert` maps a prefix only to the block it just wrote (`:369@c09f04af`). Invariant 1 keeps that block valid for the rep's lifetime. This is an argument, not a proof. The gates below check it, and three of the mutations below are aimed at it directly.
- **The writer.** `Insert` still holds `mutex_` for its whole body. Under `kOptimistic`, its two trie inserts become `expanse_sync_map_insert`, and the ordered read in `FindLeafBlockForInsert` (`:107@c09f04af`) goes through the rep's writer handle (R6), under `mutex_`, so no other trie writer runs beside it.
- **Lock order.** The sync map's fallback read takes its fallback mutex, quiesces writers and takes its writer lock (`include/expanse.h:519@c09f04af`–`:527@c09f04af`; `crates/expanse/src/sync.rs:1815@c09f04af`–`:1825@c09f04af`). No reader holds `mutex_`. The writer path takes `mutex_` and then the sync map's locks, on every path, and uses the rep's own writer handle (R6), so it never takes the handle registry's mutex (R2). A reader takes that registry mutex only on a cache miss, holding no other rep lock, and the handle it creates registers under the collector's registry lock. That is one order per lock pair and no inversion, derived from the code and the handle design below, not tested.
...
- **Other `mutex_` holders.** `SuggestCompactRange` (`:585@c09f04af`–`:596@c09f04af`) does not touch the trie and is unchanged. `ApproximateMemoryUsage` (`:447@c09f04af`–`:453@c09f04af`) is below.

**`ApproximateMemoryUsage` under `kOptimistic`.** It calls `expanse_map_mem_used` under `mutex_` today (`:449@c09f04af`). Under `kOptimistic` it calls `expanse_sync_map_mem_used` instead, still under `mutex_`.

- **What that call costs.** It is a writer-excluding read: it takes the sync map's fallback mutex, quiesces writers and takes the writer lock (`crates/expanse/src/sync.rs:7235@c09f04af`–`:7237@c09f04af` through `read_locked`, `:1815@c09f04af`–`:1825@c09f04af`; the header says so at `include/expanse.h:573@c09f04af`–`:580@c09f04af`). Its lock order is `mutex_` first, the same as `Insert`'s.
...
- **G-O3 — park points on the locate path.** Each park point is compiled only under a test macro. Every test requires `Get`, `Contains`, `Seek` and `SeekForPrev` to find the targets after the resume.
  ...
  - **(c) The prefix remap.** Keys share one 8-byte prefix `p`. Park a reader after its trie read returned block `B` for `p`. The writer first inserts a smaller key with prefix `p` at slot 0 of an earlier block `A`, which remaps `p` to `A` (`:367@c09f04af`–`:369@c09f04af`), and then fills `A` until a split remaps `p` to a later block (`:307@c09f04af`–`:310@c09f04af`). A second test runs the two remaps in the opposite order. Under `kTrieCall` and `kFullLocate` the same sequence runs with the reader parked after it has left the lock.
...
- **G-O7 — lanes.** The standard, ASan/UBSan and TSan lanes of `test-rocksdb-memtable` (`.github/workflows/ci.yml:2226@c09f04af`) run G-O1–G-O6 under every scope each applies to. The engine side is held to §12.2's G12.1–G12.5 on the measured head.

**The skip-the-mutex twin, and what rules it out.** A twin that takes `mutex_` in the locate phase only when a writer could be inside `Insert`, and skips it otherwise, behaves exactly like `kOptimistic` when no writer exists. The idle cells spawn no writer (`integrations/rocksdb/benches/bench_memtable_concurrent.cc:242@c09f04af`, `:409@c09f04af`), so no idle cell can tell the two apart, and O1 alone cannot beat that twin. Two things in this section can:
...
2. under G-O3 (a), replace `Get`'s `next_leaf` step (`:559@c09f04af`) with a return: G-O3 (a) fails;
...
8. move `SplitLeafBlock`'s trie insert (`:307@c09f04af`–`:311@c09f04af`) before the block link (`:298@c09f04af`–`:305@c09f04af`), with the writer's fence (below) still immediately before the moved insert. The test aimed at it parks the writer between the moved insert and the link, while a reader runs `Get`, `Seek`, `SeekForPrev` and `Prev` from keys in both halves of the split block, in the TSan and standard lanes;
9. move the `tail_` store (`:302@c09f04af`) after the trie insert: G-O3 (b) is the test aimed at it;
10. let `Insert`'s remap (`:367@c09f04af`–`:369@c09f04af`) run before its entry stores (`:355@c09f04af`–`:364@c09f04af`): G-O3 (c) is the test aimed at it;
...
**What TSan can and cannot see here.** The lane links `libexpanse.a` from `cargo build --release -p expanse-capi` with no sanitizer (`.github/workflows/ci.yml:2246@c09f04af`–`:2247@c09f04af`, `:2472@c09f04af`–`:2478@c09f04af`).
...
- **Code-level fences instead, where the ordering is.**
  - **Writer:** a release fence, spelled as `SplitLeafBlock` and `Insert` already spell theirs (`:278@c09f04af`, `:353@c09f04af`), after the link stores (`:298@c09f04af`–`:305@c09f04af`) and immediately before the split's trie insert, and after the entry and count stores (`:361@c09f04af`–`:364@c09f04af`) and immediately before `Insert`'s remap.
  - **Reader:** an acquire fence, spelled as `Contains` and `Get` spell theirs (`:424@c09f04af`, `:542@c09f04af`), immediately after `expanse_sync_map_reader_prev_at_or_before` returns.
...
- **Cells.** The four `ExpanseMemTable` phases of `integrations/rocksdb/benches/bench_memtable.cc`, whose rep is built with the default scope (`:481@c09f04af`): `fillrandom` (`Insert`, `:488@c09f04af`), `readrandom` (`Get`, `:539@c09f04af`), `seekrandom` (`IteratorImpl::Seek`, `:601@c09f04af`), `prefixscan` (`IteratorImpl::Next`, `:646@c09f04af`, and `ScanBatch`, `:660@c09f04af`). Also `ApproximateMemoryUsage`, which every process calls once after its phase (`:710@c09f04af`).
...
- **Writers:** the free writer, and multiple writers. `InsertConcurrently` is still `Insert` under `mutex_` (`:377@c09f04af`–`:379@c09f04af`).
...
- **The sites.** On `10876676` there are six fences, in `integrations/rocksdb/src/expanse_memtable.cc`:
  - `SplitLeafBlock` (`:278@71f4c423`) and `Insert` (`:353@71f4c423`), release;
  - `Contains` (`:424@71f4c423`), `Get` (`:553@71f4c423`), `GetAfterDelivered` (`:663@71f4c423`) and `IteratorImpl::Seek` (`:998@71f4c423`), acquire.
