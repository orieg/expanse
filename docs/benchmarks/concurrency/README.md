...
The W = 0, R = 8 `masstree_conc_str` reader cell has two committed level families, one
per measurement procedure (the tables below). Which of them is current is not
resolved and no re-measurement is planned: #1061 closed with the difference
disclosed rather than attributed.
...
- **No comparison with Masstree or HOT.** The retraction leaves the
  readers-only FFI cell's level depending on the measurement procedure, unresolved (§13), and these runs
...
## 22. The #730 readers-only sweep re-measured at `7cd5140e` — a baseline, no verdict (Refs #730)

**A baseline, not an evaluation.** All four artifacts carry
`readers_only.preregistration` `null` (table 22.1), so under METHODOLOGY §16.5
these runs are a baseline and no cell here carries a verdict label of any kind.
METHODOLOGY §16's gate is neither evaluated nor amended by this section, and its
floors and R = 1 references are read as constants: they do not move (AGENTS.md
§8.19, METHODOLOGY §16.3). A re-measured baseline is a new pre-registration
rather than an edit to §16, so anything these levels could license would have to
go through a separately locked section; none is written. No mechanism is
attributed anywhere below (AGENTS.md §8.9) — what changed between the two heads
is not measured here.

**Why the sweep was re-taken.** The only committed artifacts for the cell
METHODOLOGY §16 gates are the four at `170a4bc3` that §15 publishes, and the
`SyncExpanseStrMap` read path has changed since. Four PRs touched
`crates/expanse/src/strmap.rs` between the two heads — #983, #985, #1001 and
#1041, +1,597 / −102 lines — and the reader itself is among them:
`ExpanseStrMap::get_validated` (`crates/expanse/src/strmap.rs:1692`) went from 58
to 80 lines, and after #985 gave `StrNode` its own OCC cover word, #1001 rewrote
the function around it — it now samples and validates each node's cover word
(`node_sample`, `walk_validated_node`) where it previously called the tree-wide
`walk_validated`. Where the cell sits on current `main` was therefore
unmeasured. Four dispatches of `writer_scaling_readers_only` answer that, and
nothing else.

**What was measured.** `bench_baremetal.yml` with
`benchmark_suite=writer_scaling_readers_only` at `7cd5140e`, two runs per pin,
8 rounds per cell, one harness process per timed cell. `SyncExpanseMap`,
`SyncExpanseSet` and `SyncExpanseStrMap` readers at W = 0 and R ∈ {1, 2, 4, 8};
each reader walks its own permutation of the 2^20-key prefill once, so every
probe hits. The per-reader cost is each reader's own loop time over its probes,
averaged over the readers of a round, with the interval from
`reader_scaling_bounds.per_arm_interval` over the 8 rounds (2,000 resamples,
seed 42) — the statistic and construction METHODOLOGY §16.2 names, so the
`170a4bc3` and `7cd5140e` columns are reduced identically. `attempts_per_op` is
1.0 and `fallback_rate` 0 on every `str` cell of all four artifacts.

*(measured: reference host — Intel Core i9-12900F, 8P+8E / 24 threads, kernel
6.8, governor `powersave` on every pinned CPU, transparent huge pages
`madvise`; engine, harness and driver at `7cd5140e`; pins `0-15` (P-cores with
SMT) and `0,2,4,6,8,10,12,14` (one thread per physical P-core, "per-core" in the
tables); the CI runs and artifacts are listed in table 22.1)* (workloads:
`concurrency_ordered_readers_map_64bit`, `concurrency_readers_set_63bit`,
`concurrency_readers_str`). Every table in this section is generated by
`scripts/tables.py` from the four artifacts of this head and the four at
`170a4bc3` (AGENTS.md §8.2).

**Host load (AGENTS.md §8.17).** Table 22.1 lists the snapshots. Every cell of
all four runs records `foreign_busy_cpus` 0.01 core-equivalents, two orders of
magnitude below §8.17's bar, with `own_busy_cpus` 1.19–1.20; run-start `load1`
is 1.90–2.62, and the two runs of each pin start within 0.72 and 0.25 of each
other. No void rule of METHODOLOGY §6 or §15 applies to any of the four, and no
run was discarded. Two limits, stated rather than argued away:

- `provenance.loads` holds two snapshots taken back to back at the start of the
  sweep with identical jiffy counters, exactly as the `170a4bc3` set does, so
  **no load average is recorded after the pass**. That is the one thing
  METHODOLOGY §16.7 asks an *evaluation* run to add, and these runs do not
  carry it. §16.7 states that omitting it does not void a run; it is recorded
  here so the gap is visible rather than inferred.
- The per-cell busy-CPU window is 76.7–76.9 s here against 57.4–57.5 s at
  `170a4bc3`. The cause is unmeasured.

### 22.1 Artifacts, runs and host load

Each row is one bare-metal dispatch. `readers_only.preregistration` is the field
METHODOLOGY §16.5 reads to tell a baseline from an evaluation.

| pin | run | artifact | CI run | `provenance.commit` | `readers_only.preregistration` | `readers_only.void` | load snapshots | `load1` at start, peak | busy-CPU window per cell | cell `foreign_busy_cpus`, min – max | governor on the pinned CPUs |
|---|--:|---|---|---|---|---|--:|---|--:|---|---|
| `0-15` | 1 | `results/baseline_readers_only_writer_scaling_7cd5140e_pin0-15.json` | [35548107965](https://github.com/orieg/expanse/actions/runs/35548107965) | `7cd5140e` | `null` | empty | 2 | 2.62, 2.62 | 76.9 – 76.9 s | 0.01 – 0.01 | `powersave` |
| `0-15` | 2 | `results/baseline_readers_only_writer_scaling_7cd5140e_pin0-15_run2.json` | [35548554236](https://github.com/orieg/expanse/actions/runs/35548554236) | `7cd5140e` | `null` | empty | 2 | 1.90, 1.90 | 76.7 – 76.7 s | 0.01 – 0.01 | `powersave` |
| per-core | 1 | `results/baseline_readers_only_writer_scaling_7cd5140e_percore.json` | [35548333424](https://github.com/orieg/expanse/actions/runs/35548333424) | `7cd5140e` | `null` | empty | 2 | 1.94, 1.94 | 76.9 – 76.9 s | 0.01 – 0.01 | `powersave` |
| per-core | 2 | `results/baseline_readers_only_writer_scaling_7cd5140e_percore_run2.json` | [35548780869](https://github.com/orieg/expanse/actions/runs/35548780869) | `7cd5140e` | `null` | empty | 2 | 2.19, 2.19 | 76.9 – 76.9 s | 0.01 – 0.01 | `powersave` |

### 22.2 The `str` per-reader cost at `7cd5140e`, beside `170a4bc3`

Both columns are reduced by `reader_scaling_bounds.per_arm_interval` over the
8 rounds of the cell. The difference column is a difference of two unpaired point
estimates from separate dispatches and carries no interval; table 22.3 is where a
direction is stated.

| pin | run | R | `170a4bc3` ns per probe per reader [BCa 95%] | `7cd5140e` ns per probe per reader [BCa 95%] | unpaired difference in the point estimates (rule 18: not a paired claim) | intervals overlap across the heads | `7cd5140e` rounds beyond 3 MADs (`round_outliers`) | `7cd5140e` ns per probe to last join |
|---|--:|--:|---|---|--:|---|---|--:|
| `0-15` | 1 | 1 | 217.56 [217.23, 217.81] | 213.90 [212.82, 217.04] | -3.660 ns, -1.68 % | **no** | 2, 3 | 213.994 |
| `0-15` | 1 | 2 | 226.51 [225.90, 227.39] | 222.62 [222.02, 223.97] | -3.897 ns, -1.72 % | **no** | 3, 7 | 222.880 |
| `0-15` | 1 | 4 | 242.24 [241.31, 242.80] | 237.26 [236.33, 238.67] | -4.977 ns, -2.05 % | **no** | 3, 4 | 238.730 |
| `0-15` | 1 | 8 | 272.84 [272.54, 273.35] | 270.64 [270.22, 271.04] | -2.204 ns, -0.81 % | **no** | none | 273.363 |
| `0-15` | 2 | 1 | 218.12 [217.47, 219.89] | 212.94 [212.74, 213.41] | -5.180 ns, -2.37 % | **no** | 2 | 213.054 |
| `0-15` | 2 | 2 | 225.85 [225.67, 226.01] | 221.71 [221.28, 222.15] | -4.135 ns, -1.83 % | **no** | none | 222.086 |
| `0-15` | 2 | 4 | 241.42 [240.64, 242.20] | 236.02 [235.24, 236.54] | -5.402 ns, -2.24 % | **no** | none | 237.591 |
| `0-15` | 2 | 8 | 273.35 [272.89, 274.21] | 269.94 [269.74, 270.12] | -3.413 ns, -1.25 % | **no** | none | 271.981 |
| per-core | 1 | 1 | 217.21 [216.90, 217.50] | 212.81 [212.59, 213.01] | -4.398 ns, -2.02 % | **no** | none | 212.918 |
| per-core | 1 | 2 | 225.98 [225.54, 226.41] | 222.11 [221.76, 222.47] | -3.864 ns, -1.71 % | **no** | none | 222.458 |
| per-core | 1 | 4 | 241.37 [240.38, 242.08] | 236.60 [236.25, 237.08] | -4.767 ns, -1.98 % | **no** | none | 238.061 |
| per-core | 1 | 8 | 275.37 [273.87, 277.26] | 271.64 [270.72, 272.74] | -3.731 ns, -1.35 % | **no** | none | 280.356 |
| per-core | 2 | 1 | 217.44 [217.25, 217.69] | 213.02 [212.67, 213.86] | -4.421 ns, -2.03 % | **no** | 1 | 213.129 |
| per-core | 2 | 2 | 225.86 [225.39, 226.66] | 221.89 [221.46, 222.32] | -3.971 ns, -1.76 % | **no** | none | 222.177 |
| per-core | 2 | 4 | 240.78 [239.94, 242.26] | 236.91 [236.36, 237.94] | -3.866 ns, -1.61 % | **no** | 2 | 238.244 |
| per-core | 2 | 8 | 274.80 [273.54, 276.90] | 272.85 [271.69, 274.31] | -1.950 ns, -0.71 % | yes | none | 286.733 |

### 22.3 Which `str` cells moved in both runs (rule 18)

A direction is stated only where both runs of the pin lie on the same side of
**both** `170a4bc3` runs with disjoint intervals. The two spread columns are the
gap between the two runs of a pin at each head, the quantity a within-run interval
does not bound.

| pin | R | `7cd5140e` against `170a4bc3` | spread between the two `170a4bc3` runs | spread between the two `7cd5140e` runs |
|---|--:|---|--:|--:|
| `0-15` | 1 | lower than both `170a4bc3` runs, in both runs | 0.26 % | 0.45 % |
| `0-15` | 2 | lower than both `170a4bc3` runs, in both runs | 0.29 % | 0.41 % |
| `0-15` | 4 | lower than both `170a4bc3` runs, in both runs | 0.34 % | 0.53 % |
| `0-15` | 8 | lower than both `170a4bc3` runs, in both runs | 0.19 % | 0.26 % |
| per-core | 1 | lower than both `170a4bc3` runs, in both runs | 0.10 % | 0.10 % |
| per-core | 2 | lower than both `170a4bc3` runs, in both runs | 0.05 % | 0.10 % |
| per-core | 4 | lower than both `170a4bc3` runs, in both runs | 0.25 % | 0.13 % |
| per-core | 8 | not the same in both runs | 0.21 % | 0.45 % |

### 22.4 Where the cells sit against METHODOLOGY §16.3's registered constants

The floor and the R = 1 reference are METHODOLOGY §16.3's registered constants,
read and not recomputed (AGENTS.md §8.19). This table locates the cells against
them; it evaluates nothing.

| pin | run | R = 8 ns per probe per reader [BCa 95%] | §16.3 R = 8 floor | point estimate over the floor | interval upper bound over the floor | R = 1 ns per probe per reader [BCa 95%] | §16.3 R = 1 reference | interval lower bound against the reference |
|---|--:|---|--:|--:|--:|---|--:|--:|
| `0-15` | 1 | 270.64 [270.22, 271.04] | 259.198 ns | +11.437 ns, +4.41 % | +11.844 ns, +4.57 % | 213.90 [212.82, 217.04] | 218.1226 ns | -5.300 ns |
| `0-15` | 2 | 269.94 [269.74, 270.12] | 259.198 ns | +10.740 ns, +4.14 % | +10.925 ns, +4.21 % | 212.94 [212.74, 213.41] | 218.1226 ns | -5.378 ns |
| per-core | 1 | 271.64 [270.72, 272.74] | 261.062 ns | +10.578 ns, +4.05 % | +11.678 ns, +4.47 % | 212.81 [212.59, 213.01] | 217.4392 ns | -4.852 ns |
| per-core | 2 | 272.85 [271.69, 274.31] | 261.062 ns | +11.790 ns, +4.52 % | +13.252 ns, +5.08 % | 213.02 [212.67, 213.86] | 217.4392 ns | -4.770 ns |

### 22.5 How the `str` per-reader cost rises with R at `7cd5140e`

The ladder at this head only, so no cross-head comparison enters it.

| pin | run | R = 1 ns per probe per reader [BCa 95%] | R = 8 ns per probe per reader [BCa 95%] | R = 8 over R = 1 | R = 1 as a share of R = 8 | ns added per additional reader, R 1 → 2 | R 2 → 4 | R 4 → 8 |
|---|--:|---|---|--:|--:|--:|--:|--:|
| `0-15` | 1 | 213.90 [212.82, 217.04] | 270.64 [270.22, 271.04] | 1.2652 | 79.04 % | 8.71 ns | 7.32 ns | 8.34 ns |
| `0-15` | 2 | 212.94 [212.74, 213.41] | 269.94 [269.74, 270.12] | 1.2677 | 78.89 % | 8.77 ns | 7.15 ns | 8.48 ns |
| per-core | 1 | 212.81 [212.59, 213.01] | 271.64 [270.72, 272.74] | 1.2764 | 78.34 % | 9.30 ns | 7.25 ns | 8.76 ns |
| per-core | 2 | 213.02 [212.67, 213.86] | 272.85 [271.69, 274.31] | 1.2809 | 78.07 % | 8.87 ns | 7.51 ns | 8.98 ns |

### 22.6 The `map` and `set` arms at `7cd5140e`, beside `170a4bc3`

The two control arms METHODOLOGY §16.4 publishes without a threshold.

| arm | pin | run | R | `170a4bc3` ns per probe per reader [BCa 95%] | `7cd5140e` ns per probe per reader [BCa 95%] | intervals overlap across the heads | `7cd5140e` against `170a4bc3`, both runs (rule 18) |
|---|---|--:|--:|---|---|---|---|
| `map` | `0-15` | 1 | 1 | 49.29 [49.04, 49.49] | 49.28 [48.89, 49.59] | yes | not the same in both runs |
| `map` | `0-15` | 1 | 8 | 51.87 [51.67, 52.37] | 52.39 [51.96, 53.40] | yes | not the same in both runs |
| `map` | `0-15` | 2 | 1 | 48.87 [48.72, 49.07] | 49.06 [48.87, 49.24] | yes | not the same in both runs |
| `map` | `0-15` | 2 | 8 | 51.92 [51.79, 52.16] | 52.19 [51.76, 52.96] | yes | not the same in both runs |
| `map` | per-core | 1 | 1 | 48.98 [48.73, 49.24] | 48.92 [48.71, 49.47] | yes | not the same in both runs |
| `map` | per-core | 1 | 8 | 53.53 [51.98, 56.27] | 52.68 [51.78, 54.53] | yes | not the same in both runs |
| `map` | per-core | 2 | 1 | 48.91 [48.71, 49.07] | 49.21 [48.91, 49.77] | yes | not the same in both runs |
| `map` | per-core | 2 | 8 | 52.09 [51.70, 53.03] | 54.73 [52.69, 57.98] | yes | not the same in both runs |
| `set` | `0-15` | 1 | 1 | 63.62 [63.37, 63.78] | 63.65 [63.36, 64.27] | yes | not the same in both runs |
| `set` | `0-15` | 1 | 8 | 67.79 [67.13, 69.10] | 67.10 [66.62, 67.89] | yes | not the same in both runs |
| `set` | `0-15` | 2 | 1 | 63.53 [63.13, 63.83] | 63.70 [63.26, 63.93] | yes | not the same in both runs |
| `set` | `0-15` | 2 | 8 | 67.26 [66.71, 68.41] | 67.16 [66.71, 67.58] | yes | not the same in both runs |
| `set` | per-core | 1 | 1 | 63.45 [63.27, 63.62] | 63.23 [62.91, 63.48] | yes | not the same in both runs |
| `set` | per-core | 1 | 8 | 67.48 [66.68, 69.62] | 67.80 [66.68, 70.43] | yes | not the same in both runs |
| `set` | per-core | 2 | 1 | 63.72 [63.40, 64.16] | 63.39 [63.11, 63.62] | yes | not the same in both runs |
| `set` | per-core | 2 | 8 | 67.42 [66.58, 68.19] | 67.87 [66.86, 70.72] | yes | not the same in both runs |

### 22.7 What these levels show, and what they do not

**The registered floor is not cleared, and no cell is evaluated against it.**
METHODOLOGY §16.2 reads the **upper** bound of the R = 8 interval, because the
statistic is a cost. At this head that bound sits +10.925 ns to +13.252 ns above
the floor registered for its pin across the four runs — +4.21 % to +5.08 % of
the floor — and the point estimates sit +4.05 % to +4.52 % above it (table 22.4,
`results/baseline_readers_only_writer_scaling_7cd5140e_*.json`).
The R = 1 side condition's reference is the other constant §16.3 registers, and
this head's R = 1 interval lower bounds sit 4.770–5.378 ns below it in all four
runs. None of that is a verdict: `readers_only.preregistration` is `null`, so
under §16.5 these runs are a baseline (workload: `concurrency_readers_str`).

**R = 1 moved more than R = 8, and one cell does not agree across runs.** Under
`docs/BENCHMARKING.md` rule 18 a direction is stated only where both runs of a
pin sit on the same side of **both** runs at the other head with disjoint
intervals. Seven of the eight (pin, R) `str` cells qualify and are stated as
lower: R ∈ {1, 2, 4} at both pins, and R = 8 at pin `0-15`. The eighth — R = 8
per-core — does not: run 1's interval is disjoint below both `170a4bc3` runs and
run 2's overlaps them, so table 22.3 withholds the direction rather than
splitting the difference. The magnitudes in table 22.2 are −1.68 % to −2.37 % at
R = 1 and −0.81 % and −1.25 % at R = 8, pin `0-15`, against a between-run spread
of 0.10 %–0.53 % at either head (table 22.3) and METHODOLOGY §16.4's registered
minimum detectable differences of 0.31 % and 0.49 % for the two `0-15` R = 8
cells (workload: `concurrency_readers_str`). So the R = 8 movement at `0-15` is
above what the instrument resolves there, and it is a fraction of the 5 %
margin §16.3 registers: the cell is lower than it was and the floor is still
11–13 ns away.

**And it is not a paired before/after claim.** Rule 18's stronger form is that a
before/after claim on a concurrent cell takes the interleaved two-commit shape,
base and head alternating round by round inside one cell. These are eight
separate single-build dispatches at two heads: the CI runs named in table 22.1
were dispatched on 2026-09-21 and those in §15.1 on 2026-09-15. So what tables
22.2, 22.3 and 22.6 state is agreement across runs, not a paired delta. The
point-estimate differences are labelled unpaired in the header they sit under
for that reason, and no interval is published for any of them.

**The `map` and `set` arms state no direction, and that is weak evidence.**
Every `map` and `set` cell in table 22.6 overlaps across the heads and none
qualifies under rule 18. That is consistent with no change and also with the
instrument: METHODOLOGY §16.4 records their R = 8 relative minimum detectable
differences reaching 7.49 % (`map`, per-core, run 1) and 4.08 % (`set`, same
pin), against 0.31 %–1.36 % for the four `str` R = 8 cells, which is why §16
makes them controls with no threshold attached. An absent direction there does
not establish an absent effect.

**Most of the R = 8 per-reader cost is already present at one reader.** At pin
`0-15` run 1 the R = 1 cost is 213.901 ns and the R = 8 cost 270.635 ns, so
213.901 ÷ 270.635 = 79.04 %; across the four runs the share is 78.07 %–79.04 %
and the R = 8 ÷ R = 1 ratio 1.2652–1.2809 (table 22.5,
`results/baseline_readers_only_writer_scaling_7cd5140e_*.json`). The rise is
smooth in the reader count rather than stepped: per additional reader it is
8.71, 7.32 and 8.34 ns over the three doublings at that pin and run, and
7.15–9.30 ns over all twelve steps of the four runs, with no doubling standing
out (workload: `concurrency_readers_str`).

**The ratio in #730's title is a different cell.** #730 is titled on a 2.2×
readers-to-single-threaded ratio for the FFI `masstree_conc_str` cell; the ratio
this sweep measures is 1.2652–1.2809 *(workloads differ:
`concurrency_readers_str` vs `masstree_conc_str`)*. METHODOLOGY §16.1 records
why the two are not interchangeable — a different harness and a probe stream in
which every probe hits — and the level behind the issue's figure is formally
retracted as unsourced (`.github/superseded-figures.json`,
`masstree_readers_only_string_82966aae`). No ratio against a competitor is
formed here.

**What an evaluation would still need**, whenever a candidate change on the
wrapper's read path exists: four dispatches at that head whose artifacts record
`readers_only.preregistration` as `METHODOLOGY.md §16` rather than `null`
