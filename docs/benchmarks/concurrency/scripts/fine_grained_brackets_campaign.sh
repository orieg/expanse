#!/usr/bin/env bash
# The fine-grained write brackets measurement campaign, reference host only: the H3.2 counter
...
# artifacts against ../METHODOLOGY.md §8 with scripts/fine_grained_brackets_gate.py; nothing
...
printf 'suite=concurrency-fine-grained-brackets pid=%s start=%s\n' "$$" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "${BENCH_LOCK}/owner"
...
stamp "2/5 H3.2 per-thread counters at the head build, into results/fine_grained_brackets/"
python3 scripts/bench_counters.py --repeats 7 --no-c2c --out-dir "${MT}/results/fine_grained_brackets" \
...
python3 scripts/bench_counters.py --repeats 7 --no-c2c --out-dir "${HOT}/results/fine_grained_brackets" \
...
python3 docs/benchmarks/concurrency/scripts/fine_grained_brackets_gate.py
stamp "done"
