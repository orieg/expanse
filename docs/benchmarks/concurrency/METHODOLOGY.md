...
- **Both pins are part of the gate:** `0-15` and `0,2,4,6,8,10,12,14`. A cell is never pooled across pins.
  - `0-15` is the pin of #930's table and of every writer-sweep artifact committed before this section except #912's pair. The sizing sweeps (§14.3) ran both pins. At W = 8 it lets two writers share one physical core's SMT siblings, and the effect of that on the W = 8 cells is unmeasured (`README.md` §10).
...
- any cell's `load.foreign_busy_cpus` is above 1.0 core-equivalents, any snapshot in `provenance.loads` has a load average above 12 (half the host's 24 logical CPUs), or the two runs at one pin start at load averages more than 2 apart (§6, AGENTS.md §8.17). The foreign-CPU rule reads `load.foreign_busy_cpus` on each `throughput` result. That window opens when the arm's writer pass starts (`since` `arm:<arm>:writers`) and closes when the pass ends, so it covers the arm's whole pass and every W cell of the arm carries the same value; it does not isolate one timed cell. The phase snapshots in `provenance.loads` (`foreign_busy_cpus_since_prev`) are not a void criterion: they can read spuriously in either direction, and the sizing runs recorded -0.76 and -0.50 there;
