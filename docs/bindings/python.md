...
1. **Optimistic Reads**: Query operations (`contains`, `get`, `get_many`, `len`, `is_empty`, and on `SyncExpanseMap` the ordered reads `first`, `last`, `next` and `prev`) validate version seqlocks and read concurrently without holding a mutex on the common path, and without the Python GIL. The protocol is blocking optimistic lock coupling, not lock-free: a read that exhausts its bounded retries falls back to the writer-excluding path, and writers wait for it. Rank, range and iteration (`count_below`, `by_count`, `count_range`, `range`, `keys`/`values`/`items` and `__iter__`) and the ordered reads of `SyncExpanseSet` run with writers excluded. The map's ordered reads moved to the optimistic path in #900; under quiescence they return what the writer-excluding reads returned.
