...
| `patricia_lookup_hit` | wall-clock | Patricia trie (patricia_tree) vs Expanse point lookup at 100% hit across five u64 key distributions. |
| `patricia_lookup_miss` | wall-clock | Patricia trie (patricia_tree) vs Expanse point lookup at 50% hit / 50% same-distribution miss. |
| `patricia_insert` | wall-clock | Patricia trie (patricia_tree) vs Expanse cold-build insertion, generator and shuffled orders. |
| `patricia_memory` | wall-clock | Patricia trie (patricia_tree) vs Expanse live-heap census, u64 and shared-prefix string keys. |
| `patricia_string` | wall-clock | Patricia trie (patricia_tree) vs ExpanseStrMap point lookup on shared-prefix string keys, 50% hit. |
...
| [`crates/expanse/benches/patricia_insert.rs`](../crates/expanse/benches/patricia_insert.rs) | 10k to 1M | both — generator draw order (ascending on `sequential` and `sparse_stride`) and a Fisher–Yates permutation under `PROBE_SHUFFLE_SEED`; `order` recorded per row | Every unique key inserted once per round into a fresh structure | N/A (all inserts are new keys) | None | `black_box` of each insert's previous-value result | Insert loop only; construction of the empty map precedes the window and drop follows it | Identical keys, values and order; arm order alternates per round | Median per arm + BCa 95% CI on the paired per-round ratio | **PENDING** `[unmeasured]`: no committed run yet. |
| [`crates/expanse/benches/patricia_lookup_hit.rs`](../crates/expanse/benches/patricia_lookup_hit.rs) | 10k to 1M | generator draw order — ascending on `sequential` and `sparse_stride`; a Patricia tree's node set is fixed by its key set, so build order does not change the structure probed | Every present key once per round, shuffled under `PROBE_SHUFFLE_SEED`; same stream every round | 100% | None | `black_box` of the returned value | Probe loop only; build and encoding outside the window | Identical keys, values and probe order; arm order alternates per round | Median per arm + BCa 95% CI on the paired per-round ratio | **PENDING** `[unmeasured]`: no committed run yet. |
| [`crates/expanse/benches/patricia_lookup_miss.rs`](../crates/expanse/benches/patricia_lookup_miss.rs) | 10k to 1M | generator draw order — ascending on `sequential` and `sparse_stride`; a Patricia tree's node set is fixed by its key set, so build order does not change the structure probed | N/2 present + N/2 absent keys, shuffled under `PROBE_SHUFFLE_SEED`; same stream every round | 50% hit / 50% miss | Same-distribution rejection sampling (`gen_distribution_misses`) | `black_box` of the returned value or 0 | Probe loop only; build and encoding outside the window | Identical keys, values and probe order; arm order alternates per round | Median per arm + BCa 95% CI on the paired per-round ratio | **PENDING** `[unmeasured]`: no committed run yet. |
| [`crates/expanse/benches/patricia_memory.rs`](../crates/expanse/benches/patricia_memory.rs) | 1k to 1M | both — generator draw order (ascending on `sequential` and `sparse_stride`) and a Fisher–Yates permutation under `PROBE_SHUFFLE_SEED`; both recorded per row | N/A (memory) | N/A | None | None; live bytes read from the TrackingAlloc counter | Build loop only; each structure dropped before the next is built | Identical key sets and values, one allocator hook for all arms | Exact deterministic byte count | **PENDING** `[unmeasured]`: no committed run yet. |
| [`crates/expanse/benches/patricia_string.rs`](../crates/expanse/benches/patricia_string.rs) | 10k to 1M | generator draw order (random ids); a Patricia tree's node set is fixed by its key set, so build order does not change the structure probed | N/2 present + N/2 absent keys, shuffled under `PROBE_SHUFFLE_SEED`; same stream every round | 50% hit / 50% miss | Same generator, independent seed, rejected on membership (`gen_prefixed_path_misses`) | `black_box` of the returned value or 0 | Probe loop only; build, key validation and probe assembly outside the window | Identical keys, values and probe order; arm order alternates per round | Median per arm + BCa 95% CI on the paired per-round ratio | **PENDING** `[unmeasured]`: no committed run yet. |
