...
| LLM inference & speculative decoding | [`llm_inference/`](benchmarks/llm_inference/README.md) | `bench_draft_quality`, `bench_llm_datastore`, `bench_grammar_masks`, `bench_prefix_lru` | HuggingFace PromptLookup (adaptive/fixed), Native Suffix Array (`cdivsufsort`), Dense Bitmask (`[u64]`), `roaring::Bitmap`, `collections.OrderedDict` | Draft quality: Expanse variable-length LSM wins alpha vs HF Adaptive (+7.8% code, +7.1% summary, +11.3% JSON; 1.07x–1.11x tok/s ceiling); Dynamic datastore: Expanse wins continuous ingestion whenever update batch B < 177k; Grammar masks: Expanse/Roaring win 1.4x–3.0x lower RAM; Prefix LRU: 9.5x lower RAM + 1.48M/s rank-eviction |
