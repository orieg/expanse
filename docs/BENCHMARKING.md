...
| arm | 1 thread, run 1 | 16 threads, run 1 [BCa 95%] | run 2 | C(16), run 1 [paired BCa 95%] | run 2 |
|---|---:|---|---|---|---|
| `SyncExpanseSet` | 76.4 | 562 [554, 570] | 415 [333, 493] | 7.36 [7.25, 7.46] | 5.44 [4.37, 6.47] |
| `SyncExpanseMap` | 38.2 | 361 [360, 361] | 361 [360, 361] | 9.44 [9.43, 9.46] | 9.44 [9.43, 9.46] |
| `SyncExpanseBlobMap` | 31.0 | 306 [305, 307] | 307 [306, 307] | 9.86 [9.84, 9.88] | 9.87 [9.84, 9.89] |
| `SyncExpanseBytesMap` | 11.4 | 126 [126, 127] | 126 [125, 126] | 11.08 [11.05, 11.11] | 11.03 [10.99, 11.06] |
| `SyncExpanseStrMap` | 6.89 | 73.3 [71.8, 74.6] | 75.6 [74.1, 76.0] | 10.63 [10.42, 10.82] | 10.99 [10.79, 11.06] |
| `DashMap<Vec<u8>, u64>` | 15.4 | 130 [130, 130] | 129 [129, 130] | 8.46 [8.44, 8.48] | 8.39 [8.36, 8.45] |
| `SkipMap<u64, Vec<u8>>` | 3.46 | 38.3 [38.2, 38.4] | 38.4 [38.3, 38.5] | 11.05 [11.01, 11.08] | 11.10 [11.06, 11.14] |
| `RwLock<BTreeMap<u64, (Vec<u8>, u32)>>` | 10.5 | 15.3 [15.2, 15.3] | 16.0 [15.9, 16.0] | 1.45 [1.45, 1.46] | 1.52 [1.52, 1.52] |
| `Mutex<ExpanseBlobMap>` | 41.2 | 5.41 [5.36, 5.45] | 5.27 [5.21, 5.33] | 0.13 [0.13, 0.13] | 0.13 [0.13, 0.13] |
| `Mutex<ExpanseBytesMap>` | 12.1 | 3.39 [3.37, 3.41] | 3.49 [3.47, 3.51] | 0.28 [0.28, 0.28] | 0.29 [0.29, 0.29] |
| `Mutex<ExpanseStrMap>` | 8.88 | 2.68 [2.67, 2.69] | 2.67 [2.66, 2.68] | 0.30 [0.30, 0.30] | 0.30 [0.30, 0.30] |

**50% read / 50% write** (read + write M ops/s):

| arm | 1 thread, run 1 | 16 threads, run 1 [BCa 95%] | run 2 | C(16), run 1 [paired BCa 95%] | run 2 |
|---|---:|---|---|---|---|
| `SyncExpanseSet` | 42.9 | 82.9 [76.5, 88.9] | 84.8 [77.4, 91.1] | 1.93 [1.78, 2.07] | 1.98 [1.81, 2.12] |
| `SyncExpanseMap` | 28.0 | 62.5 [53.7, 67.9] | 62.2 [54.6, 66.9] | 2.23 [1.92, 2.43] | 2.23 [1.96, 2.40] |
| `SyncExpanseBlobMap` | 27.2 | 7.31 [6.68, 7.63] | 7.21 [6.60, 7.52] | 0.27 [0.26, 0.28] | 0.27 [0.25, 0.28] |
| `SyncExpanseBytesMap` | 4.81 | 3.12 [3.10, 3.14] | 3.11 [3.10, 3.14] | 0.65 [0.63, 0.66] | 0.65 [0.63, 0.66] |
| `SyncExpanseStrMap` | 6.21 | 4.06 [4.05, 4.07] | 4.03 [4.02, 4.03] | 0.65 [0.65, 0.66] | 0.65 [0.65, 0.65] |
| `DashMap<Vec<u8>, u64>` | 10.7 | 83.6 [82.6, 85.0] | 83.6 [82.5, 85.1] | 7.80 [7.65, 7.95] | 7.77 [7.60, 7.90] |
| `SkipMap<u64, Vec<u8>>` | 2.05 | 17.5 [17.2, 17.6] | 17.5 [17.3, 17.7] | 8.54 [8.31, 8.69] | 8.54 [8.34, 8.69] |
| `RwLock<BTreeMap<u64, (Vec<u8>, u32)>>` | 8.50 | 3.64 [3.63, 3.64] | 3.55 [3.54, 3.55] | 0.43 [0.43, 0.43] | 0.42 [0.42, 0.42] |
| `Mutex<ExpanseBlobMap>` | 35.2 | 5.08 [4.73, 5.27] | 4.89 [4.53, 5.07] | 0.15 [0.14, 0.15] | 0.14 [0.14, 0.15] |
| `Mutex<ExpanseBytesMap>` | 6.25 | 2.52 [2.51, 2.52] | 2.86 [2.86, 2.87] | 0.40 [0.39, 0.41] | 0.46 [0.45, 0.46] |
| `Mutex<ExpanseStrMap>` | 7.50 | 2.31 [2.31, 2.31] | 2.36 [2.35, 2.36] | 0.31 [0.31, 0.31] | 0.31 [0.31, 0.31] |

**What both runs show.** At 100% read every `Sync*` arm scales to sixteen
threads and every `Mutex<Expanse*>` arm falls below its own one-thread
throughput: `SyncExpanseMap` reaches C(16) = 9.44 [9.43, 9.46] in both runs,
`SyncExpanseBlobMap` 9.86–9.87, `SyncExpanseStrMap` 10.63–10.99 and
`SyncExpanseBytesMap` 11.03–11.08, against 8.39–8.46 for `DashMap` and
0.13–0.30 for the mutex arms (workload: `core_concurrency`). At 50/50 the two
arms that admit concurrent writers scale and the three that serialise writers
lose throughput as threads are added: `SyncExpanseMap` C(16) = 2.23 [1.92, 2.43] in
run 1 and `SyncExpanseSet` 1.93 [1.78, 2.07], with run 2's estimates inside both
intervals, against 0.27–0.65 for `SyncExpanseBlobMap`, `SyncExpanseStrMap` and
`SyncExpanseBytesMap` in both runs. `SyncExpanseSet` and `SyncExpanseMap` take
writers through optimistic lock coupling on per-node version words
(`docs/ARCHITECTURE.md` §4.2); the blob, string and bytes wrappers serialise
writers on one mutex. The concurrent-write baselines scale further than either
multi-writer arm — `DashMap` 7.77–7.80 and `SkipMap` 8.54 at 50/50 —
and what limits the Expanse arms there is unmeasured.
...
- **`bytes/key` on random keys is density-dependent, and not monotonically.** The
  same `ExpanseSet` measures 6.71–21.02 B/key under expanse occupancy alone,
...
- **Allocator-held bytes depend on insertion order; `mem_used()` does not.** The
  ...
  `mem_used()` either way (workload: `hot_instrument_bridge`). A census-instrument
...
Under a 50/50 mix `SyncExpanseMap` goes 28.0 → 40.9 → 62.5 M read + write ops/s across
1→4→16 threads in run 1, C(16) = 2.23 [1.92, 2.43], and 2.23 [1.96, 2.40] in run 2
(`results/baseline_concurrent_mixed.json`; workload: `core_concurrency`).
`SyncExpanseMap` admits concurrent writers through optimistic lock coupling
(`docs/ARCHITECTURE.md` §4.2). The single-writer blob, string and bytes arms
lose throughput as threads are added (0.27–0.65 in both runs), and `DashMap`
(7.77–7.80) and `SkipMap` (8.54) scale further than either
multi-writer arm; what limits the Expanse arms is unmeasured.
