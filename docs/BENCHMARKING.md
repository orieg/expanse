...
| [`crates/expanse/benches/ycsb.rs`](../crates/expanse/benches/ycsb.rs) | 100k, 1M, 10M recorded in every bench id and result row | both — every cell is built once sorted ascending and once Fisher–Yates shuffled from the suite PRNG, the order recorded in every bench id and result row; the operation stream is generated from the canonical draw order and is identical across the two | 200,000 ops per cell in the rounds/latency report; one deterministic stream per workload, reused across engines, orders and rounds | 100% on reads — every read targets a present key, Zipfian θ = 0.99 over the canonical draw order (workload D: Zipfian over recency, in-run inserts first) | n/a — no miss probes; every read key is drawn from the population or from the stream's own earlier inserts | blob arms read byte 0 of the 128 B payload on every read and scanned record; the `ExpanseMap` arm holds `u64` values and has no payload to dereference | Op loop only. Setup and teardown are outside the timed region. Latency percentiles are window means (64 ops per `Instant` pair), never a per-op bracket | One op stream for every arm; key-parity scan predicate (selectivity identical by construction); a per-cell work checksum (`consumed`) asserted equal across arms | Published cells come from 8 per-round samples with BCa 95% intervals and paired per-round ratios (`scripts/ycsb_bench.py`) | **PASS** `[verified: RUN (21a382f3, the reference host)]`: 8 paired rounds on the reference host across 100k, 1M, 10M populations and both insertion orders (shuffled and sorted), with window-mean latencies and paired BCa 95% ratios; runs 35419627251 and 35419631571. |
| [`crates/expanse/benches/ycsb_dense.rs`](../crates/expanse/benches/ycsb_dense.rs) | 100k, 1M, 10M recorded in every bench id and result row | both — every cell is built once sorted ascending and once Fisher–Yates shuffled from the suite PRNG, the order recorded in every bench id and result row; the operation stream is generated from the canonical draw order and is identical across the two | 200,000 ops per cell in the rounds/latency report; one deterministic stream per workload, reused across engines, orders and rounds | 100% on reads — every read targets a present key, Zipfian θ = 0.99 over the canonical draw order (workload D: Zipfian over recency, in-run inserts first) | n/a — no miss probes; every read key is drawn from the population or from the stream's own earlier inserts | blob arms read byte 0 of the 128 B payload on every read and scanned record; the `ExpanseMap` arm holds `u64` values and has no payload to dereference | Op loop only. Setup and teardown are outside the timed region. Latency percentiles are window means (64 ops per `Instant` pair), never a per-op bracket | One op stream for every arm; key-parity scan predicate (every other key of a dense run, identical across arms); a per-cell work checksum (`consumed`) asserted equal across arms | Published cells come from 8 per-round samples with BCa 95% intervals and paired per-round ratios (`scripts/ycsb_bench.py`) | **PASS** `[verified: RUN (21a382f3, the reference host)]`: dense clustered key shape confirms range scan inversion on dense keys: at 1M sorted ExpanseBlobMap leads BTreeMap 1.027x [1.026, 1.029] and at 10M sorted 1.157x [1.155, 1.158]; runs 35419627251 and 35419631571. |
