...
| `remove_retention` | a deterministic `mem_used()` census from an example, not a `[[bench]]` target: it needs no bare-metal host and reproduces to the byte on any 64-bit machine. Run `cargo run --release -p expanse-trie --example remove_retention -- --json <path>` (`docs/benchmarks/remove_retention/README.md`). |
