...
| [`crates/expanse/examples/writer_scaling.rs`](../crates/expanse/examples/writer_scaling.rs) | prefill 2^20 keys (1M), plus 2^20 fresh keys inserted concurrently by W writers | sorted — prefill ascending, matching expanse-hot-bench; fresh stream in generator draw order | none — pure writer scaling (R = 0), insert-only | n/a — no read probes | same-generator rejection sampling against prefill | map arms check stored values against key-derived expectation | barrier release to last-writer join; prefill and teardown outside | symmetric across thread counts; W in {1, 2, 4, 8} on physical P-cores | throughput ops/sec emitted raw, paired bootstrap BCa 95% CI for C(N); lock fallbacks and their six causes (partition-checked per row) from the occ-stats counters pass | pending measurement |
