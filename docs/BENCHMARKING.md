...
| STM32H747I-DISCO on-target (hardware) | [`stm32h747/`](benchmarks/stm32h747/README.md) | `integrations/stm32h747` firmware, both cores | sorted array + `bsearch`/`memmove`, open-addressing hash, newlib `tsearch`; `cpsid`/`cpsie` critical section (ISR); HSEM lock (cross-core) | see suite README: cache-line geometry confirmed (1.7–1.9× cache-on/off at 2:1 core:bus); ISR entry latency bounded 29–113× (M7) / 53–58× (M4) tighter than masking; loses point lookup and unordered ingest to the hash table (4.9× / 11×) and sorted array, wins steady-state expiry (3.7×) and bytes/key on dense keys; cross-core correct only with an uncached heap, unsupported cacheable case fails safe |
