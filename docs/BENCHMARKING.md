...
| `writer_scaling_ablation_blob_shared_arena` | wall-clock | Expanse multi-writer scaling diagnostic: default build vs ablation-blob-shared-arena on the blob arm (Refs #929); outputs ablation_blob_shared_arena_writer_scaling.json. |
...
| `writer_scaling_929_str_gate` | wall-clock | The #929 SyncExpanseStrMap multi-writer gate (concurrency METHODOLOGY.md §17.3): the str arm, default build vs ablation-str-serial-writers, interleaved (build x W) rounds at the registered 8 rounds and W in {1, 2, 4, 8}, one process per cell, under the applied pin (dispatch once per pin per run); outputs gate_929_str_writer_scaling.json with this run's §17.9 verdicts. |
| `writer_scaling_929_str_gate_v2` | wall-clock | The second #929 SyncExpanseStrMap multi-writer gate (concurrency METHODOLOGY.md §19.4): the same str cells as writer_scaling_929_str_gate (default build vs ablation-str-serial-writers, 8 interleaved rounds, W in {1, 2, 4, 8}, one process per cell, under the applied pin), read as G1 scaling, G2 level against the serialised build's best cell at any writer count, and G3 the single-writer price floor; outputs gate_929_str_v2_writer_scaling.json. |
| `writer_scaling_929_blob_gate` | wall-clock | The #929 SyncExpanseBlobMap multi-writer gate (concurrency METHODOLOGY.md §21.4): blob cells (default build vs ablation-blob-serial-writers, 8 interleaved rounds, W in {1, 2, 4, 8}, one process per cell, under the applied pin), read as G1 scaling, G2 level against the serialised build's best cell at any writer count, G3 the single-writer price floor, and zero-restart tripwire; outputs gate_929_blob_writer_scaling.json. |
| `writer_scaling_929_bytes_gate` | wall-clock | The #929 SyncExpanseBytesMap multi-writer gate (concurrency METHODOLOGY.md §22.4): bytes cells (default build vs ablation-bytes-serial-writers, 8 interleaved rounds, W in {1, 2, 4, 8}, one process per cell, under the applied pin), read as G1 scaling, G2 level against the serialised build's best cell at any writer count, G3 the single-writer price floor, G4 overwrite under skew, and zero-restart tripwire; outputs gate_929_bytes_writer_scaling.json. |
| `ycsb_concurrent` | wall-clock | Concurrent YCSB benchmark suite over SyncExpanseMap across competitor arms (Zipfian θ = 0.99 and uniform, read-latest and read-modify-write, METHODOLOGY.md §20, Refs #1006); outputs baseline_ycsb_concurrent.json. |
...
| `leaf_cap_cascaded` | Callgrind | Callgrind `contains` / `get`, hit and miss, over 1M keys at 63 bits (λ = 30.52): the read path where `LEAF_CAP` changes the structure. Single-build; the cap-32 / cap-48 pair is two runs of it (`docs/benchmarks/hot_comparison/METHODOLOGY.md` §9.10.5). |
...
| `rocksdb_locate_profile` | `perf stat` | LBR call-graph sampling (`perf record --call-graph lbr`) of one idle reader in the RocksDB MemTable concurrent harness: the share of a Get inside the locked region and inside the trie call, the inputs the #802 narrowed-mutex arm is pre-registered on. Diagnostic only. |
...
| `rocksdb_concurrent_heldout` | wall-clock | RocksDB pluggable MemTable concurrent read scaling with an idle writer only, readers 1..7 interleaved per round: the held-out reader counts (R = 3, 5, 6) the #802 idle-curve model check is evaluated on, pre-registered in the suite's METHODOLOGY section 5.12. Dispatch with `cpu_pin=0,2,4,6,8,10,12,14`. |
| `rocksdb_concurrent_narrowed` | wall-clock | RocksDB pluggable MemTable concurrent read scaling under two seek lock scopes, interleaved per round: `full` (the mutex over the whole locate phase) and `trie` (the mutex only around `expanse_map_prev_at_or_before`), idle and paced writers, readers 1, 2, 4, 7. The #802 narrowed-mutex arm, pre-registered in the suite's METHODOLOGY section 5.14. Dispatch once per `cpu_pin`. |
| `rocksdb_concurrent_optimistic_counters` | `perf stat` | Per-thread hardware counters on the #802 optimistic-seek arm: paced writer at R = 1 and R = 7 under the `full`, `trie` and `opt` seek lock scopes, 5 rounds per cell, with `ld_blocks.store_forward` added (METHODOLOGY section 5.16). Reported and never gated; the engine's read-fallback rate is named unavailable. Dispatch once per `cpu_pin`, never beside a timing run. |
| `rocksdb_concurrent_optimistic` | wall-clock | RocksDB pluggable MemTable concurrent read scaling under three seek lock scopes, interleaved per round: `full`, `trie` and `opt` (the locate phase takes no lock and reads an `expanse_sync_map_t` through a per-thread reader handle), idle and paced writers, readers 1, 2, 4, 7, 78 rounds per cell. The #802 optimistic-seek arm, pre-registered in the suite's METHODOLOGY section 5.16. Dispatch once per `cpu_pin`, after the single-threaded bound and the soundness gates pass on the measured head. |
...
| `rcpc_ab_aarch64` | needs a Linux AArch64 host with `lrcpc`; the /bench host is self-hosted x86-64. Run `.github/workflows/aarch64_rcpc_ab.yml` via workflow_dispatch, twice (docs/BENCHMARKING.md rule 18). |
...
| `hot_concurrent` | needs an initialised `third_party/hot` submodule plus HOT's nested `third-party/tbb` submodule (libtbb is built from it into the cargo build dir; no system package) and an x86-64 host with AVX2 and BMI2. Each cell runs in its own process because ROWEX's reclamation state is process-global, and all threads must sit inside the P-core pin. It is driven by `docs/benchmarks/hot_comparison/run.sh --only-concurrent`, which takes the host-wide benchmark lock, not by `cargo bench`. |
| `hot_string_memory` | needs an initialised `third_party/hot` submodule and an x86-64 host with AVX2 and BMI2, and each cell must run in its own process because HOT's node pool is a process-global `static`. It is driven by `docs/benchmarks/hot_comparison/run.sh strings`, which runs the string validation gate first, takes the host-wide benchmark lock and sweeps population, not by `cargo bench`. |
| `hot_string_latency` | needs an initialised `third_party/hot` submodule and an x86-64 host with AVX2 and BMI2, and each cell must run in its own process because HOT's node pool is a process-global `static`. It is driven by `docs/benchmarks/hot_comparison/run.sh strings`, which runs the string validation gate first and takes the host-wide benchmark lock, not by `cargo bench`. |
| `masstree_latency` | needs both `third_party/hot` and `third_party/masstree` submodules initialised and an x86-64 host with AVX2 and BMI2 (the crate also builds the HOT shim, and both arms are bound to `-march=haswell`). Each cell runs in its own process on a fresh Masstree thread slot because Masstree's pools and RCU limbo are per thread and outlive every table. It is driven by `docs/benchmarks/masstree_comparison/run.sh`, which runs the validation gate first and takes the host-wide benchmark lock, not by `cargo bench`. |
| `masstree_string_latency` | needs both `third_party/hot` and `third_party/masstree` submodules initialised and an x86-64 host with AVX2 and BMI2 (the crate also builds the HOT shim, and both arms are bound to `-march=haswell`). Each cell runs in its own process on a fresh Masstree thread slot because Masstree's pools and RCU limbo are per thread and outlive every table. It is driven by `docs/benchmarks/masstree_comparison/run.sh`, which runs the validation gate first and takes the host-wide benchmark lock, not by `cargo bench`. |
| `masstree_memory` | needs both `third_party/hot` and `third_party/masstree` submodules initialised and an x86-64 host with AVX2 and BMI2 (the crate also builds the HOT shim, and both arms are bound to `-march=haswell`). Each cell runs in its own process on a fresh Masstree thread slot because Masstree's pools and RCU limbo are per thread and outlive every table. It is driven by `docs/benchmarks/masstree_comparison/run.sh`, which runs the validation gate first and takes the host-wide benchmark lock, not by `cargo bench`. |
| `masstree_concurrent` | needs both `third_party/hot` and `third_party/masstree` submodules initialised and an x86-64 host with AVX2 and BMI2 (the crate also builds the HOT shim, and both arms are bound to `-march=haswell`). Each cell runs in its own process on a fresh Masstree thread slot because Masstree's pools and RCU limbo are per thread and outlive every table. It is driven by `docs/benchmarks/masstree_comparison/run.sh`, which runs the validation gate first and takes the host-wide benchmark lock, not by `cargo bench`. Every cell keeps writers + readers <= 16 inside the P-core pin; the health cells come from a separate occ-stats build. |
| `remove_retention` | a deterministic `mem_used()` census from an example, not a `[[bench]]` target: it needs no bare-metal host and reproduces to the byte on any 64-bit machine. Run `cargo run --release -p expanse-trie --example remove_retention -- --json <path>` (`docs/benchmarks/remove_retention/README.md`; pre-registration in `METHODOLOGY.md` there). |
...
8. **Masstree** ([`kohler/masstree-beta`](https://github.com/kohler/masstree-beta) `1119842`, C++ through FFI, both the single-threaded and the concurrent configuration of its template) — trie-of-B+-trees baseline and the second measurement of write concurrency ([#661](https://github.com/orieg/expanse/issues/661), [`docs/benchmarks/masstree_comparison/`](benchmarks/masstree_comparison/README.md)). Its memory census carries two instruments — the shared allocator interposition, quantized to Masstree's 2 MiB pool slab, and Masstree's own node census — and insertion order is published as a sensitivity dimension because a B+-tree's insertion cost, leaf fill and footprint depend on it, and so do the trie's insertion cost and allocator footprint; only the trie's own node census is order-invariant.
...
2. **System-load hygiene.** Before the first run and between comparison runs, snapshot load (`ps -A -o %cpu,%mem,command | sort -rn | head`; load average vs core count). A non-target process above ~100% CPU, or a load-average shift > 2 between arms, contaminates the run: discard it, don't reinterpret it. Laptops running concurrent sessions are shared infrastructure. The snapshot is not advisory: it is recorded in the artifact and gated (`AGENTS.md` §8.17, `scripts/check_bench_provenance.py`), because a load average lags a newly started process by about thirty seconds and a green pre-check can sit in front of a contended run.
...
17. **Per-key memory figures state their expanse occupancy.** For any workload
    ...
    reference shape). The tooth repeats one byte level down at
    λ ≈ 256 × `LEAF_CAP`, so a λ in the thousands is not "past the curve";
    and a figure read from the *C-allocator* instrument (bytes held while the
    structure is live) also states its **insertion order**: the engine's
    `mem_used()` is order-independent, allocator-held bytes are not — 12.62
    versus 8.14 B/key for the same 1M @64 `ExpanseSet` inserted in generator
...
18. **A published cell is replicated before its level is quoted.** Within-run
    ...
    **A before/after claim on a concurrent cell therefore takes the two-commit
    form.** The FFI concurrent runners accept a base build of the same harness
    (`run_all.py --ab-base-bin <binary> --ab-base-commit <sha>`) and alternate
    the base and head processes round by round inside every cell (the harness's
    `--rounds 1 --round-offset K`, so the arm order stays the interleaving the
    single-build cell uses), reducing each build's rounds separately into
    `results/baseline_concurrent_ab*.json`. Two such runs make the pair; the
    base half must reproduce the committed pair's union before the head half
    means anything (`docs/benchmarks/concurrency/METHODOLOGY.md` §8.4). A head
    run interleaved against nothing — or against a base run taken on another
    day — is measuring the host.
...
19. **Insertion order is a workload dimension, declared and — where it
    ...
    `crates/expanse-hot-bench` sort the population after drawing it
    (`workload.rs` by value, `strings.rs` byte-lexicographically), and both
    comparative FFI suites build in that order. Ascending insertion is a
    B+-tree's best case: the key appends to the rightmost leaf and a split
    leaves a maximal key alone in the new one. It is not a neutral default,
    and the numbers say so. The Masstree sensitivity set measured leaf fill
    100% sorted against 70.7% shuffled, 22.76 against 33.10 B/key structural,
    and the `random` integer insert ratio 0.755 [0.749, 0.761] sorted against
    1.877 [1.856, 1.907] shuffled — a loss and a win on one cell *(measured:
    reference host, `docs/benchmarks/masstree_comparison/results/baseline_sensitivity.json`
    at harness commit `82966aae`; workloads differ: `masstree_memory` vs
    `masstree_latency` — the fill and byte figures are the memory census, the
    ratios the latency pillar)*. Lookup ratios move too (3.186 sorted, 3.546
    shuffled on the same cell, workload `masstree_latency`). That pre-registration had been informed
    by a shuffled Step 0 build and was evaluated on sorted cells; one of its
    ten unpredicted losses traces to the mismatch and the mechanism is
    plausible for two more
    ([`masstree_comparison/METHODOLOGY.md`](benchmarks/masstree_comparison/METHODOLOGY.md)
    §10.2). So, binding:
    - **Every harness declares `insertion_order`** in its workload shape
      table, opening with one of `both`, `sorted`, `shuffled`, `generator` or
      `n/a`. `scripts/check_bench_shapes.py` rejects anything else, and its
      globs reach `crates/expanse-hot-bench/src/bin/` — where the competitor
      arms live — as well as the benches and examples.
    - **A competitor whose cost or footprint is order-sensitive is measured in
      both orders**: the generator's sorted order and a Fisher–Yates
      permutation from the suite PRNG. Every result row records which. This is
      a review judgement, not something the linter decides; a B+-tree, a
      skip list and any structure with a rightmost-append path are the obvious
      cases, and "we did not check" is not a reason to declare `sorted`.
    - **The pre-registration states which order each prediction is evaluated
      on**, and a prediction formed on one order is not evaluated on the other.
    - **`mem_used` is the order-invariant instrument of the pair.** A digital
      trie's node census is fixed by the key set, so it must be identical in
      both orders; the allocator census is not, and moved 16.67 → 23.62 B/key
      on the same population. The invariant is pinned as an exact equality in
      [`crates/expanse/tests/test_mem_used_order_invariant.rs`](../crates/expanse/tests/test_mem_used_order_invariant.rs),
      not left to prose.
    - **Only `expanse-hot-bench` sorts.** `art_common`, `search_common` and the
      criterion harnesses insert in generator draw order, and `zset_common`
      shuffles deliberately so neither engine sees a favourable monotonic
      build. The declarations say so per harness; do not generalise one
      suite's regime to another.
...
| File | Population ($N$) | Insertion Order | Probes & Reuse | Hit Rate | Miss Gen Method | Value Dereference | Measured Region | Arm Symmetry | Statistics | Verdict & Notes |
|---|---|---|---|---|---|---|---|---|---|---|
| [`crates/expanse-capi/benches/smoke_instructions.rs`](../crates/expanse-capi/benches/smoke_instructions.rs) | 10k | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the shuffle in this file is applied to the probe stream, not to the build | 10k (shuffled), reuse 1.0 | 100% | None (hits only) | `sink ^= *slot` | Clean (setup in setup) | Self (C ABI) | iai Callgrind exact counts | **PASS** `[verified: RUN (CI callgrind-smoke)]` |
| [`crates/expanse-capi/benches/vs_stock.rs`](../crates/expanse-capi/benches/vs_stock.rs) | `POP = 30_000`, `POP_BIG = 1_500_000` (L204, 209); the `judysl_*` arms use `POP` | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the shuffle in this file is applied to the probe stream, not to the build | 30k / 1.5M, reuse 1.0 | 100% / mixed | Interleaved keys | `*slot` dereferenced | Deliberately leaks to exclude drop | **Three arms**: `*_expanse` (rlib, LTO-linked), `*_expanse_dl` (dlopen'd cdylib), and `*_stock` (dlopen'd libjudy). `*_expanse_dl` vs `*_stock` is symmetric; `*_expanse` vs `*_stock` is asymmetric (LTO bias). | iai Callgrind exact counts (simulated 8 MiB LL) | **PASS** `[verified: RUN (CI instruction-counts)]`: Carrying both `expanse` and `expanse_dl` isolates LTO delta; 1.5M population is load-bearing in #456. |
| [`crates/expanse-capi/examples/bench_vs_libjudy.rs`](../crates/expanse-capi/examples/bench_vs_libjudy.rs) | 100k, 1M | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the shuffle in this file is applied to the probe stream, not to the build | $2 \times N$ distinct, reuse 1.0 | 50% | Independent PRNG + membership rejection | `*(pval as *mut Word)` read | Clean (timing outside setup/drop) | Symmetric (`dlopen` vs `dlopen`) | Paired ratio + BCa bootstrap 95% CI | **PASS** `[verified: RUN (reference host, results/baseline_vs_libjudy.json)]`: Gold standard reference after #457. |
...
| File | Population ($N$) | Insertion Order | Probes & Reuse | Hit Rate | Miss Gen Method | Value Dereference | Measured Region | Arm Symmetry | Statistics | Verdict & Notes |
|---|---|---|---|---|---|---|---|---|---|---|
| [`crates/expanse/benches/batch_lookup.rs`](../crates/expanse/benches/batch_lookup.rs) | 100k, 4M | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the shuffle in this file is applied to the probe stream, not to the build | 4M (CHUNK 1024 rolling) | 50% | Same uniform-random generator as the population under a different seed, rejected on membership, bounded budget that panics on exhaustion (`generate_miss_keys`) | `black_box(&out[..CHUNK])` | Clean rolling offset | Symmetric (scalar vs batch lanes) | Criterion estimate | ✅ **RESOLVED in #470 — was DEFECT (Class 3)** `[verified: CODE READ]`: Fixed high-bit XOR alternating pattern corrupts batched descent depth (though #467 confirmed not the root of the 40% jump). |
| [`crates/expanse/benches/comparative.rs`](../crates/expanse/benches/comparative.rs) | 10k, 100k | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | `pop` pre-built shuffled probes per cell, wrapping counter (no `%`) | 50% (`contains`, `rank`, map lookup); N/A for `select` (probed by index) and `scan` (full iteration) | Same generator as the population, rejected on membership, bounded budget that panics on exhaustion; monotonic `sparse`/`dense` start the candidate stream at offset >= N | `black_box(contains)` / `black_box(get().copied())` | Clean: containers, key vectors and probe streams built outside `b.iter`; the insert cell uses `iter_batched(PerIteration)` and returns the container so Drop lands post-batch | Key and payload widths symmetric (`u64` keys, `u64` values across Expanse / hashbrown / `BTreeMap`); rank aligned (`count_below(k+1)` is exactly Roaring's inclusive `rank(k)`); one shared probe stream per cell. **Disclosed and NOT eliminated**: the key domain is 32-bit because `RoaringBitmap` is definitionally a `u32` set. Nothing is truncated (every cast widens), but zero-extension gives the Expanse arm four degenerate single-child levels before a discriminating byte, a cost Roaring does not pay — so **no cell here is comparable to a 64-bit cell elsewhere in the repo**. `sparse` here is a stride of 1000 in a 32-bit space, not the `i << 40` the rest of the repo means by the word | Criterion estimate | ⚠️ **PARTIAL — probe defects fixed, domain asymmetry disclosed** `[verified: CODE READ + RUN]`: was **DEFECT (Class 2, 5)**. Fixed: 100% hit rate on every lookup cell (now a pre-built, pre-shuffled 50/50 stream), the loop-carried `%` inside every timed window, `HashMap<u32, u32>` against `u64`-keyed rivals (§8.16), and `count_below` vs inclusive `rank` comparing two different quantities. **Not fixed**: "32-bit key truncation" was a mis-diagnosis — there is no truncation — and the real 32-bit *domain* asymmetry is inherent to pairing against Roaring, so it is disclosed in `arm_symmetry`, not removed. |
| [`crates/expanse/benches/compare.rs`](../crates/expanse/benches/compare.rs) | 10k, 1M, 4M | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the shuffle in this file is applied to the probe stream, not to the build | 4096 (10k/1M) / 2M (4M), looped | 50% (set) / 100% (map) | Same generator as the population, resumed at offset >= N, rejected on membership, bounded budget that panics on exhaustion (`generate_miss_keys`) | `black_box(get())` | Clean: `bench_insert` uses `iter_batched(PerIteration)` and returns the built container, so Drop lands post-batch | Asymmetric: SipHash `HashSet` vs `BTree` vs `Expanse` | Criterion estimate | ✅ **RESOLVED in #470 — was DEFECT (Class 1, 2, 3, 4, 5)** `[verified: CODE READ]`: 4k probes fit in L1/L2; XOR misses; Drop inside timed insert. |
| [`crates/expanse/benches/concurrency.rs`](../crates/expanse/benches/concurrency.rs) | 1M draws (keyspace 2M); `SyncExpanseMap32` arm: 4,096 draws over an 8k keyspace whose upper 4,096 keys the writer churns (32-bit) | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; prefill and the concurrent stream both draw from the bounded-keyspace PRNG | Continuous stream; `EXPANSE_BENCH_ROUNDS` interleaved 500 ms windows per thread count (Williams order), all on one prefilled structure per engine and workload | ~39% at 100% read (derived: 1M uniform draws over 2M keys occupy 1 − e^(−1/2) of the keyspace); under a mix, writes insert even keys and remove odd ones, which moves occupancy toward half | Bounded keyspace random stream | `black_box(sink)` | Clean (`run_rounds`): a window starts at a barrier after thread creation and per-thread setup, and its rates divide by its own elapsed time | Symmetric within three key types, not across them: u64 → u64 over 1M draws (`SyncExpanseMap`, `SyncExpanseSet`, with no third-party arm), u64 → 128-byte payload and u32 over 200k (`SyncExpanseBlobMap`, `Mutex<ExpanseBlobMap>`, `RwLock<BTreeMap>`, and `SkipMap`, whose values are `(Vec<u8>, u32)` although its label reads `Vec<u8>`), and 37-byte string keys → u64 over 100k (`SyncExpanseStrMap`, `SyncExpanseBytesMap`, their `Mutex` twins, `DashMap`). Compare arms only within a key type | Tables: mean ops/sec over a thread count's windows; `EXPANSE_BENCH_SAMPLES` carries every window for BCa 95% intervals (`docs/benchmarks/concurrency/scripts/mixed_concurrency.py`) | **MEASURED** `[verified: RUN (reference host, runs 34881026495 and 34882381735)]`: every cell of both runs, with its BCa interval, is in `docs/benchmarks/concurrency/README.md` §12; report-only, no gate is pre-registered on it. The #375 bounded-keyspace correction still holds. |
| [`crates/expanse/benches/embedded.rs`](../crates/expanse/benches/embedded.rs) | 500, 2k, 10k | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | 500, 2k, 10k, looped | 100% | None | `matched += 1` / `sum += v` | **LEAKY DROP**: `bench_sensor_indexing` drops set inside `b.iter` (L35, L46) | Symmetric (BTree vs HashBrown vs Expanse32) | Criterion estimate | ✅ **RESOLVED in #470 — was DEFECT (Class 4, 6)** `[verified: CODE READ]`: Drops set in timed loop; does not touch blob payload in scan. |
| [`crates/expanse/benches/embedded_memtable.rs`](../crates/expanse/benches/embedded_memtable.rs) | 500, 2k, 5k | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | 500, 2k, 5k, looped | 100% hits on present timestamps/IDs; 50% hits on mixed point lookups | None | direct integer reading and 28-byte slab/arena tracking payload dereference | timed inner loop over operations (allocations and setup outside) | symmetric random key sequences, matching 28-byte BLE tracking payload layout across all arms | Criterion estimate | ✅ Verified symmetric workloads with BCa CIs on reference host |
| [`crates/expanse/benches/instructions.rs`](../crates/expanse/benches/instructions.rs) | 50k; the `sync_*` arms' `leaf` variants build `LEAF_POP` (24) keys, below `ROOT_LEAF_CAP`, so the map or set stays a root leaf; `sync_strmap_insert_sorted` builds `UUID_POP` (20k) UUIDv4 strings; the remove-retention arms: `*_remove_partial` builds 200k random 60-bit keys, `*_rebuild_drained` and `*_compact_drained` the same tree drained to 62.5k by those removes, the `set_subtree_*` arms 64,512 one-key prefixes plus `SUBTREE_E` (1,024) driven level-6 expanses of 25–33 keys; the `BranchU` floor arms (#1079) build `FLOOR_DIGITS` (200) one-key top digits (drain) or `THRASH_TOP` (193) (thrash) | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the shuffle in this file is applied to the probe stream, not to the build. Exception: `sync_strmap_insert_sorted` inserts its keys sorted ascending, the order #1162 reported | 50k (shuffled), reuse 1.0; `leaf` variants cycle their 24 keys to 50k probes (reuse ≈ 2,083); the concurrent count arms take the first `COUNT_OPS` (1,000); `*_remove_partial` removes 137,500 keys in a Fisher–Yates order; `*_rebuild_drained` clones the drained tree once (62,500 keys) and drops the drained one; `*_compact_drained` compacts it in place once; the `set_subtree_*` arms make one operation per driven expanse, or `OSC_CYCLES` (8) cycles of 2 × band operations per expanse; the floor drain arms remove their 200 keys once in a shuffled order, and the thrash arm runs `THRASH_CYCLES` (100) cycles of two removals and two reinsertions of the same two keys | 100% | None for reads; the concurrent count arms write absent keys drawn from the population's distribution and rejected on membership (`fresh_keys`) | `black_box` on retrieved values | Clean (setup in setup) | Internal trie paths | iai Callgrind exact counts | **PASS** `[verified: RUN (CI instruction-counts)]`: Canonical instruction reference. |
| [`crates/expanse/benches/leaf_cap_cascaded.rs`](../crates/expanse/benches/leaf_cap_cascaded.rs) | 1,000,000 uniform random keys masked to 63 bits (λ = N / 2^(63−48) = 30.52; the 2M @64 cell at half the N) | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the shuffle in this file is applied to the probe stream, not to the build | 1,000,000 hit probes (the population, fixed-seed Fisher-Yates shuffle) and 1,000,000 miss probes, each probed once; reuse 1.0 | 100% on the `hit` arms, 0% on the `miss` arms (separate arms, so each Ir/probe is one descent shape) | Rejection-sampled from the same XorShift64 generator at the same 63-bit width under a second seed, rejected on membership and deduplicated (§8.6) | `hits += contains(k)` / `sink ^= get(k).unwrap_or(0)`, both through `black_box` | Clean (build, shuffle and miss sampling in `setup`; structure leaked, not dropped) | Set and map arms probe bit-identical key vectors; across a cap-32 / cap-48 pair the only variable is `LEAF_CAP` (build-time patch) | iai Callgrind exact counts | **PASS** `[verified: RUN (c81eaf5d, x86_64 dev host, Callgrind in a container)]`: Cascaded-regime read path; the cap-32 / cap-48 pair is published in `docs/benchmarks/hot_comparison/results/leaf_cap_cascaded_callgrind.json`. |
| [`crates/expanse/benches/leaf_cap_cascaded_wallclock.rs`](../crates/expanse/benches/leaf_cap_cascaded_wallclock.rs) | 1,000,000 uniform random keys masked to 63 bits (λ = N / 2^(63−48) = 30.52) | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the shuffle in this file is applied to the probe stream, not to the build | 65,536 hit probes (drawn from the population under a fixed seed) and 65,536 miss probes, cycled by criterion; reuse > 1 | 100% on the `hit` arms, 0% on the `miss` arms (separate arms) | Rejection-sampled from the same XorShift64 generator at the same 63-bit width under a second seed, rejected on membership and deduplicated (§8.6) | `black_box(contains(k))` / `black_box(get(k))` | Clean (build and probe generation before the group; `b.iter` holds one lookup) | Set and map arms probe bit-identical key vectors; across a cap-32 / cap-48 pair the only variable is `LEAF_CAP` (build-time patch), the same binary layout otherwise | criterion, n = 100 samples per arm; BCa 95% via `scripts/bench_baseline.py` | **PASS** `[verified: RUN (fa1704d4, hybrid reference host, P-core pinned)]`: five-run pair published in `docs/benchmarks/hot_comparison/results/leaf_cap_cascaded_contains_cap{32,48}_{a,repeat,b}.json`; cap 48 at 0.82–0.86× of cap 32 on every arm, same-build repeat within 0.5% (METHODOLOGY §9.10.5). |
| [`crates/expanse/benches/smoke_instructions.rs`](../crates/expanse/benches/smoke_instructions.rs) | 10k | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the shuffle in this file is applied to the probe stream, not to the build | 10k, reuse 1.0 | 100% | None | `sink ^= map.get().unwrap_or(0)` | Clean (setup in setup) | Internal trie paths | iai Callgrind exact counts | **PASS** `[verified: RUN (CI callgrind-smoke)]`: Fast smoke gate. |
| [`crates/expanse/examples/line_transfer.rs`](../crates/expanse/examples/line_transfer.rs) | one 128-byte-aligned `AtomicU64`; no keys | n/a — no data structure is built | `round_trips` (argument, default 200,000) hand-overs per direction, after a 10% warm-up of the same shape | n/a | n/a | the turn number is loaded with `Acquire` and compared on every wait; the final value is `black_box`ed | from both threads passing a barrier to A observing the last reply; thread spawn, affinity calls and warm-up are outside | both threads run the same loop with roles swapped; `spin` and `park` differ only in how the waiter waits; `pause` has one thread and no transfer; the pair (a, b) is measured once per invocation and the driver covers every unordered pair of physical P-cores | one point per invocation; the driver repeats and takes BCa 95% intervals (§8.4) | pending measurement |
...
| File | Population ($N$) | Insertion Order | Probes & Reuse | Hit Rate | Miss Gen Method | Value Dereference | Measured Region | Arm Symmetry | Statistics | Verdict & Notes |
|---|---|---|---|---|---|---|---|---|---|---|
| [`crates/expanse/benches/hashbrown_container_dists.rs`](../crates/expanse/benches/hashbrown_container_dists.rs) | 10k, 100k, 500k | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | Full keys, reuse 1.0 | 100% | None | `black_box(get)` | Clean | Symmetric | Raw ns/op (no CI) | **PASS / MINOR (Class 7)** `[verified: CODE READ]`: Clean timing, missing CI intervals. |
| [`crates/expanse/benches/hashbrown_memory_alloc.rs`](../crates/expanse/benches/hashbrown_memory_alloc.rs) | 1k to 500k | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | N/A (Memory) | N/A | N/A | Live bytes tracked | Clean `GlobalAlloc` hook | Symmetric keys | Exact byte count | **PASS** `[verified: CODE READ]`: Deterministic memory allocator census. |
| [`crates/expanse/benches/hashbrown_native_suite.rs`](../crates/expanse/benches/hashbrown_native_suite.rs) | 10k, 100k, 500k | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | Sub-slice `iters < pop` sequential index | 100% hit / 100% miss arms | Separate PRNG seed (no membership check) | `black_box(get)` | **LEAKY DROP**: `insert_growing` drops `m` inside `bench_op` (L218, L231, L244) | Symmetric | Raw ns/op (no CI) | ✅ **RESOLVED in #470 — was DEFECT (Class 2, 4, 7)** `[verified: CODE READ]`: Sequential sub-slice walk; Drop inside timed insert loop; no CI. |
| [`crates/expanse/benches/hashbrown_tail_latency.rs`](../crates/expanse/benches/hashbrown_tail_latency.rs) | 100k | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | 100k inserts | N/A | N/A | Records clamped latency | `Instant::now()` per op (calibrated overhead) | Symmetric | HdrHistogram percentiles | **PASS** `[verified: CODE READ]`: Documented per-op calibration overhead. |
| [`crates/expanse/benches/hashbrown_ycsb.rs`](../crates/expanse/benches/hashbrown_ycsb.rs) | 500k dense sequential keys `1..=N` (`--quick`: 50k; `--population` overrides), recorded in every row | both — every cell is built once ascending and once Fisher–Yates shuffled from the suite PRNG, the order recorded in every row; the operation stream is the same for both | 100k ops per cell (`--quick`: 20k; `--ops` overrides); one seeded stream per workload, reused across arms, orders and rounds | 100% on reads — Zipfian θ = 0.99 over the population (workload D: Zipfian over recency, in-run inserts first) | n/a — no miss probes; every read key is present | `black_box` on op results; values are `u64`, so there is no payload to dereference | Op loop only: the population is built before the timer starts and dropped after it stops | One op stream for every arm; a per-cell work checksum asserted equal across arms; hashbrown is disqualified from E (no ordered scan) and reported as such, never as zero | Per-round Mops/s rows; BCa 95% intervals and paired per-round ratios are computed by `scripts/ycsb_bench.py` | **PASS** `[verified: RUN (21a382f3, the reference host)]`: 8-round bare-metal measurements with BCa 95% intervals and paired per-round ratios; runs 35419619726 and 35419623818. |
...
| File | Population ($N$) | Insertion Order | Probes & Reuse | Hit Rate | Miss Gen Method | Value Dereference | Measured Region | Arm Symmetry | Statistics | Verdict & Notes |
|---|---|---|---|---|---|---|---|---|---|---|
| [`crates/expanse/benches/art_insert.rs`](../crates/expanse/benches/art_insert.rs) | 10k to 1M | generator draw order — ascending on `sequential` and `sparse`, draw order on `random`, `clustered` and `zipfian`; `art_common` neither sorts nor shuffles, and `dedupe_preserve_order` keeps first-seen order | Fresh cold build per round | 0% (Fresh distinct insertions) | None | Insertion return value `black_box`'d | Cold build loop | All containers grow cold from empty; identical key sequences | Median + BCa 95% Bootstrap CI over paired rounds | **PASS** `[verified: CODE READ]`: ART comparison dynamic insertion benchmark. |
| [`crates/expanse/benches/art_lookup_hit.rs`](../crates/expanse/benches/art_lookup_hit.rs) | 10k to 1M | generator draw order — ascending on `sequential` and `sparse`, draw order on `random`, `clustered` and `zipfian`; `art_common` neither sorts nor shuffles, and `dedupe_preserve_order` keeps first-seen order | Interleaved shuffled probe stream | 100% Hit | None | `black_box(*val)` | Clean lookup loop | Symmetric keys, cold insertion, identical PRNG | Median + BCa 95% Bootstrap CI over paired rounds | **PASS** `[verified: CODE READ]`: ART comparison point lookup hit benchmark. |
| [`crates/expanse/benches/art_lookup_miss.rs`](../crates/expanse/benches/art_lookup_miss.rs) | 10k to 1M | generator draw order — ascending on `sequential` and `sparse`, draw order on `random`, `clustered` and `zipfian`; `art_common` neither sorts nor shuffles, and `dedupe_preserve_order` keeps first-seen order | 50% hit / 50% miss interleaved stream | 50% Hit / 50% Miss | Same-distribution rejection sampling (`gen_distribution_misses`) | `black_box(*val)` on hit | Clean lookup loop over mixed probe stream | Symmetric keys, cold insertion, identical PRNG | Median + BCa 95% Bootstrap CI over paired rounds | **PASS** `[verified: CODE READ]`: ART comparison point lookup miss benchmark. |
| [`crates/expanse/benches/art_memory.rs`](../crates/expanse/benches/art_memory.rs) | 1k to 1M | generator draw order — ascending on `sequential` and `sparse`, draw order on `random`, `clustered` and `zipfian`; `art_common` neither sorts nor shuffles, and `dedupe_preserve_order` keeps first-seen order | N/A (Memory) | N/A | None | Live layout bytes tracked via TrackingAlloc | Clean GlobalAlloc hook | Symmetric keys and cold insertion | Exact deterministic byte count | **PASS** `[verified: CODE READ]`: ART comparison memory footprint benchmark. |
| [`crates/expanse/benches/art_scan.rs`](../crates/expanse/benches/art_scan.rs) | 10k to 1M | generator draw order — ascending on `sequential` and `sparse`, draw order on `random`, `clustered` and `zipfian`; `art_common` neither sorts nor shuffles, and `dedupe_preserve_order` keeps first-seen order | Full iteration, and bounded scans from `max(1000, 10^6/k)` shuffled starts per timed window | 100% (Existing keys in populated range) | None | `black_box(k ^ v)` during traversal | Clean scan loop; starts and population built outside it | Symmetric keys, PRNG and starts; visited-element counts compared per round and the cell voided when they differ | Median + BCa 95% Bootstrap CI over paired rounds, arms rotated round-robin; every cell carries `rounds_raw` | **PASS** `[verified: RUN (b447dbc0, reference host)]`: every `k > 0` cell at 1M reverses to Expanse under scan starts that scale with `k`; the `k = 0` control reproduces the superseded run within 2%. |
| [`crates/expanse/benches/art_small_payload.rs`](../crates/expanse/benches/art_small_payload.rs) | 1 to 7 keys | generator draw order — ascending; `gen_sequential` is the only generator at this population | Looped probe stream (10k lookups) & batched builds (1k constructions) | 100% Hit & 50/50 Rejection Miss & Dynamic Insertion | Rejection sampling | `black_box(*val)` / insertion sink | Clean lookup & cold build loops | Symmetric keys and PRNG | Median + BCa 95% Bootstrap CI over paired rounds; exact byte and allocation census | Empirically measured in baseline_small_payload.json |
| [`crates/expanse/benches/bench_grammar_masks.rs`](../crates/expanse/benches/bench_grammar_masks.rs) | 32k vocab | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | 32k, looped | 100% | None | `black_box(matches)` | Clean | Symmetric (Roaring vs Expanse) | Raw ns (no CI) | **PASS** `[verified: CODE READ]`: Domain-specific LLM mask benchmark. |
| [`crates/expanse/benches/bench_llm_datastore.rs`](../crates/expanse/benches/bench_llm_datastore.rs) | 100k tokens | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the token stream is ingested in stream order, and the sort in this file is the sorted-window competitor building its own index | Prefix search | 100% | None | `black_box(search)` | Clean | Symmetric | Raw ns (no CI) | **PASS** `[verified: CODE READ]`: Domain-specific datastore benchmark. |
| [`crates/expanse/benches/domain.rs`](../crates/expanse/benches/domain.rs) | 10k, 50k, 100k | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | Domain sets and key slices | 100% hits on insertion & resolution; 50% overlap on intersections | None | direct byte slice reading via `resolve()` | timed inner loop over operations (allocations and setup outside); glibc heap trimming disabled for the process so a batch's freed result sets are not returned to the OS and re-faulted inside the window (#701) | symmetric key sequences across DomainSet and ExpanseSet | Criterion estimate | **PASS** `[verified: CODE READ]`: Interned domain posting-list algebra and ingestion. |
| [`crates/expanse/benches/large_values.rs`](../crates/expanse/benches/large_values.rs) | 10k, 14k, 50k, 262k | generator draw order — the population is inserted as drawn, neither sorted nor shuffled, except the cold-DRAM predicate-scan sweep, which inserts shuffled on purpose so an ordered key scan touches arena offsets out of order and the prefetcher cannot hide the DRAM latency | Full scan | Selective $\sigma$ | Uniform meta filter | `view.len()` only in `bench_inline_vs_heap` (L80) & `selectivity_sweep` (L144); fixed in `cold_dram_large` | **LEAKY DROP**: `bench_inline_vs_heap` drops map inside `b.iter` (L43, L58) | Symmetric | Criterion estimate | ✅ **RESOLVED in #470 — was DEFECT (Class 4, 6)** `[verified: CODE READ]`: Retains uncorrected `selectivity_sweep` & `inline_vs_heap` beside corrected `cold_dram_large`. |
| [`crates/expanse/benches/patricia_insert.rs`](../crates/expanse/benches/patricia_insert.rs) | 10k to 1M | both — generator draw order and a Fisher–Yates permutation under `PROBE_SHUFFLE_SEED`, timed as separate arms in the same rounds | Every unique key inserted once per repetition into a fresh structure; repetitions calibrated to `MIN_WINDOW` | N/A (all inserts are new keys) | None | `black_box` of each insert's previous-value result | Insert loops only; each empty map is constructed before its window and dropped after it; one discarded warm-up pass per arm | Identical keys, values and orders; arm order rotates per round | Median per arm + geometric-mean paired ratios (Expanse/twin per order, generator/shuffled per arm) with BCa 95% CI on per-round log ratios | **PENDING** `[unmeasured]`: no committed run yet. |
| [`crates/expanse/benches/patricia_lookup_hit.rs`](../crates/expanse/benches/patricia_lookup_hit.rs) | 10k to 1M | both — generator draw order (ascending on `sequential` and `sparse_stride`) and a Fisher–Yates permutation under `PROBE_SHUFFLE_SEED`; one row per order | Every present key once per repetition, shuffled; repetitions calibrated so each timed pass is at least `MIN_WINDOW` | 100% | None | `black_box` of the returned value | Probe passes only; builds, validation and key encoding outside the window; one discarded warm-up pass per arm | Identical keys, values, probe order and build order; arms built separately; arm order rotates per round | Median per arm + geometric-mean Expanse/twin ratio with BCa 95% CI on per-round log ratios | **PENDING** `[unmeasured]`: no committed run yet. |
| [`crates/expanse/benches/patricia_lookup_miss.rs`](../crates/expanse/benches/patricia_lookup_miss.rs) | about n keys: a random half of the distinct keys of a 2n generator draw, n from 10k to 1M | both — generator draw order of the kept half and a Fisher–Yates permutation under `PROBE_SHUFFLE_SEED`; one row per order | Half the population (random) + as many held-out keys, shuffled; repetitions calibrated to `MIN_WINDOW` | 50% hit / 50% miss | Held-out random half of the same generator draw (`split_half`), so misses interleave the population | `black_box` of the returned value or 0 | Probe passes only; builds, validation and key encoding outside the window; one discarded warm-up pass per arm | Identical keys, values, probe order and build order; arms built separately; arm order rotates per round | Median per arm + geometric-mean Expanse/twin ratio with BCa 95% CI on per-round log ratios | **PENDING** `[unmeasured]`: no committed run yet. |
| [`crates/expanse/benches/patricia_memory.rs`](../crates/expanse/benches/patricia_memory.rs) | 1k to 1M | both — `u64`: generator order and a Fisher–Yates permutation under `PROBE_SHUFFLE_SEED`; strings: generator (random ids) and sorted; per-order counts on every row | N/A (memory) | N/A | None | None; counters read from the allocator hook | Build loop only; each structure dropped before the next is built | Identical key sets and values, one allocator hook for all arms; every arm owns its key bytes | Exact deterministic byte counts (requested and usable) | **PENDING** `[unmeasured]`: no committed run yet. |
| [`crates/expanse/benches/patricia_scan.rs`](../crates/expanse/benches/patricia_scan.rs) | 10k to 1M | both — `u64`: generator order and a Fisher–Yates permutation; strings: generator (random ids) and sorted; one row per order | Full traversal once per repetition; 64 fixed prefixes per repetition; repetitions calibrated to `MIN_WINDOW` | N/A (scans) | None | Every visited value summed; the sum reaches `black_box` | Scan passes only; builds and validation outside the window; one discarded warm-up pass per arm | Identical keys, values and build order; arms built separately; arm order rotates per round; the two twins without a borrowing prefix read pay an owned key per entry (disclosed); prefix cells add `expanse_unbounded`, the earlier Expanse walk, on its own map | Median per arm + geometric-mean Expanse/twin ratio with BCa 95% CI on per-round log ratios | **PENDING** `[unmeasured]`: no committed run yet. |
| [`crates/expanse/benches/patricia_string.rs`](../crates/expanse/benches/patricia_string.rs) | 10k to 1M, at each of four prefix lengths | both — generator order (random ids) and sorted byte order; one row per order | Half the population (random) + as many absent keys, shuffled; repetitions calibrated to `MIN_WINDOW` | 50% hit / 50% miss | Same generator, independent seed, rejected on membership (`gen_path_misses`) | `black_box` of the returned value or 0 | Probe passes only; builds, validation, NUL checks and probe assembly outside the window; one discarded warm-up pass per arm | Identical keys, values, probe order and build order; arms built separately; arm order rotates per round | Median per arm + geometric-mean Expanse/twin ratio with BCa 95% CI on per-round log ratios | **PENDING** `[unmeasured]`: no committed run yet. |
| [`crates/expanse/benches/search_boolean.rs`](../crates/expanse/benches/search_boolean.rs) | Synthetic postings | sorted ascending — a posting list is an ordered docID set by definition (`search_common` sorts and dedups), so no second order exists to measure | Postings sets | Intersection | N/A | `black_box(result)` | Clean | Symmetric (Roaring vs Expanse) | Raw ms (no CI) | **PASS** `[verified: CODE READ]`: Boolean index evaluation. |
| [`crates/expanse/benches/search_instructions.rs`](../crates/expanse/benches/search_instructions.rs) | 1k, 10k, 100k | sorted ascending — a posting list is an ordered docID set by definition (`search_common` sorts and dedups), so no second order exists to measure | Postings pairs | Intersection | N/A | `black_box(count)` | Clean (setup in setup) | Symmetric | iai Callgrind exact counts | **PASS** `[verified: CODE READ]`: Deterministic boolean instructions. |
| [`crates/expanse/benches/search_memory.rs`](../crates/expanse/benches/search_memory.rs) | Synthetic postings | sorted ascending — a posting list is an ordered docID set by definition (`search_common` sorts and dedups), so no second order exists to measure | N/A (Memory) | N/A | N/A | Live bytes tracked | Clean `GlobalAlloc` hook | Symmetric | Exact byte count | **PASS** `[verified: CODE READ]`: Memory census. |
| [`crates/expanse/benches/search_wand.rs`](../crates/expanse/benches/search_wand.rs) | 100k postings | sorted ascending — a posting list is an ordered docID set by definition (`search_common` sorts and dedups), so no second order exists to measure | Monotonic targets | Skip scan | N/A | `black_box(skipscan)` | Clean | Symmetric (Roaring vs Expanse) | Throughput Mops/s | **PASS** `[verified: CODE READ]`: WAND skip-scan comparison. |
| [`crates/expanse/benches/ycsb.rs`](../crates/expanse/benches/ycsb.rs) | 100k by default; 1M and 10M opt-in through `YCSB_POPULATIONS`, recorded in every bench id and result row | both — every cell is built once sorted ascending and once Fisher–Yates shuffled from the suite PRNG, the order recorded in every bench id and result row; the operation stream is generated from the canonical draw order and is identical across the two | 20,000 ops per criterion iteration; 200,000 ops per cell in the rounds/latency report (`YCSB_OPS` overrides); one deterministic stream per workload, reused across engines, orders and rounds | 100% on reads — every read targets a present key, Zipfian θ = 0.99 over the canonical draw order (workload D: Zipfian over recency, in-run inserts first) | n/a — no miss probes; every read key is drawn from the population or from the stream's own earlier inserts | blob arms read byte 0 of the 128 B payload on every read and scanned record; the `ExpanseMap` arm holds `u64` values and has no payload to dereference | Op loop only. Criterion routines return the structure so its drop is outside the timed region; the report builds and drops outside the runner's timer. Latency percentiles are window means (64 ops per `Instant` pair), never a per-op bracket | One op stream for every arm; key-parity scan predicate (selectivity identical by construction); a per-cell work checksum (`consumed`) asserted equal across arms | Criterion for local iteration; published cells come from per-round samples with BCa 95% intervals and paired per-round ratios (`scripts/ycsb_bench.py`) | **PASS** `[verified: RUN (21a382f3, the reference host)]`: 8-round bare-metal measurements across 100k, 1M, 10M populations, sorted and shuffled orders, with BCa 95% intervals, paired per-round ratios, and 64-op window-mean latencies (refuting 38 µs tail); runs 35419627251 and 35419631571. |
| [`crates/expanse/benches/ycsb_dense.rs`](../crates/expanse/benches/ycsb_dense.rs) | 100k by default; 1M and 10M opt-in through `YCSB_POPULATIONS`, recorded in every bench id and result row | both — every cell is built once sorted ascending and once Fisher–Yates shuffled from the suite PRNG, the order recorded in every bench id and result row; the operation stream is generated from the canonical draw order and is identical across the two | 20,000 ops per criterion iteration; 200,000 ops per cell in the rounds/latency report (`YCSB_OPS` overrides); one deterministic stream per workload, reused across engines, orders and rounds | 100% on reads — every read targets a present key, Zipfian θ = 0.99 over the canonical draw order (workload D: Zipfian over recency, in-run inserts first) | n/a — no miss probes; every read key is drawn from the population or from the stream's own earlier inserts | blob arms read byte 0 of the 128 B payload on every read and scanned record; the `ExpanseMap` arm holds `u64` values and has no payload to dereference | Op loop only. Criterion routines return the structure so its drop is outside the timed region; the report builds and drops outside the runner's timer. Latency percentiles are window means (64 ops per `Instant` pair), never a per-op bracket | One op stream for every arm; key-parity scan predicate (every other key of a dense run, identical across arms); a per-cell work checksum (`consumed`) asserted equal across arms | Criterion for local iteration; published cells come from per-round samples with BCa 95% intervals and paired per-round ratios (`scripts/ycsb_bench.py`) | **PASS** `[verified: RUN (21a382f3, the reference host)]`: dense clustered key shape confirms range scan inversion on dense keys: at 1M sorted ExpanseBlobMap leads BTreeMap 1.027x [1.026, 1.029] and at 10M sorted 1.157x [1.155, 1.158]; runs 35419627251 and 35419631571. |
| [`crates/expanse/benches/zset_memory.rs`](../crates/expanse/benches/zset_memory.rs) | 10k, 100k | shuffled — `zset_common::shuffled_members` builds through a Fisher-Yates permutation so neither engine sees a favourable monotonic build | N/A (Memory) | N/A | N/A | Live bytes tracked | Clean `GlobalAlloc` hook | Symmetric (SkipList vs Expanse) | Exact byte count | **PASS** `[verified: CODE READ]`: Memory census. |
| [`crates/expanse/benches/zset_range.rs`](../crates/expanse/benches/zset_range.rs) | 10k, 100k | shuffled — `zset_common::shuffled_members` builds through a Fisher-Yates permutation so neither engine sees a favourable monotonic build | Range windows | Range scan | Bounded score window | `black_box((m, sc))` | Clean | Symmetric | Median reduction | **PASS** `[verified: CODE READ]`: Range scan benchmark. |
| [`crates/expanse/benches/zset_rank.rs`](../crates/expanse/benches/zset_rank.rs) | 10k, 100k | shuffled — `zset_common::shuffled_members` builds through a Fisher-Yates permutation so neither engine sees a favourable monotonic build | Rank queries | Rank queries | Bounded score window | `black_box(acc)` | Clean | Symmetric | Median reduction | **PASS** `[verified: CODE READ]`: Rank query benchmark. |
| [`crates/expanse/benches/zset_zadd.rs`](../crates/expanse/benches/zset_zadd.rs) | 10k, 100k | shuffled — `zset_common::shuffled_members` builds through a Fisher-Yates permutation so neither engine sees a favourable monotonic build | Ops stream | Churn | Bounded score stream | `black_box(len)` | Clean | Symmetric | Median reduction | **PASS** `[verified: CODE READ]`: ZADD churn benchmark. |
...
| File | Population ($N$) | Insertion Order | Probes & Reuse | Hit Rate | Miss Gen Method | Value Dereference | Measured Region | Arm Symmetry | Statistics | Verdict & Notes |
|---|---|---|---|---|---|---|---|---|---|---|
| [`crates/expanse/benches/avx512_bitmap.rs`](../crates/expanse/benches/avx512_bitmap.rs) | 256 to 4,194,304 bitmap pairs (16 KiB to 256 MiB) | n/a — bitmap pairs are filled by index and no keyed population is built; the pointer-chase permutation is a traversal order, not an insertion order | Whole buffer traversed per iteration; buffer reused across arms | N/A (cardinality kernel, every pair is visited) | N/A | Every `count_and` result accumulated and `black_box`ed | Clean — buffers and permutation built in setup, outside `iter` | All four arms read the identical buffers in the identical order, with unaligned loads and an accumulator reduced once outside the loop; `scalar_popcnt` is the production-configuration baseline the vector arms are rated against | Criterion sampling; BCa 95% intervals via `scripts/bench_baseline.py --harvest` | **REPORT-ONLY** `[verified: RUN (Zen 5 AVX-512 host)]`: ceiling probe; gates nothing. |
| [`crates/expanse/examples/allocator_overhead.rs`](../crates/expanse/examples/allocator_overhead.rs) | N keys per shape (default 1,000,000; first argument) | generator draw order — each key is inserted as drawn, neither sorted nor shuffled | N/A (Memory) | N/A | N/A | `mem_used()` accounting beside allocator-reported usable bytes and RSS | Clean: keys generated into a reused buffer, one tree live at a time | One `ExpanseMap` or `ExpanseStrMap` per shape, 8-byte values | Exact byte counts; RSS is a single reading per shape | Diagnostic census; no gate. |
| [`crates/expanse/examples/avx512_probe.rs`](../crates/expanse/examples/avx512_probe.rs) | 1 probe | n/a — a CPUID query and one instruction; no population | Single CPUID query, optional single instruction | N/A | N/A | CPUID probe | Clean — untimed | Diagnostic check | Boolean status | **PASS** `[verified: RUN (Zen 5 AVX-512 host, native and under Callgrind)]`: CPUID masking and SIGILL both reproduced. |
| [`crates/expanse/examples/bench_lookup_compare.rs`](../crates/expanse/examples/bench_lookup_compare.rs) | 10k, 100k, 500k, 1M | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the shuffle in this file is applied to the probe stream, not to the build | $\min(N, 1\text{M})$, sampled with replacement | 50% hit / 50% miss | **DEGENERATE XOR**: `hit_k ^ (1<<63) ^ 0x5A5A_...` (L249) | Dereferences `*pval` for JudyL; `black_box(v)` for others | Clean (Instant outside build/drop) | **ASYMMETRIC**: `dlsym` function pointer for JudyL vs static Rust inlining | Raw medians (no BCa CI) | ✅ **RESOLVED in #470 — was DEFECT (Class 3, 5, 7)** `[verified: CODE READ]`: Source of `bench_report.py` table; degenerate XOR misses, dynamic fn ptr dispatch bias, no BCa CIs. |
| [`crates/expanse/examples/bytes_per_key.rs`](../crates/expanse/examples/bytes_per_key.rs) | 1k to 1M census; the `random` gate also samples 2M | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | N/A (Memory) | N/A | N/A | `mem_used()` accounting | Clean | Pure 64-bit census | Exact byte count | **PASS** `[verified: RUN (6c63826a)]`: Deterministic memory density census. |
| [`crates/expanse/examples/bytes_per_key_32.rs`](../crates/expanse/examples/bytes_per_key_32.rs) | 10k; 5k on the uniform-random arm | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | N/A (Memory) | N/A | N/A (no probes; the uniform-random arm rejects duplicates on insert) | `mem_used()` accounting | Clean | Pure 32-bit census | Exact byte count | **PASS** `[verified: RUN (4733dca5)]`: Deterministic 32-bit memory census. The uniform-random arm is the measured source of the sparse constant in `scripts/embedded_envelope.py`. |
| [`crates/expanse/examples/concurrent_scaling.rs`](../crates/expanse/examples/concurrent_scaling.rs) | 1M | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | Continuous stream in 500ms window | ~50% | Bounded keyspace | `black_box(sink)` | Clean | Symmetric across thread counts | Throughput ops/sec | **PASS** `[verified: CODE READ]`: Multi-threaded scaling driver. |
| [`crates/expanse/examples/fallback_maturity.rs`](../crates/expanse/examples/fallback_maturity.rs) | 0 / 10k / 100k / 1M prefill x 63- and 64-bit keyspace, 50k measured inserts per cell | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | none — insert-only, each key drawn once | n/a — no read probes | n/a — measured keys are the tail of the same single draw, so they are disjoint from the prefill by construction rather than by rejection | n/a — counter census, no payload read | counter delta around the measured inserts; prefill and teardown are outside it | map and set share one generator and seed, and are swept over the same keyspace widths | exact deterministic event counters; no interval (§8.4) | **PASS** `[verified: CODE READ]`: deterministic attribution census. |
| [`crates/expanse/examples/keyspace_density.rs`](../crates/expanse/examples/keyspace_density.rs) | 100k to 2.6M at 64 bits; 100k to 2M at 63 and 62 bits (plus exact-λ 27/40/58 cells); 2M at 58, 57 and 55 bits and 1.2M to 2.7M at 56 bits | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | N/A (Memory) | N/A | N/A | `mem_used()` accounting | Clean | Same PRNG and seed at every width; the width is the only variable | Exact byte count | **PASS** `[verified: RUN (fcca1c0d)]`: Deterministic density sweep; reproduces the §9.4 sweep of the HOT suite. |
| [`crates/expanse/examples/lookup_profile.rs`](../crates/expanse/examples/lookup_profile.rs) | 1M | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the shuffle in this file is applied to the probe stream, not to the build | 1M (shuffled), reuse 1.0 | 100% | None | Accumulates checksum | Clean | Internal attribution only | Diagnostic checksum | **PASS** `[verified: CODE READ]`: Sampling profiler attribution harness. |
| [`crates/expanse/examples/occ_stats_probe.rs`](../crates/expanse/examples/occ_stats_probe.rs) | 1M | generator draw order — the population is inserted as drawn, neither sorted nor shuffled | Continuous stream | ~50% | Bounded keyspace | Counted atomic stats | Clean | Protocol event counts | Exact event counters | **PASS** `[verified: CODE READ]`: Deterministic event counter probe. |
| [`crates/expanse/examples/perf_point_lookup.rs`](../crates/expanse/examples/perf_point_lookup.rs) | 1M (configurable via `EXPANSE_PERF_POP`); the string arms draw the suites' `short` (mean 12.0 B) and `counter` (12 B) shapes | both — the lookup arms insert in generator draw order (the shuffle in this file is applied to the probe stream, not to the build); the prefix-scan arms insert in generator order (`strmap_prefix_scan`) or sorted (`strmap_prefix_scan_sorted`) | 1M distinct per pass, reuse 1.0 | 100% default (configurable via `EXPANSE_PERF_HIT_PCT`) | Independent PRNG + membership rejection; the string arms continue the shape's own generator past the population and reject on membership (§8.6) | `sink ^= *pval`; the string and set arms add rather than XOR, since values `0..pop` XOR back to zero over a full pass and the sink would stop seeing its own loop | Phase differencing (`probe - build`) | Twin to Callgrind `core_instructions` | `perf stat` hardware counters | **PASS** `[verified: RUN (reference host, results/baseline_perf_counters.json)]`: Gold standard hardware counter reference. |
| [`crates/expanse/examples/popcnt_probe.rs`](../crates/expanse/examples/popcnt_probe.rs) | 1 probe | n/a — a single instruction; no population | Single instruction | N/A | N/A | CPUID probe | Clean | Diagnostic check | Boolean status | **PASS** `[verified: RUN (CI instruction-counts)]`: CPUID popcnt verification. |
| [`crates/expanse/examples/remove_retention.rs`](../crates/expanse/examples/remove_retention.rs) | N = 3.2M removed to M = 1M at the headline cell (random @64, λ 48.8 → 15.3 keys per 2-byte expanse); random @64 also at 2M → 1M (λ 30.5 → 15.3), 4M → 1M (λ 61.0 → 15.3), 3.2M → 2M, 3.2M → 320k and 1M → 312.5k; random @62 and @56, sequential, sparse and clustered at 3.2M → 1M; the `model_pins` shapes hold at most 433 keys | generator — the population is inserted in draw order; removal order is the declared per-cell variable (`shuffled`, `sorted`, `range`) | N/A (Memory) | N/A — every removal hits a present key | N/A | `mem_used()` / `mem_held()` accounting, `ExpanseStats::node_bytes` per form, `NodeAlloc::census` per slab class (with the allocator's live and total allocation counts), requested heap bytes from a counting global allocator | Clean: readings taken after the build, after the removals, after `shrink_to_fit()`, on a separate fresh build, on the rebuilt tree after `clone()` and after the drained tree is dropped, and on a second drained tree after `compact()`; each heap peak spans exactly the `clone()` or `compact()` call | The drained tree, the fresh build, the rebuilt tree and the compacted tree hold the identical key set (asserted); `shuffled` and `sorted` remove the identical set | Exact byte and page counts; no interval (deterministic accounting) | Diagnostic census; the Step 0a gate reads the headline cell (`docs/benchmarks/remove_retention/METHODOLOGY.md`). |
| [`crates/expanse/examples/strmap_drain_retention.rs`](../crates/expanse/examples/strmap_drain_retention.rs) | G × R keys (default G = 1,000, R = 1,000) | sorted — groups ascending, keys ascending within a group | N/A (Memory) | N/A | N/A | `mem_used()` / `mem_held()` beside counted live heap bytes, `mallinfo2` and RSS | Clean: one variant per process; readings after `malloc_trim(0)` | Same keys, values and order in every variant; only the second map and the scan buffers differ | Exact byte counts; RSS is a single reading per point | Diagnostic census; no gate. |
| [`crates/expanse/examples/value_compression_eval.rs`](../crates/expanse/examples/value_compression_eval.rs) | 100k per dataset | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; datasets are consumed in file order | 100k samples per shape | 100% | N/A (Evaluation census) | Codec round-trip verification | Offline codec census | Symmetric comparison across datasets | Exact count and promotion ratio | **PASS** `[verified: RUN (Phase 0)]`: Empirical evaluation of value compression crossover. |
| [`crates/expanse/examples/writer_scaling.rs`](../crates/expanse/examples/writer_scaling.rs) | prefill 2^20 keys (1M), plus 2^20 fresh keys inserted concurrently by W writers; reader mode (map only) adds 256 hotspot keys, one at offset 1 of every terminal byte of a 2^16-wide expanse, to every cell's prefill, and a hotspot cell's writers insert that expanse's other 65,280 keys instead of the 2^20 fresh keys; readers-only mode (set, str) prefills the arm's 2^20-key writer-sweep prefill and inserts nothing; a `one_top_byte` cell (#1144) prefills 2^20 keys under one top byte and inserts 2^20 fresh keys of that shape, with no hotspot keys. Writer mode's `bytes` arm uses the `str` arm's `short` keys under a fixed-key SipHash hasher; its `blob` arm uses the map arm's 64-bit keys with a 32-byte key-derived arena payload and non-zero 24-bit metadata; the blob and bytes overwrite cells prefill the same 2^20 keys and perform 2^20 overwrites of them, inserting no fresh key | sorted — prefill ascending (reader mode: the sorted union with the hotspot keys), matching expanse-hot-bench; fresh stream in generator draw order; hotspot fresh keys Fisher–Yates shuffled; blob overwrite targets in per-writer stream draw order over a Fisher–Yates rank table | writer mode: none (R = 0), insert-only; the blob overwrite cell re-targets prefilled keys, uniformly or Zipfian θ = 0.99 over the rank table, each writer on its own stream. Reader mode: R readers, each cycling its own Fisher–Yates permutation of the present uniform prefill (`uniform`) or of the 255 hotspot keys above the expanse's first terminal byte (`hotspot`); `get` or `prev_before` on a reader handle, or `prev_before` or `count_below` under `with_locked`, over the identical stream; a `one_top_byte` cell's readers cycle permutations of its own prefill; a `count_locked` cell may run with R = 0 as its writers-only control. Readers-only mode (set, str): R readers, each walking its own Fisher–Yates permutation of the prefill once, `contains` or `get` on a reader handle | writer mode: n/a. Reader mode: 100% — every probe is a present key; a hotspot `prev_before` fails in its own terminal byte and answers from the sibling below, until a writer inserts that byte's offset-0 key. Readers-only mode: 100% | same-generator rejection sampling against prefill; reader probes draw no misses | map arms check stored values against key-derived expectation; the blob arm checks payload bytes and metadata, and its overwrite cell that each read-back key holds its prefill or exactly one overwrite that targeted it; reader results fold key and value into a `black_box` accumulator | writer mode: barrier release to last-writer join; the blob overwrite cell's prefill, Zipfian table, rank draws and read-back check are outside. Reader mode: barrier release to the last writer join (writers) and to the last reader join (readers); with W ≥ 1 readers stop once the writers have joined, with W = 0 each makes 2^20 probes; prefill, workload generation, reader registration, answer checks and teardown outside; every reader-mode throughput row also records each reader's own loop time | symmetric across thread counts; W in {1, 2, 4, 8} on physical P-cores; reader mode: `prev` and `prev_locked` run the same per-reader probe stream over the same tree, interleaved within each round by the driver; readers-only mode: the map (reader mode W = 0 `get`), set and str arms at R in {1, 2, 4, 8}, the R cells of each arm interleaved within each round by the driver | throughput ops/sec emitted raw, paired bootstrap BCa 95% CI for C(N); lock fallbacks and their six causes (partition-checked per row) from the occ-stats counters pass; reader mode: reader Mops/s with a BCa 95% CI per cell, the P12.5 per-round paired `prev` / `prev_locked` ratio with a BCa 95% CI, the #1144 per-round paired writer Mops/s with and without a counting reader with a BCa 95% CI, and P12.4's summed `read_fallbacks ÷ read_ops` from the counters pass; readers-only mode: reader Mops/s with a BCa 95% CI per (arm, R), S(R) = T(R) / T(1) paired within each round with a BCa 95% CI, slowest-over-mean reader loop time per round, and summed read counters | pending measurement |
| [`crates/expanse/examples/ycsb_concurrent.rs`](../crates/expanse/examples/ycsb_concurrent.rs) | prefill 2^20 keys (1,048,576 keys) drawn from XorShift64 over the full 64-bit space, distinct, present before the window opens; A-dram and C-dram prefill 2^24 keys (16,777,216 keys); quick runs prefill 4,096 keys | sorted — prefill ascending in every arm, matching expanse-hot-bench and writer_scaling convention | T concurrent worker threads executing the family mix across pre-generated operation streams; point lookups and updates/inserts/RMW | 100% — every point read names a present key, and the harness counts misses and reports a cell with any as void; miss_gen_method is n/a | n/a | every read folds its value into a per-thread accumulator passed to black_box after the window; every write's return value is folded likewise (the previous value for a map insert and for fetch_add; a value-cell store returns nothing and folds 0); the post-window oracle checks every population key against the per-thread last writes, the per-key RMW counts and the final count | barrier release to join of last thread, with each thread's own loop timed between two instants taken before anything it owns can drop; prefill, stream generation, Zipfian tables, reader-handle registration and deregistration, stream deallocation, post-run checks and teardown outside | symmetric across arms and thread counts T in {1, 2, 4, 8} (and extra load points T in {3, 6} for olc and mutex on families A and B); one generic operation loop serves every arm | throughput total M ops/s emitted raw, paired bootstrap BCa 95% CI across interleaved Williams rounds; lock fallbacks and their causes, lock restarts, read-validation failures and stripe contended acquisitions from the occ-stats build, which emits no timing | pending measurement |
...
| File | Population ($N$) | Insertion Order | Probes & Reuse | Hit Rate | Miss Gen Method | Value Dereference | Measured Region | Arm Symmetry | Statistics | Verdict & Notes |
|---|---|---|---|---|---|---|---|---|---|---|
| [`crates/expanse-wasm-fuel/src/lib.rs`](../crates/expanse-wasm-fuel/src/lib.rs) | 10,000 keys per arm (the driver's default); `sequential` is 0..N, `clustered` is runs of 8 keys 4,096 apart, `random` is XorShift64 seed `0x0DDB_1A5E_5EED_0001` truncated to the key width, duplicates dropped | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; the sorted copy in this file computes the range windows and is not the build | 10,000 probes per arm in a seeded Fisher-Yates order, each once; reuse 1.0; `iterate` walks every key once; `range` is 100 windows that together cover the key space once | `insert` and `remove`: 100%, every key exactly once; `get` and `contains`: 50% hit / 50% miss | misses are drawn from the same XorShift64 stream after the population and rejected on membership; never a transform of a present key | every value, key and count the loop produces is XORed or added into the `u64` the export returns | fuel of phase 1 minus fuel of phase 0, exact integers; the build and the probe selection are in both phases and cancel | identical key stream, probe order and windows for the map and set arms and for both targets; the module source is byte-identical across wasm32 and wasm64 | none needed: wasmtime fuel is deterministic per module and runtime version; the driver runs every export twice and refuses to publish unless both runs agree to the unit | Callgrind analogue for the wasm targets. Fewer fuel units is better; `scripts/wasm_fuel.py --check-baseline` gates a change the way `perf_report.py` gates instruction counts |
| [`crates/expanse-wasm/tests/bench.js`](../crates/expanse-wasm/tests/bench.js) | 50,000 keys (`--quick`: 10,000); `random` is XorShift64 seed `0x0DDB_1A5E_5EED_0001` (u32 rows: low 32 bits, duplicates dropped; legacy rows: full u64), `sequential` is 0..N, `clustered` is runs of 8 keys 4,096 apart | generator draw order — the population is inserted as drawn, neither sorted nor shuffled — `sequential` is 0..N, `random` and `clustered` are inserted as generated | N probes per arm in a seeded Fisher-Yates order, each once; reuse 1.0; `iter` walks every key once; `range` is 100 windows of N/100 entries that together cover the key space once | `lookup_hit50`: 50% hit / 50% miss; legacy `lookup`: 100% hit (kept for baseline continuity, labelled as such); `insert` and `remove`: every key once | misses drawn from the continuation of the same XorShift64 stream and rejected on membership; never a transform of a present key | every returned value, key and count is folded into `sink_checksum`, which is emitted in the JSON | the arm's loop only; structures are built before the timer and dropped after it; the legacy `insert` row clears and refills inside the timer as it always has | identical keys, probe order and windows for every structure in a row. The Expanse `iter` cell crosses the JS boundary once per element (`first`/`next`; the package has no batch walk), so it measures the boundary as much as the engine; `WasmBTreeMap32` has no ordered walk in its JS surface so its `iter` cell is n/a and its `range` uses the same in-wasm `batch_range_scan` as the Expanse class; JS `Map`/`Set` have no ordered operations | min of `--rounds` (default 3) interleaved rounds per cell, single process; no interval and no gate — publish only with BCa 95% CIs from the quiet host (§8.4) | indicative only; the cross-language nightly reads the legacy rows, `docs/benchmarks/wasm/README.md` explains what each family can and cannot claim |

---

### Group 7: Comparative FFI Suites (`crates/expanse-hot-bench/src/bin/`)

| File | Population ($N$) | Insertion Order | Probes & Reuse | Hit Rate | Miss Gen Method | Value Dereference | Measured Region | Arm Symmetry | Statistics | Verdict & Notes |
|---|---|---|---|---|---|---|---|---|---|---|
| [`crates/expanse-hot-bench/src/bin/hot_concurrent.rs`](../crates/expanse-hot-bench/src/bin/hot_concurrent.rs) | prefill 2^20 uniform random keys (λ = 16 at 64 bits; 63-bit domain on the set arm), plus 2^20 fresh keys inserted concurrently by W writers | sorted ascending — the shared generator sorts and dedups the population (`workload.rs`); this harness takes no `order` token | R readers cycle a shuffled 2^20-probe stream against the prefill until the writers finish; at W = 0 each reader makes exactly one pass; hits strided across the whole sorted population, not its prefix (§8.6) | 50% against the prefill; some misses become hits as writers land, identically on both arms | same-generator rejection sampling (§8.6); fresh writer keys rejected on prefill membership | map arm fetches the stored value on both sides and checks it against its key-derived expectation; set arm checks presence of every prefill probe | barrier release to last-writer join (writers) and to last-reader join (readers); prefill, teardown and population walks outside | identical prefill, probe and fresh-key streams; both arms below any external lock through their native concurrent APIs (§8.16, §11.3 decision 4); same ISA target; W + R ≤ 16 inside the P-core pin, which is 16 logical CPUs on 8 physical P-cores — a cell with W + R > 8 places two threads per physical core (SMT siblings), and no interval says so | per-round throughput emitted raw, arms interleaved per round (`--rounds N --round-offset K` runs rounds K..K+N of that interleaving in one process, so a two-commit runner can alternate two builds round by round with the arm order continuous across them — the only before/after form docs/BENCHMARKING.md rule 18 admits); BCa 95% CIs on the Expanse ÷ ROWEX ratio computed by the runner (§8.4); every row carries the harness's own operation counts (`*_read_ops` = probes completed by the readers, `*_write_ops` = fresh keys inserted) as the divisor for any per-operation figure (§8.9 principle 5); `--health` emits event ratios from a diagnostic build and never a timing; `--arm <expanse or rowex>` runs one arm alone and emits `counters` rows (`read_ops`, `write_ops`, elapsed) for `scripts/bench_counters.py`'s per-thread `perf stat` attach, never a comparison | pending measurement |
| [`crates/expanse-hot-bench/src/bin/hot_latency.rs`](../crates/expanse-hot-bench/src/bin/hot_latency.rs) | selected per invocation; `random` cells also carry λ | both — the shared generator sorts and dedups (`workload.rs`), and the trailing `order` token selects that or a Fisher-Yates permutation from the suite PRNG; the runner sweeps both and every result row carries the order (§12.2) | shuffled stream, one pass, `population`-many probes; `scan` takes `max(1000, 10⁶ / k)` starts from that stream, cycling it when shorter (§12.1); hits strided across the whole sorted population, not its prefix (§8.6) | 100% for `lookup_hit`, 50% for `lookup_miss`, n/a otherwise | same-generator rejection sampling (§8.6) | Arm B fetches the stored value on both sides and sinks it (§9.8) | probe loop only; build and teardown outside the timed window; the arm timed first alternates per round and is recorded as `first_arm` (§12.1) | identical key and probe streams within a pairing; same ISA target; both arms see the same insertion order, `sorted` unless the cell says `shuffled` (§12.2) | per-round medians emitted raw; BCa 95% CIs computed by the harvester (§8.4) | pending measurement |
| [`crates/expanse-hot-bench/src/bin/hot_memory_curve.rs`](../crates/expanse-hot-bench/src/bin/hot_memory_curve.rs) | swept; reported as λ, not as N (§9.6); `rowex_set` / `rowex_map` arms (feature `rowex`) census ROWEX against `SyncExpanseSet` / `SyncExpanseMap` on the same λ targets, build-only, single writer (§11.3 decision 1) | both — the shared generator sorts and dedups (`workload.rs`), and the trailing `order` token selects that or a Fisher-Yates permutation from the suite PRNG; the runner sweeps both and every result row carries the order (§12.2) | N/A (memory census) | N/A | N/A | N/A — live bytes held from the C allocator | build only; census armed around the build, teardown excluded | one allocator interposition measures both arms (§9.1); identical key streams within a pairing; both arms see the same insertion order, `sorted` unless the cell says `shuffled` (§12.2) | exact byte counts, deterministic — no interval (§8.4) | pending measurement |
| [`crates/expanse-hot-bench/src/bin/hot_string_latency.rs`](../crates/expanse-hot-bench/src/bin/hot_string_latency.rs) | selected per invocation; mean key length reported per cell | both — `strings.rs` sorts byte-lexicographically, and the trailing `order` token selects that or a Fisher-Yates permutation from the suite PRNG; the runner sweeps both and every result row carries the order | shuffled stream, one pass, `population`-many probes, every probe its own allocation; `scan` takes `max(1000, 10⁶ / k)` starts from that stream, cycling it when shorter (§12.1); hits strided across the whole sorted population, not its prefix (§8.6) | 100% for `lookup_hit`, 50% for `lookup_miss`, n/a otherwise | same-generator rejection sampling (§8.6) | both sides return the stored word and fold it; Arm C/E sinks must be equal (§10.2) | probe loop only; string generation, build and teardown outside the timed window; the arm timed first alternates per round and is recorded as `first_arm` (§12.1) | identical strings and probe stream within a pairing; same ISA target (§3.3); HOT column withheld where the §10.4 predicate fails; both arms see the same insertion order, `sorted` unless the cell says `shuffled` (§12.2) | per-round medians emitted raw; BCa 95% CIs computed by the harvester (§8.4) | pending measurement |
| [`crates/expanse-hot-bench/src/bin/hot_string_memory.rs`](../crates/expanse-hot-bench/src/bin/hot_string_memory.rs) | swept by the runner (§10.5); one cell per invocation | both — `strings.rs` sorts byte-lexicographically, and the trailing `order` token selects that or a Fisher-Yates permutation from the suite PRNG; the runner sweeps both and every result row carries the order | N/A (memory census) | N/A | N/A | N/A — live bytes held from the C allocator | build only; census armed around each index build and, separately, around the string table; teardown excluded | one allocator interposition measures both indexes (§9.1); the harness-owned strings are counted on neither side and published as their own column (§10.3); both arms see the same insertion order, `sorted` unless the cell says `shuffled` (§12.2) | exact byte counts, deterministic — no interval (§8.4) | pending measurement |
| [`crates/expanse-hot-bench/src/bin/hot_string_validate.rs`](../crates/expanse-hot-bench/src/bin/hot_string_validate.rs) | 100k per shape for fidelity; 50k for the allocation-count checks; 1k for the truncation finding | sorted byte-lexicographic — `strings.rs` sorts the population; a fidelity gate, not a timing | 50/50 shuffled stream, one pass, every probe its own allocation; hits strided across the whole sorted population, not its prefix (§8.6) | 50% | same-generator rejection sampling (§8.6) | both sides return the stored word and the two sinks are compared | N/A — deterministic checks, no timing | identical strings and probe stream on both sides of every pairing | exact counts; no interval (§8.4) | gate — pass/fail, never a published figure |
| [`crates/expanse-hot-bench/src/bin/masstree_concurrent.rs`](../crates/expanse-hot-bench/src/bin/masstree_concurrent.rs) | prefill 2^20 keys (uniform random u64 at 64 bits, or `short` strings), plus 2^20 fresh keys inserted concurrently by W writers | sorted ascending — the shared generator sorts and dedups the population (`workload.rs`); this harness takes no `order` token | R readers cycle a shuffled 2^20-probe stream against the prefill until the writers finish; at W = 0 each reader makes exactly one pass; hits strided across the whole sorted population, not its prefix (§8.6) | 50% against the prefill; some misses become hits as writers land, identically on both arms | same-generator rejection sampling (§8.6); fresh writer keys rejected on prefill membership | both sides fetch the stored value and check it against its key-derived expectation | barrier release to last-writer join (writers) and to last-reader join (readers); prefill, teardown and walks outside; Masstree's per-64-op `quiesce` inside its threads (§3.2) | identical prefill, probe and fresh-key streams; both arms below any external lock through their native concurrent APIs (§8.16); one thread slot per Masstree thread; same ISA target; W + R ≤ 16 inside the P-core pin, which is 16 logical CPUs on 8 physical P-cores — a cell with W + R > 8 places two threads per physical core (SMT siblings), and no interval says so | per-round throughput emitted raw, arms interleaved per round (`--rounds N --round-offset K` runs rounds K..K+N of that interleaving in one process, so a two-commit runner can alternate two builds round by round with the arm order continuous across them — the only before/after form docs/BENCHMARKING.md rule 18 admits); BCa 95% CIs on the Expanse ÷ Masstree ratio computed by the runner (§8.4); every row carries the harness's own operation counts (`*_read_ops` = probes completed by the readers, `*_write_ops` = fresh keys inserted) as the divisor for any per-operation figure (§8.9 principle 5); `--health` emits event ratios from a diagnostic build and never a timing; `--arm <expanse or masstree>` runs one arm alone and emits `counters` rows (`read_ops`, `write_ops`, elapsed) for `scripts/bench_counters.py`'s per-thread `perf stat` attach, never a comparison | pending measurement |
| [`crates/expanse-hot-bench/src/bin/masstree_latency.rs`](../crates/expanse-hot-bench/src/bin/masstree_latency.rs) | selected per invocation; `random` cells also carry λ | both — the shared generator sorts and dedups (`workload.rs`), and the trailing `order` token selects that or a Fisher-Yates permutation from the suite PRNG; the runner sweeps both and every result row carries the order (§12.2) | shuffled stream, one pass, `population`-many probes; scan: `max(1000, 10⁶ / k)` starts cycled from the probe stream; hits strided across the whole sorted population, not its prefix (§8.6) | 100% for `lookup_hit`, 50% for `lookup_miss`, n/a otherwise | same-generator rejection sampling (§8.6) | both sides fetch the stored value and fold it; the two sinks must agree | probe loop only; build and teardown outside the timed window; Masstree's per-64-op `quiesce` inside its loop (§3.2); the arm timed first alternates per round | identical key and probe streams, full 64-bit domain; same ISA target (§3.5) | per-round medians emitted raw; BCa 95% CIs computed by the harvester (§8.4) | pending measurement |
| [`crates/expanse-hot-bench/src/bin/masstree_memory.rs`](../crates/expanse-hot-bench/src/bin/masstree_memory.rs) | swept by the runner: λ targets on `random` u64 (§5), one N = 10⁶ cell per structured distribution, the string population sweep; one cell per invocation | both — the shared generator sorts and dedups (`workload.rs`), and the trailing `order` token selects that or a Fisher-Yates permutation from the suite PRNG; the runner sweeps both and every result row carries the order (§12.2) | N/A (memory census) | N/A | N/A | N/A — live bytes held from the C allocator, and each engine's own node census | build only; the census is armed before the thread slot and table are created and disarmed after the walk; teardown excluded | one allocator interposition measures both arms; Masstree's figure is slab-quantized and carries its measured slack and the `QUANTUM_DOMINATED` flag; Masstree `json_stats` beside Expanse `mem_used` (§3.3) | exact byte counts, deterministic — no interval (§8.4) | pending measurement |
| [`crates/expanse-hot-bench/src/bin/masstree_string_latency.rs`](../crates/expanse-hot-bench/src/bin/masstree_string_latency.rs) | selected per invocation; mean key length reported per cell | both — `strings.rs` sorts byte-lexicographically, and the trailing `order` token selects that or a Fisher-Yates permutation from the suite PRNG; the runner sweeps both and every result row carries the order | shuffled stream, one pass, `population`-many probes, every probe its own allocation; scan: `max(1000, 10⁶ / k)` starts cycled from the probe stream; hits strided across the whole sorted population, not its prefix (§8.6) | 100% for `lookup_hit`, 50% for `lookup_miss`, n/a otherwise | same-generator rejection sampling (§8.6) | both sides fetch the stored value and fold it; the two sinks must agree | probe loop only; string generation, build and teardown outside; Masstree's per-64-op `quiesce` inside its loop (§3.2); the arm timed first alternates per round | identical strings and probe stream; both sides copy key bytes into their own nodes (§4); same ISA target; Masstree column withheld where the §3.4 predicate fails | per-round medians emitted raw; BCa 95% CIs computed by the harvester (§8.4) | pending measurement |
| [`crates/expanse-hot-bench/src/bin/masstree_validate.rs`](../crates/expanse-hot-bench/src/bin/masstree_validate.rs) | 200k per integer distribution and 100k per string shape for fidelity; 1k for the key-length finding; 200k prefill + 200k fresh for the threaded checks; 100k for the census | sorted — `workload.rs` sorts the integer population and `strings.rs` the string shapes; a fidelity gate, not a timing | 50/50 shuffled stream, one pass, every string probe its own allocation; hits strided across the whole sorted population, not its prefix (§8.6) | 50% | same-generator rejection sampling (§8.6) | both sides return the stored value and the two sinks are compared | N/A — deterministic checks, no timing | identical keys and probe stream on both sides of every pairing | exact counts; no interval (§8.4) | gate — pass/fail, never a published figure |

---

### Group 8: RocksDB MemTable Integration (`integrations/rocksdb/benches/`)

| File | Population ($N$) | Insertion Order | Probes & Reuse | Hit Rate | Miss Gen Method | Value Dereference | Measured Region | Arm Symmetry | Statistics | Verdict & Notes |
|---|---|---|---|---|---|---|---|---|---|---|
| [`integrations/rocksdb/benches/bench_memtable.cc`](../integrations/rocksdb/benches/bench_memtable.cc) | 100,000 entries: 16-byte user key (`usr_` + 12 zero-padded digits of `rng() % 10^10`), 64-byte value, `mt19937_64` seed 1337. Each arm encodes its own copy into its own `Arena`, so no arm reads another's bytes | generator — inserted in the draw order of the suite PRNG; no sort and no shuffle is applied | 50,000 probes drawn as `keys[rng() % N]`, so every probe names a key that IS in the structure. The same probe vector is reused by `readrandom` and `seekrandom`, and by all three arms | **~9% (measured 8.98%)**, not ~100% as "every probe names a present key" suggests. Entries carry `seq = 1000 + i` for i in [0, 100000), so sequence numbers run 1,000..100,999, while every `LookupKey` is built at snapshot 10,000. MVCC hides every entry newer than the query, so only the 9,001 keys with `1000 + i <= 10000` can be found — 9.001% of the population, and 8.98% of probes hit in practice. `readrandom` is therefore predominantly a miss-path measurement | n/a — no miss keys are generated. The ~91% of probes that miss are *present* keys made invisible by the snapshot above, which descends to the key's own leaf and fails on sequence rather than terminating early in a different expanse. That is a different shape from a key-space miss and is not the same thing AGENTS.md §8.6's miss-shape rule asks for | none — the `Get` callback takes `const char*` unnamed and only increments a counter, so no arm reads the stored value. In `--arm` mode that counter is printed as the `consumed` column, so the probe loop's result is consumed; in the human-readable mode it is not printed at all, and the loop survives only because `Get` is a virtual call into a separate object file. `seekrandom` consumes nothing in either mode: `Seek` returns void and the cursor it moves is never read | `chrono::high_resolution_clock` around each benchmark's probe or insert loop. Entry encoding, `LookupKey` construction and iterator construction are outside it. The three reps live to the end of `main`, so no destructor runs inside a timed window. In `--arm <phase>` mode the fixture build still runs in full (it is what the read phases need) and only the named phase is timed and reported, so each phase starts from the same state instead of being warmed by the phases before it in a single process | identical `BenchBytewiseComparator`, identical probe and key streams, one `Arena` per arm. `ReferenceSkipListRep` models a variable-height `InlineSkipList` tower (8 B key pointer + `height` × 8 B, E[height] = 4/3) after the #372 strawman retraction; `ReferenceVectorRep` models an unindexed append vector. All three implementations of a phase are timed inside ONE invocation, so a published ratio's two arms share one host state; the `consumed` column is the same for all of them and the driver refuses a round where it is not, which is what makes a scan that terminated early visible rather than merely fast. Within `prefixscan` the iterator arm runs before the batch arm and warms the data for it — pre-existing, and unchanged here. Single-threaded throughout: no arm is thread-safe and none is driven concurrently (see `bench_memtable_concurrent.cc` for the concurrent arm) | point estimates only; this binary emits no interval. `docs/benchmarks/rocksdb_memtable/scripts/single_threaded_bench.py` owns the rounds — one `--arm <phase>` process per measured cell, phases interleaved within each round — and turns them into BCa 95% per-arm intervals plus **paired** per-round ratio intervals, with a load snapshot and a busy-CPU delta per cell (#868) | published in `docs/benchmarks/rocksdb_memtable/METHODOLOGY.md` §6 — two runs at `7cd5140e`, pin `0-15`, through `docs/benchmarks/rocksdb_memtable/scripts/single_threaded_bench.py` (#868), artifacts `results/baseline_rocksdb.json` and `results/baseline_rocksdb_run2.json`. §2's `6cb64b45` wall-clock figures are superseded by them and stay in place as that run's record. §6.2 reads every row against `docs/BENCHMARKING.md` rule 18: point lookup and its vs-VectorRep twin move up in both runs, batch scan against SkipList moves **down** in both, and no other cell is confirmed as moved |
| [`integrations/rocksdb/benches/bench_memtable_concurrent.cc`](../integrations/rocksdb/benches/bench_memtable_concurrent.cc) | 100,000 keys, 16-byte user key, 64-byte value — the same fixture `bench_memtable.cc` builds, so both are read on one workload | generator — keys are inserted in the draw order of the suite PRNG (mt19937_64, seed 1337), exactly as `bench_memtable.cc` does; no sort and no shuffle is applied | R reader threads cycle a per-thread shuffled stream of the pre-populated keys for the cell's whole window; each reader has its own stream and offset so readers do not share a cursor. Probes are drawn ONLY from the pre-populated set, never from keys the writer adds during the cell, so the hit rate does not drift with the writer's progress | 50% by construction against the pre-populated set: half the probes are present keys, half are same-generator misses | same-generator rejection sampling (§8.6) — miss keys are drawn from the same `mt19937_64` stream that built the population and rejected on membership, never a transform of a present key | every `Get` callback reads the entry pointer and accumulates its first payload byte into a per-thread sink consumed after the join, so no probe is dead code | barrier release to the cell's deadline, readers only. Population build, thread spawn, probe-stream generation, the writer's pacing sleeps and all teardown are outside it | single-arm: this is an Expanse self-scaling curve, not a comparison. `ReferenceSkipListRep` and `ReferenceVectorRep` in `bench_memtable.cc` are not thread-safe (raw `Node* next[]` with a non-atomic `count_`; `EnsureSorted` sorts through a `const_cast` in a `const` method with no lock), so a concurrent cell against either would be a data race rather than a baseline — see METHODOLOGY §5.5 | ONE cell per process invocation, emitted as a single raw CSV row; the driver orders invocations so `(writer-mode × R)` interleaves within each round (§8.20.2) and computes the paired BCa 95% interval on `S(R) = T(R)/T(1)` across rounds (§8.4). This binary emits no interval and no ratio | pending measurement |
...
| **docs/DATABASE.md (L516–L526)**: YCSB Workloads A–F Throughput Table | `crates/expanse/benches/ycsb.rs` | ✅ **RESOLVED in #1005 — was IMPACTED**: Re-measured on the reference host across 8 paired rounds, 100k/1M/10M populations, both insertion orders and two key shapes with BCa 95% intervals (see "Standardized YCSB Workload Suite", §2). | None — re-measurement completed in #1005. |
...
| RocksDB MemTable | [`rocksdb_memtable/`](benchmarks/rocksdb_memtable/README.md) | `benches/bench_memtable.cc` (`integrations/rocksdb`) | `ReferenceSkipListRep` (fair variable-height `InlineSkipList` equivalent), `VectorRep` | 1.42× higher RAM key density (13.2 vs 18.7 B/entry; #372 strawman retracted, and reproduced byte-for-byte at `7cd5140e`); at `7cd5140e`, two runs each — sequential scan 3.1426× [3.1061, 3.2269] and 3.0744× [3.0480, 3.1095], batch scan 2.1376× [2.0452, 2.3111] and 2.1301× [2.0820, 2.1930], point lookup 1.4915× [1.4901, 1.4939] and 1.4985× [1.4913, 1.5073], range seek 1.5318× [1.5225, 1.5377] and 1.5348× [1.5268, 1.5414], insert 1.4734× [1.4414, 1.4856] and 1.4757× [1.4609, 1.4859]; VectorRep ceiling 10.5 B/entry. The `6cb64b45` figures are superseded; batch scan is the one cell both runs move clear of them, and it moves **down** (a loss). §6 of the suite METHODOLOGY reads each row against rule 18 |
...
| STM32H747I-DISCO on-target (hardware) | [`stm32h747/`](benchmarks/stm32h747/README.md) | `integrations/stm32h747` firmware, both cores | sorted array + `bsearch`/`memmove`, open-addressing hash, newlib `tsearch`; `cpsid`/`cpsie` critical section (ISR); HSEM lock (cross-core) | see suite README: cache-line geometry confirmed (1.7–1.9× cache-on/off at 2:1 core:bus, all of it `LSUCNT` at an identical instruction count); ISR entry latency bounded 29–103× (M7) / 53–54× (M4) tighter than masking; loses point lookup and unordered ingest to the hash table (5.1× / 12×) and sorted array, wins steady-state expiry (3.7×) and bytes/key on dense keys; cross-core correct only with an uncached heap — the unsupported cacheable case returned wrong values in two of eight captures, so it does not fail safe |
...
| Masstree | [`masstree_comparison/`](benchmarks/masstree_comparison/README.md) | `masstree_latency`, `masstree_string_latency`, `masstree_memory`, `masstree_concurrent` (crate `expanse-hot-bench`, feature `masstree`) | `kohler/masstree-beta` `1119842` through a C++ FFI shim, `concurrent = false` against `ExpanseMap` / `ExpanseStrMap` and `concurrent = true` against `SyncExpanseMap` / `SyncExpanseStrMap` | see suite README (Expanse wins integer point lookup 3.2×–12.4× and every integer scan; Masstree wins write concurrency from two writers on integers and one on strings, readers under any writer, every string scan and every string insert, and sorted-order integer insertion on `random`/`sparse`/`clustered`; memory splits by density and by insertion order) |
...
| `set_rebuild_drained/*` · `map_rebuild_drained/*` | `clone()` of the `*_remove_partial` tree after its removes (200k grown, 62.5k kept), then drop of the drained tree | The rebuild arm of the remove-retention census ([`remove_retention/README.md`](benchmarks/remove_retention/README.md), "Allocator census and rebuild"): the copy's iteration and build plus the old tree's free, per surviving key. |
| `set_compact_drained/*` · `map_compact_drained/*` | `compact()` of the `*_remove_partial` tree after its removes (200k grown, 62.5k kept), in place | The compact arm of the remove-retention suite ([`remove_retention/METHODOLOGY.md`](benchmarks/remove_retention/METHODOLOGY.md) §12): the ordered walk, the bottom-up build of the new tree (the set through `from_sorted_iter`'s builder, the map through its bulk builder) and the drop of the old one. G-cost compares it with the rebuild arm of the same run. |
...
| `sync_map_count_after_write/*` | Insert an absent key, `count_below` through `with_locked`, remove it | Minus `sync_map_count_locked` and `sync_map_write_twin` (same probe and absent-key streams, `COUNT_OPS` = 1,000 steps each), the fold of the ancestor `pop0` the writes left stale (#1144). |
...
| `one_top_byte` | 64-bit uniform keys under one top byte: `0xA5 << 56 \| (r >> 8)` | Every key shares `digit(key, 8)`, so the concurrent map's per-top-digit dirty mask has one bit for the whole population (#1144): each count after a write refolds the whole tree. |

### 3. Arm Inventory (generated)

<!-- BEGIN ARM INVENTORY TABLE (generated by scripts/check_bench_shapes.py --write) -->

Every deterministic Callgrind arm, and the ops count `perf_report.py` divides it by. Generated by `python3 scripts/check_bench_shapes.py --write`; do not hand-edit.

**`N` is keys, not engine calls.** It is the arm's operation count, which is what `Ins / Op` divides by. How often an operation reaches a given engine function is a separate question — for inserts, [ALGORITHMS.md](ALGORITHMS.md) §3.2.

#### [`crates/expanse-capi/benches/smoke_instructions.rs`](../crates/expanse-capi/benches/smoke_instructions.rs)
...
#### [`crates/expanse-capi/benches/vs_stock.rs`](../crates/expanse-capi/benches/vs_stock.rs)
...
| `cost` | `map_compact_drained` | `random60` | 62,500 |
| `cost` | `map_count_below` | `random`, `sequential`, `one_top_byte` | 50,000 |
| `cost` | `map_cursor_scan` | `random`, `sequential`, `clustered` | 50,000 |
...
| `cost` | `set_subtree_split` | `e1024` | 1,024 |
| `cost` | `set_subtree_split_control` | `e1024` | 1,024 |
...
| `cost` | `sync_map_branchu_thrash` | `top` | 200 |
...
| `cost` | `sync_map_drain_floor` | `top` | 200 |
| `cost` | `sync_map_get` | `random`, `leaf` | 50,000 |
| `cost` | `sync_map_insert` | `random` | 50,000 |
| `cost` | `sync_map_next_after_scan` | `random`, `sequential`, `clustered` | 50,000 |
...
#### [`crates/expanse/benches/leaf_cap_cascaded.rs`](../crates/expanse/benches/leaf_cap_cascaded.rs)

| Group | Benchmark | Arms | `N` per arm |
|---|---|---|---:|
| `cascaded` | `cascaded_map_get` | `hit`, `miss` | 1,000,000 |
| `cascaded` | `cascaded_set_contains` | `hit`, `miss` | 1,000,000 |

#### [`crates/expanse/benches/search_instructions.rs`](../crates/expanse/benches/search_instructions.rs)

| Group | Benchmark | Arms | `N` per arm |
|---|---|---|---:|
| `boolean` | `expanse_composed_and` | `dense`, `clustered`, `sparse`, `zipfian` | 1 |
| `boolean` | `expanse_native_and` | `dense`, `clustered`, `sparse`, `zipfian` | 1 |
| `boolean` | `roaring_and` | `dense`, `clustered`, `sparse`, `zipfian` | 1 |
| `boolean_andnot` | `expanse_composed_andnot` | `dense`, `clustered`, `sparse`, `zipfian` | 1 |
| `boolean_andnot` | `expanse_native_andnot` | `dense`, `clustered`, `sparse`, `zipfian` | 1 |
| `boolean_andnot` | `roaring_andnot` | `dense`, `clustered`, `sparse`, `zipfian` | 1 |
| `boolean_or` | `expanse_composed_or` | `dense`, `clustered`, `sparse`, `zipfian` | 1 |
| `boolean_or` | `expanse_native_or` | `dense`, `clustered`, `sparse`, `zipfian` | 1 |
| `boolean_or` | `roaring_or` | `dense`, `clustered`, `sparse`, `zipfian` | 1 |
| `materialize` | `expanse_materialize` | `dense`, `clustered`, `sparse`, `zipfian` | 1 |
| `materialize` | `roaring_materialize` | `dense`, `clustered`, `sparse`, `zipfian` | 1 |
| `wand` | `expanse_wand` | `dense`, `clustered`, `sparse` | 1 |
| `wand` | `roaring_wand` | `dense`, `clustered`, `sparse` | 1 |

#### [`crates/expanse/benches/smoke_instructions.rs`](../crates/expanse/benches/smoke_instructions.rs)
...
| Harness | Why its arms are not listed |
|---|---|
| [`crates/expanse-capi/examples/bench_vs_libjudy.rs`](../crates/expanse-capi/examples/bench_vs_libjudy.rs) | standalone driver: no declared arms |
| [`crates/expanse-hot-bench/src/bin/hot_concurrent.rs`](../crates/expanse-hot-bench/src/bin/hot_concurrent.rs) | FFI suite binary: arms are selected by CLI arguments |
| [`crates/expanse-hot-bench/src/bin/hot_latency.rs`](../crates/expanse-hot-bench/src/bin/hot_latency.rs) | FFI suite binary: arms are selected by CLI arguments |
| [`crates/expanse-hot-bench/src/bin/hot_memory_curve.rs`](../crates/expanse-hot-bench/src/bin/hot_memory_curve.rs) | FFI suite binary: arms are selected by CLI arguments |
| [`crates/expanse-hot-bench/src/bin/hot_string_latency.rs`](../crates/expanse-hot-bench/src/bin/hot_string_latency.rs) | FFI suite binary: arms are selected by CLI arguments |
| [`crates/expanse-hot-bench/src/bin/hot_string_memory.rs`](../crates/expanse-hot-bench/src/bin/hot_string_memory.rs) | FFI suite binary: arms are selected by CLI arguments |
| [`crates/expanse-hot-bench/src/bin/hot_string_validate.rs`](../crates/expanse-hot-bench/src/bin/hot_string_validate.rs) | FFI suite binary: arms are selected by CLI arguments |
| [`crates/expanse-hot-bench/src/bin/masstree_concurrent.rs`](../crates/expanse-hot-bench/src/bin/masstree_concurrent.rs) | FFI suite binary: arms are selected by CLI arguments |
| [`crates/expanse-hot-bench/src/bin/masstree_latency.rs`](../crates/expanse-hot-bench/src/bin/masstree_latency.rs) | FFI suite binary: arms are selected by CLI arguments |
| [`crates/expanse-hot-bench/src/bin/masstree_memory.rs`](../crates/expanse-hot-bench/src/bin/masstree_memory.rs) | FFI suite binary: arms are selected by CLI arguments |
| [`crates/expanse-hot-bench/src/bin/masstree_string_latency.rs`](../crates/expanse-hot-bench/src/bin/masstree_string_latency.rs) | FFI suite binary: arms are selected by CLI arguments |
| [`crates/expanse-hot-bench/src/bin/masstree_validate.rs`](../crates/expanse-hot-bench/src/bin/masstree_validate.rs) | FFI suite binary: arms are selected by CLI arguments |
| [`crates/expanse-wasm-fuel/src/lib.rs`](../crates/expanse-wasm-fuel/src/lib.rs) | wasm fuel instrument: each arm is a `#[no_mangle]` export that `scripts/wasm_fuel.py` instantiates under wasmtime and reads fuel back from, so the arm list is that script's, not an iai attribute's |
| [`crates/expanse-wasm/tests/bench.js`](../crates/expanse-wasm/tests/bench.js) | Node wall-clock harness: rows are JS objects built at run time, and the deterministic wasm instrument is the fuel crate above |
| [`crates/expanse/benches/art_insert.rs`](../crates/expanse/benches/art_insert.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/art_lookup_hit.rs`](../crates/expanse/benches/art_lookup_hit.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/art_lookup_miss.rs`](../crates/expanse/benches/art_lookup_miss.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/art_memory.rs`](../crates/expanse/benches/art_memory.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/art_scan.rs`](../crates/expanse/benches/art_scan.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/art_small_payload.rs`](../crates/expanse/benches/art_small_payload.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/avx512_bitmap.rs`](../crates/expanse/benches/avx512_bitmap.rs) | criterion: arm ids are built at run time |
| [`crates/expanse/benches/batch_lookup.rs`](../crates/expanse/benches/batch_lookup.rs) | criterion: arm ids are built at run time |
| [`crates/expanse/benches/bench_grammar_masks.rs`](../crates/expanse/benches/bench_grammar_masks.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/bench_llm_datastore.rs`](../crates/expanse/benches/bench_llm_datastore.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/comparative.rs`](../crates/expanse/benches/comparative.rs) | criterion: arm ids are built at run time |
| [`crates/expanse/benches/compare.rs`](../crates/expanse/benches/compare.rs) | criterion: arm ids are built at run time |
| [`crates/expanse/benches/concurrency.rs`](../crates/expanse/benches/concurrency.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/domain.rs`](../crates/expanse/benches/domain.rs) | criterion: arm ids are built at run time |
| [`crates/expanse/benches/embedded.rs`](../crates/expanse/benches/embedded.rs) | criterion: arm ids are built at run time |
| [`crates/expanse/benches/embedded_memtable.rs`](../crates/expanse/benches/embedded_memtable.rs) | criterion: arm ids are built at run time |
| [`crates/expanse/benches/hashbrown_container_dists.rs`](../crates/expanse/benches/hashbrown_container_dists.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/hashbrown_memory_alloc.rs`](../crates/expanse/benches/hashbrown_memory_alloc.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/hashbrown_native_suite.rs`](../crates/expanse/benches/hashbrown_native_suite.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/hashbrown_tail_latency.rs`](../crates/expanse/benches/hashbrown_tail_latency.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/hashbrown_ycsb.rs`](../crates/expanse/benches/hashbrown_ycsb.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/large_values.rs`](../crates/expanse/benches/large_values.rs) | criterion: arm ids are built at run time |
| [`crates/expanse/benches/leaf_cap_cascaded_wallclock.rs`](../crates/expanse/benches/leaf_cap_cascaded_wallclock.rs) | criterion: arm ids are built at run time |
| [`crates/expanse/benches/patricia_insert.rs`](../crates/expanse/benches/patricia_insert.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/patricia_lookup_hit.rs`](../crates/expanse/benches/patricia_lookup_hit.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/patricia_lookup_miss.rs`](../crates/expanse/benches/patricia_lookup_miss.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/patricia_memory.rs`](../crates/expanse/benches/patricia_memory.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/patricia_scan.rs`](../crates/expanse/benches/patricia_scan.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/patricia_string.rs`](../crates/expanse/benches/patricia_string.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/search_boolean.rs`](../crates/expanse/benches/search_boolean.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/search_memory.rs`](../crates/expanse/benches/search_memory.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/search_wand.rs`](../crates/expanse/benches/search_wand.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/ycsb.rs`](../crates/expanse/benches/ycsb.rs) | criterion: arm ids are built at run time |
| [`crates/expanse/benches/ycsb_dense.rs`](../crates/expanse/benches/ycsb_dense.rs) | criterion: arm ids are built at run time |
| [`crates/expanse/benches/zset_memory.rs`](../crates/expanse/benches/zset_memory.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/zset_range.rs`](../crates/expanse/benches/zset_range.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/zset_rank.rs`](../crates/expanse/benches/zset_rank.rs) | standalone driver: no declared arms |
| [`crates/expanse/benches/zset_zadd.rs`](../crates/expanse/benches/zset_zadd.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/allocator_overhead.rs`](../crates/expanse/examples/allocator_overhead.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/avx512_probe.rs`](../crates/expanse/examples/avx512_probe.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/bench_lookup_compare.rs`](../crates/expanse/examples/bench_lookup_compare.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/bytes_per_key.rs`](../crates/expanse/examples/bytes_per_key.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/bytes_per_key_32.rs`](../crates/expanse/examples/bytes_per_key_32.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/concurrent_scaling.rs`](../crates/expanse/examples/concurrent_scaling.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/fallback_maturity.rs`](../crates/expanse/examples/fallback_maturity.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/keyspace_density.rs`](../crates/expanse/examples/keyspace_density.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/line_transfer.rs`](../crates/expanse/examples/line_transfer.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/lookup_profile.rs`](../crates/expanse/examples/lookup_profile.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/occ_stats_probe.rs`](../crates/expanse/examples/occ_stats_probe.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/perf_point_lookup.rs`](../crates/expanse/examples/perf_point_lookup.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/popcnt_probe.rs`](../crates/expanse/examples/popcnt_probe.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/remove_retention.rs`](../crates/expanse/examples/remove_retention.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/strmap_drain_retention.rs`](../crates/expanse/examples/strmap_drain_retention.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/value_compression_eval.rs`](../crates/expanse/examples/value_compression_eval.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/writer_scaling.rs`](../crates/expanse/examples/writer_scaling.rs) | standalone driver: no declared arms |
| [`crates/expanse/examples/ycsb_concurrent.rs`](../crates/expanse/examples/ycsb_concurrent.rs) | standalone driver: no declared arms |
| [`integrations/rocksdb/benches/bench_memtable.cc`](../integrations/rocksdb/benches/bench_memtable.cc) | standalone driver: no declared arms |
| [`integrations/rocksdb/benches/bench_memtable_concurrent.cc`](../integrations/rocksdb/benches/bench_memtable_concurrent.cc) | standalone driver: no declared arms |

<!-- END ARM INVENTORY TABLE -->
...
`shrink_to_fit()` gives back 4.31 B/key of `mem_held` on random keys but only 1.48 B/key of RSS. The released blocks become glibc free chunks: after `shrink_to_fit()` and `malloc_trim(0)`, glibc's free-chunk bytes (`mallinfo2().fordblks`) stand 4.61 B/key above the start of the run *(measured: same host, commit a4b03ad5; [`results/allocator_overhead_a4b03ad5.txt`](../results/allocator_overhead_a4b03ad5.txt), `free/shr` column; workload: `example_allocator_overhead`)*. So the memory is reusable by the rest of the process but stays resident, and `malloc_trim` hands the kernel only whole OS pages inside free chunks. Slab pages the tree could return whole are tracked in [#1108](https://github.com/orieg/expanse/issues/1108). What stays retained sits on pages that still hold a live node, and returning it means moving nodes, which `shrink_to_fit()` never does: the value-pointer contract (`docs/COMPAT.md`) keeps a slot pointer valid until the next structural mutation. The 64-bit `ExpanseSet::compact()` / `ExpanseMap::compact()` do move nodes, under their own contract (every value pointer invalidated, O(n), a transient peak of old plus new); their measurements are in [`benchmarks/remove_retention/README.md`](benchmarks/remove_retention/README.md) §4. `packed-suffix` takes UUID keys from 87.14 to 70.64 B/key RSS (−18.9%) and costs the prefix shape 2.14 B/key (+4.5%): its suffix allocations are mostly 19 and 40 bytes, sizes glibc already rounds as tightly as a class does, and the feature raises that shape's engine retention (`mem_held − mem_used`) from 1.64 to 3.64 B/key. It stays off by default for that reason; turn it on for long, high-entropy string keys. Size classes at 8-byte granularity, which would also help the short suffixes, are tracked in [#1109](https://github.com/orieg/expanse/issues/1109).

**A drained map returns its memory on `shrink_to_fit()` or `clear()`** *(measured: Intel Xeon E5-2697 v4, glibc 2.41, commit dabd13f8; artifact [`results/strmap_drain_retention_dabd13f8.txt`](../results/strmap_drain_retention_dabd13f8.txt); workload: `example_strmap_drain_retention`)*. `examples/strmap_drain_retention.rs` builds 1,000 groups of R 17-byte keys and drains them group by group: an ordered scan of the group, then its keys removed in order. The drained map reports `mem_used()` 0 and still holds 262, 352, 1,548 and 12,165 B/group for R = 10, 100, 1,000 and 10,000 — the counted live heap equals `mem_held()` in every variant, with or without a second `ExpanseMap` and scan buffers beside it, so the retention is the engine's, not the allocator's. RSS after `malloc_trim(0)` stands at 905, 1,139, 3,564 and 24,973 B/group; at R = 10,000 about half of that is the retained pages themselves and the rest is free memory glibc cannot return while those pages sit between it (derived: 24,973 − 12,165). One `shrink_to_fit()` after the drain brings RSS to 610, 565, 586 and 639 B/group, the same as dropping the map; that remainder is present after the map is dropped too, so it is the allocator's, not the map's. All three map types keep a drained tree's blocks for its next insert and return them on `shrink_to_fit()` or `clear()`. Returning them automatically when `remove` takes the last key was measured and not adopted: for the integer maps it cost +0.25% on `map_remove` and +0.19% on `sync_set_remove` without attribution *(measured: `instruction-counts` CI job, run [35954949332](https://github.com/orieg/expanse/actions/runs/35954949332) with the release against run [35954172250](https://github.com/orieg/expanse/actions/runs/35954172250) without it)*, and for the string map, which shipped it in 0.8.1, it made a map that repeatedly empties release and re-carve its pages every cycle: +2,015% on `strmap_oscillate`, +4.00% on `strmap_refill` and +16.5% on `strmap_refill_small` against the same tree without it, and keeping up to eight slab pages instead still cost +44.9%, +1.35% and +6.58%, above the bounds fixed before measuring (5%, 1%, 1%) *(measured: `instruction-counts` CI job, deterministic Callgrind counts on each head: run [36005265036](https://github.com/orieg/expanse/actions/runs/36005265036/job/107652724138) (its Callgrind job; the run itself was cancelled after the job completed) with the release, run [36005344509](https://github.com/orieg/expanse/actions/runs/36005344509) without it, run [36005348087](https://github.com/orieg/expanse/actions/runs/36005348087) with the eight-page variant)*. `tests/test_strmap_drain_release.rs` and `tests/test_intmap_drain_release.rs` pin the behaviour on counted live bytes.
...
*(measured: reference host — Intel i9-12900F, pin `0-15`, governor `powersave`;
commit `76432c5c`; two dispatches of the `concurrency` suite, runs [34881026495](https://github.com/orieg/expanse/actions/runs/34881026495) and
[34882381735](https://github.com/orieg/expanse/actions/runs/34882381735); threads 1, 4 and 16, 18 interleaved rounds of 500 ms windows per
cell, largest foreign busy CPUs over a group 0.04 in both runs;
`docs/benchmarks/concurrency/results/baseline_concurrent_mixed.json` and
`results/baseline_concurrent_mixed_run2.json`)* (workload: `core_concurrency`)

`docs/benchmarks/concurrency/scripts/mixed_concurrency.py` runs the bench as an
instrument: one prefilled structure per engine and workload, whole Williams
cycles of rounds with one window per thread count, each window started at a
barrier after its threads' setup and divided by its own elapsed time. A cell is
the mean over its windows with a BCa 95% interval; C(16) = T(16) / T(1) is
paired within each round, T being read plus write operations per second. Every
cell of both runs, the four-thread counts included, is in
[`docs/benchmarks/concurrency/README.md` §12](benchmarks/concurrency/README.md).
This section's previous figures came from one window per cell that started
before its threads were spawned and divided by the window constant; they are
withdrawn, not comparable with these, and listed in
`.github/superseded-figures.json`.

The same driver's `--step2-gate` mode is the two-build instrument that
[`docs/benchmarks/concurrency/METHODOLOGY.md` §13](benchmarks/concurrency/METHODOLOGY.md)
pre-registers for #568 Step 2 (the `mixed_concurrency_step2_gate` suite). It
builds the head and `1edfa952`, the latter with the head's
...
through a populated tree rather than a miss walk. The prefill inserts 1,000,000
draws over 2,000,000 keys, so at 100% read about 39% of probes hit (derived:
1 − e^(−1/2)); under a mix, writes insert even keys and remove odd ones, which
moves occupancy toward half. Every thread picks a read or a write per operation,
so under a mix the total is a mixed-operation rate, not read scaling.

**`SyncExpanseMap32` arm — the 32-bit protocol-health instrument.** The
single-writer / many-reader `sync32` wrapper has no lock to contend on, so
throughput alone cannot say whether the optimistic path is healthy: a read path
that degrades to permanent `Busy` still "scales". The arm therefore fixes the
roles by protocol — one writer inserting and removing over the upper 4,096 keys
of an 8,192-key space, `N` readers probing all of it with `try_get` after a
prefill of 4,096 draws — and sweeps the **writer's duty** instead of a read
percentage: full duty (the Busy ceiling, not a health number) and 1M, 100k and
10k mutations/s, deadline-paced (a saturated CAN bus is ~10k frames/s). It
reports the **Busy rate** — the share of read attempts abandoned to an open
write bracket — and the writer's **refused** mutations (`ArenaFull` /
`ReclaimBacklog`, the signal of a reader stalling reclamation). The collapse to
watch for is a Busy rate approaching 100% at a paced duty, not a throughput
number.

![sync32 protocol health: Busy rate per writer duty and reader count](./assets/bench_sync32_health.svg)

*(Drawn by `scripts/generate_asset_svgs.py` from the `sync32_health` block of
`docs/assets/data/bench_assets.json`, which `mixed_concurrency.py --write-assets`
writes from the two artifacts and CI's `lint` job checks against them with
`--check-assets` — no value in the chart is typed by hand.)*

| writer duty | readers | validated reads M/s, run 1 [BCa 95%] | writes M/s, run 1 | Busy rate, run 1 | run 2 | refused writes, runs 1 + 2 |
|---|--:|---|---|--:|--:|--:|
| full | 1 | 0.143 [0.129, 0.159] | 12.0 | 99.5% | 99.4% | 0 |
| full | 4 | 1.09 [0.98, 1.22] | 7.49 | 98.2% | 98.2% | 0 |
| full | 16 | 15.7 [14.5, 17.5] | 3.42 | 90.6% | 86.8% | 0 |
| 1M/s | 1 | 36.9 [36.7, 37.1] | 1.00 | 19.3% | 20.9% | 0 |
| 1M/s | 4 | 129 [127, 130] | 1.00 | 18.3% | 19.4% | 0 |
| 1M/s | 16 | 321 [299, 326] | 1.00 | 15.6% | 21.3% | 0 |
| 100k/s | 1 | 49.7 [49.7, 49.8] | 0.100 | 1.81% | 1.85% | 0 |
| 100k/s | 4 | 193 [193, 193] | 0.100 | 1.4% | 1.49% | 0 |
| 100k/s | 16 | 465 [457, 470] | 0.100 | 2.21% | 1.05% | 0 |
| 10k/s | 1 | 51.5 [51.4, 51.7] | 0.010 | 0.159% | 0.156% | 0 |
| 10k/s | 4 | 201 [201, 201] | 0.010 | 0.119% | 0.119% | 0 |
| 10k/s | 16 | 484 [481, 487] | 0.010 | 0.114% | 0.113% | 0 |

Refusals are zero at every duty and reader count in both runs. At the 10k/s duty
the Busy rate is 0.113%–0.159% across both runs, so reads validate at least
99.8% of the time. The paced 1M/s and 100k/s Busy rates move between the
runs — 15.6% and 21.3% with sixteen readers at 1M/s, 2.21% and
1.05% at 100k/s — so those are a band, not a level (rule 18). Before
[#594](https://github.com/orieg/expanse/issues/594) the sixteen-reader rows
refused mutations at full duty and at the 1M/s and 100k/s paces (run
[33556060582](https://github.com/orieg/expanse/actions/runs/33556060582),
measured with the previous single-window harness).

**100% read** (read + write M ops/s):

| arm | keys → values | 1 thread, run 1 | 16 threads, run 1 [BCa 95%] | run 2 | C(16), run 1 [paired BCa 95%] | run 2 |
|---|---|---:|---|---|---|---|
| `SyncExpanseMap` | u64 → u64, 1M draws | 38.2 | 361 [360, 361] | 361 [360, 361] | 9.44 [9.43, 9.46] | 9.44 [9.43, 9.46] |
| `SyncExpanseSet` | u64, 1M draws | 76.4 | 562 [554, 570] | 415 [333, 493] | 7.36 [7.25, 7.46] | 5.44 [4.37, 6.47] |
| `SyncExpanseBlobMap` | u64 → 128-byte payload, 200k draws | 31.0 | 306 [305, 307] | 307 [306, 307] | 9.86 [9.84, 9.88] | 9.87 [9.84, 9.89] |
| `SkipMap<u64, Vec<u8>>` | u64 → 128-byte payload, 200k draws | 3.46 | 38.3 [38.2, 38.4] | 38.4 [38.3, 38.5] | 11.05 [11.01, 11.08] | 11.10 [11.06, 11.14] |
| `RwLock<BTreeMap<u64, (Vec<u8>, u32)>>` | u64 → 128-byte payload, 200k draws | 10.5 | 15.3 [15.2, 15.3] | 16.0 [15.9, 16.0] | 1.45 [1.45, 1.46] | 1.52 [1.52, 1.52] |
| `Mutex<ExpanseBlobMap>` | u64 → 128-byte payload, 200k draws | 41.2 | 5.41 [5.36, 5.45] | 5.27 [5.21, 5.33] | 0.13 [0.13, 0.13] | 0.13 [0.13, 0.13] |
| `SyncExpanseBytesMap` | 37-byte string → u64, 100k draws | 11.4 | 126 [126, 127] | 126 [125, 126] | 11.08 [11.05, 11.11] | 11.03 [10.99, 11.06] |
| `SyncExpanseStrMap` | 37-byte string → u64, 100k draws | 6.89 | 73.3 [71.8, 74.6] | 75.6 [74.1, 76.0] | 10.63 [10.42, 10.82] | 10.99 [10.79, 11.06] |
| `DashMap<Vec<u8>, u64>` | 37-byte string → u64, 100k draws | 15.4 | 130 [130, 130] | 129 [129, 130] | 8.46 [8.44, 8.48] | 8.39 [8.36, 8.45] |
| `Mutex<ExpanseBytesMap>` | 37-byte string → u64, 100k draws | 12.1 | 3.39 [3.37, 3.41] | 3.49 [3.47, 3.51] | 0.28 [0.28, 0.28] | 0.29 [0.29, 0.29] |
| `Mutex<ExpanseStrMap>` | 37-byte string → u64, 100k draws | 8.88 | 2.68 [2.67, 2.69] | 2.67 [2.66, 2.68] | 0.30 [0.30, 0.30] | 0.30 [0.30, 0.30] |

**50% read / 50% write** (read + write M ops/s):

| arm | keys → values | 1 thread, run 1 | 16 threads, run 1 [BCa 95%] | run 2 | C(16), run 1 [paired BCa 95%] | run 2 |
|---|---|---:|---|---|---|---|
| `SyncExpanseMap` | u64 → u64, 1M draws | 28.0 | 62.5 [53.7, 67.9] | 62.2 [54.6, 66.9] | 2.23 [1.92, 2.43] | 2.23 [1.96, 2.40] |
| `SyncExpanseSet` | u64, 1M draws | 42.9 | 82.9 [76.5, 88.9] | 84.8 [77.4, 91.1] | 1.93 [1.78, 2.07] | 1.98 [1.81, 2.12] |
| `SyncExpanseBlobMap` | u64 → 128-byte payload, 200k draws | 27.2 | 7.31 [6.68, 7.63] | 7.21 [6.60, 7.52] | 0.27 [0.26, 0.28] | 0.27 [0.25, 0.28] |
| `SkipMap<u64, Vec<u8>>` | u64 → 128-byte payload, 200k draws | 2.05 | 17.5 [17.2, 17.6] | 17.5 [17.3, 17.7] | 8.54 [8.31, 8.69] | 8.54 [8.34, 8.69] |
| `RwLock<BTreeMap<u64, (Vec<u8>, u32)>>` | u64 → 128-byte payload, 200k draws | 8.50 | 3.64 [3.63, 3.64] | 3.55 [3.54, 3.55] | 0.43 [0.43, 0.43] | 0.42 [0.42, 0.42] |
| `Mutex<ExpanseBlobMap>` | u64 → 128-byte payload, 200k draws | 35.2 | 5.08 [4.73, 5.27] | 4.89 [4.53, 5.07] | 0.15 [0.14, 0.15] | 0.14 [0.14, 0.15] |
| `SyncExpanseBytesMap` | 37-byte string → u64, 100k draws | 4.81 | 3.12 [3.10, 3.14] | 3.11 [3.10, 3.14] | 0.65 [0.63, 0.66] | 0.65 [0.63, 0.66] |
| `SyncExpanseStrMap` | 37-byte string → u64, 100k draws | 6.21 | 4.06 [4.05, 4.07] | 4.03 [4.02, 4.03] | 0.65 [0.65, 0.66] | 0.65 [0.65, 0.65] |
| `DashMap<Vec<u8>, u64>` | 37-byte string → u64, 100k draws | 10.7 | 83.6 [82.6, 85.0] | 83.6 [82.5, 85.1] | 7.80 [7.65, 7.95] | 7.77 [7.60, 7.90] |
| `Mutex<ExpanseBytesMap>` | 37-byte string → u64, 100k draws | 6.25 | 2.52 [2.51, 2.52] | 2.86 [2.86, 2.87] | 0.40 [0.39, 0.41] | 0.46 [0.45, 0.46] |
| `Mutex<ExpanseStrMap>` | 37-byte string → u64, 100k draws | 7.50 | 2.31 [2.31, 2.31] | 2.36 [2.35, 2.36] | 0.31 [0.31, 0.31] | 0.31 [0.31, 0.31] |

**What both runs show.** The arms fall into three key types, and a comparison holds only within one: u64 → u64 over 1M draws (`SyncExpanseMap`, `SyncExpanseSet`, with no third-party arm), u64 → 128-byte payload over 200k (`SyncExpanseBlobMap`, `Mutex<ExpanseBlobMap>`, `RwLock<BTreeMap>`, `SkipMap`), and 37-byte string keys → u64 over 100k (`SyncExpanseStrMap`, `SyncExpanseBytesMap`, their `Mutex` twins, `DashMap`). At 100% read every `Sync*` arm scales to sixteen threads and every `Mutex<Expanse*>` arm falls below its own one-thread throughput: `SyncExpanseMap` reaches C(16) = 9.44 [9.43, 9.46] in both runs; `SyncExpanseBlobMap` 9.86–9.87 against `SkipMap`'s 11.05–11.10, at 306–307 against 38.3–38.4 M ops/s; `SyncExpanseStrMap` 10.63–10.99 and `SyncExpanseBytesMap` 11.03–11.08 against `DashMap`'s 8.39–8.46, which serves the higher level (129–130 against 126 M ops/s for `SyncExpanseBytesMap`); the mutex arms 0.13–0.30 (workload: `core_concurrency`). At 50/50 the two arms that admit concurrent writers scale — `SyncExpanseMap` C(16) = 2.23 [1.92, 2.43] in run 1 and `SyncExpanseSet` 1.93 [1.78, 2.07], with run 2's estimates inside both intervals — and the three that serialise writers lose throughput as threads are added while the concurrent-write structures of their key types scale: `SyncExpanseBlobMap` 0.27 against `SkipMap`'s 8.54, `SyncExpanseStrMap` and `SyncExpanseBytesMap` 0.65 against `DashMap`'s 7.77–7.80. `SyncExpanseSet` and `SyncExpanseMap` take writers through optimistic lock coupling on per-node version words (`docs/ARCHITECTURE.md` §4.2); the blob, string and bytes wrappers serialise writers on one mutex. The integer arms have no baseline of their key type in this harness, and what limits their 50/50 scaling is unmeasured.

**What they do not.** `SyncExpanseSet` at 100% read does not reproduce between
the runs: sixteen threads measured 562 M ops/s in run 1 and
415 in run 2, C(16) = 7.36 [7.25, 7.46] against 5.44 [4.37, 6.47]. Run 2 holds one
contiguous episode: in rounds 3–9 every sixteen-thread window
measured 183–201 M ops/s against 541–567 M in its other rounds, and
4 of those rounds' four-thread windows 87–94 M against 287–292 M,
while every one-thread window stayed at 76 M; the group's foreign busy-CPU
delta was 0.01. The cause is unmeasured, so the cell's level is not
claimed. The `sync32` full-duty cell is heavy-tailed in run 2 (3 of its
18 sixteen-reader windows at 59–127 M ops/s against
13–24 M). Across every cell, 22 of 78 throughput intervals do
not overlap between the runs: the `SyncExpanseSet` 100%-read cells move by up to
26%, and the other 19 by at most 13.8%, the
between-run spread rule 18 describes
(`results/baseline_concurrent_mixed.json`).

**Mechanism.** The Step 0 counters describe the single-writer engine: with one
writer and eight readers the readers spent 53–60% of their time spinning on the
odd tree-level version (P0.1 `CONFIRMED` on every integer cell, both runs), the
writer's RFO misses per insert rose ~6× on flat instructions (P0.2
`CONFIRMED`), and `perf c2c` placed about half of the contended-line samples on
the line holding the tree version and the writer mutex *(measured: reference
host, tree at `a1982ff2`; every table generated from the artifacts in
`docs/benchmarks/concurrency/`)*. Multi-writer optimistic lock coupling on
`SyncExpanseSet` and `SyncExpanseMap` followed (`docs/ARCHITECTURE.md` §4.2); its
gates are pre-registered in `docs/benchmarks/concurrency/METHODOLOGY.md` §10 and
evaluated in that suite's README §9, and no counter here attributes the 50/50
scaling above to it. Single-threaded trees skip the version brackets entirely
...
- **`bytes/key` on random keys is density-dependent, and not monotonically.** The
  same `ExpanseSet` measures 7.08–21.02 B/key under expanse occupancy alone,
  with a cascade at λ ≈ `LEAF_CAP` and a second one at λ ≈ 256 × `LEAF_CAP`;
  keyspace width and population are the same parameter (halving the keyspace
  is doubling N). No `random` memory cell is comparable to another without
  its λ, and the committed 1M cell sits in the first trough
  ([`ARCHITECTURE.md` §3.5](ARCHITECTURE.md#35-per-key-memory-is-a-sawtooth-in-expanse-occupancy-and-leaf_cap-sets-the-tooth);
  workload: `example_keyspace_density`).
- **Allocator-held bytes depend on insertion order; `mem_used()` does not.** The
  same 1M @64 `ExpanseSet` holds 12.62 B/key from the C allocator when built
  in generator order and 8.14 when built from sorted keys, at 7.92 B/key
  `mem_used()` either way (measured at `86daaddf`, before #826 changed the
  capacity-class ladder; workload: `hot_instrument_bridge`). A census-instrument
  figure without its insertion order is under-specified.
...
### libexpanse vs stock libjudy, JudyL surface — dedicated quiet host *(measured: reference host — Intel i9-12900F, 24 threads, 30 MiB L3, Linux 6.8.0, commit `4c4e852a`, [run 33151981386](https://github.com/orieg/expanse/actions/runs/33151981386); harness `crates/expanse-capi/examples/bench_vs_libjudy.rs`, 15 paired rounds with arms interleaved per round, load average 0.16 at start; stock `libJudy.so.1` `dlopen`'d; workload: `capi_bench_vs_libjudy`)*

Wall-clock ns per operation, 1,000,000-key populations, `expanse_dl` (the like-for-like `dlopen`'d arm) over stock; `< 1.00` = libexpanse faster / smaller:

| dist | insert ours / stock (ratio [BCa 95% CI]) | get ours / stock (ratio [BCa 95% CI]) | B/key ours / stock (ratio) |
|---|---|---|---|
| **sequential 1M** | **12.2 / 22.4 ns (0.545× [0.544, 0.546])** | **12.4 / 14.2 ns (0.872× [0.869, 0.877])** | 8.56 / 8.32 (1.03×) |
| **random 1M** | **56.8 / 70.6 ns (0.804× [0.802, 0.807])** | 41.0 / 39.8 ns (**1.031× [1.024, 1.038]**) | 16.70 / 17.67 (**0.95×**) |
| **clustered 1M** | **20.1 / 21.5 ns (0.933× [0.932, 0.935])** | **13.1 / 14.4 ns (0.904× [0.902, 0.908])** | 8.61 / 9.32 (**0.92×**) |

Every ratio and interval is the `expanse_dl_over_stock` series of [`results/baseline_vs_libjudy.json`](../results/baseline_vs_libjudy.json) (BCa over the 15 per-round paired ratios); the ns and B/key figures are the `expanse_dl` and `stock` rows of the same run's step log, since the artifact holds ratios only. B/key is deterministic allocator accounting (`JudyLMemUsed`), so it carries no interval. The ns columns are per-arm medians and their quotient need not equal the paired ratio, which is the figure to cite.

Honest reading: libexpanse wins insert across all three distributions (0.545×–0.933×) and wins sequential/clustered lookup (0.872× / 0.904×). It is **slower on random 1M lookup, 1.031× [1.024, 1.038]**; the pre-repair harness recorded a larger deficit here, which is superseded. Why random costs more than sequential is **unmeasured**: no counter run covers this arm, and the two mechanisms with emitted-code evidence are tracked in [#455](https://github.com/orieg/expanse/issues/455). It is the engine's weak arm on this surface. On the `x86-64-v1` CI runner above, without the SIMD leaf/branch kernels, the same operations are slower than stock. The swing between the two tables is the microarchitecture tier, not a code change (per-routine v1→v3 instruction deltas span −1.9% to −42.6% — [`docs/visualizer_data.json`](visualizer_data.json)). The 100k-population rows are in [Measured: libexpanse vs stock libjudy](#measured-libexpanse-vs-stock-libjudy).

The sequential and random rows previously carried figures from commit `43b46f38` (5-round medians, no intervals) that the repaired harness does not reproduce — sequential lookup was published at 0.68× against 0.872× [0.869, 0.877] here. Those figures are superseded and registered in `.github/superseded-figures.json`.
...
*(Measured: reference host — Intel i9-12900F, 24 threads, 30 MiB L3, Ubuntu 22.04 / kernel 6.8, commit `21a382f3`, runs [35419627251](https://github.com/orieg/expanse/actions/runs/35419627251) and [35419631571](https://github.com/orieg/expanse/actions/runs/35419631571), pinned CPUs `0,2,4,6,8,10,12,14`, load average ≤ 1.0; `benches/ycsb.rs` and `benches/ycsb_dense.rs`, seed `0x1234_5678_9ABC`, 8 paired rounds with BCa 95% bootstrap intervals and paired per-round ratios; artifact `results/baseline_ycsb.json`)*

> **Harness methodology note (remediated in #1005).**
> Previous publications of this suite carried three defects that are now resolved:
> 1. `ExpanseBlobMap` writes were rejected with `MetaOverflow` on 255 of every 256 writes due to unmasked 64-bit metadata against a 24-bit arena field; writes are now masked and checked.
...
**Fisher–Yates Shuffled Build Order:**

| Workload | `ExpanseMap (u64)` | `ExpanseBlobMap (128B)` | `BTreeMap (128B)` | `SkipMap (128B)` (RocksDB) |
|---|---:|---:|---:|---:|
| **A** (50R / 50U) | **38.73** [38.59, 38.81] | **17.94** [17.58, 18.49] | 14.04 [13.99, 14.08] | 3.09 [3.05, 3.11] |
| **B** (95R / 5U) | **47.41** [47.22, 47.50] | **30.27** [30.04, 30.54] | 15.63 [15.55, 15.69] | 5.15 [5.11, 5.17] |
| **C** (100% Read) | **48.94** [48.59, 49.15] | **31.84** [31.15, 32.26] | 15.86 [15.76, 15.94] | 5.68 [5.63, 5.72] |
| **D** (95R-Latest / 5I) | **65.83** [65.39, 66.09] | **34.79** [34.65, 34.98] | 17.79 [17.71, 17.90] | 7.29 [7.24, 7.31] |
| **E** (95% Scan / 5I) | 0.87 [0.87, 0.87] | 0.70 [0.70, 0.70] | **1.58** [1.57, 1.58] | 0.33 [0.33, 0.33] |
| **F** (50R / 50 RMW) | **33.45** [33.32, 33.51] | **15.48** [15.41, 15.54] | 15.05 [15.00, 15.09] | 2.75 [2.72, 2.76] |

**Sorted Ascending Build Order:**

| Workload | `ExpanseMap (u64)` | `ExpanseBlobMap (128B)` | `BTreeMap (128B)` | `SkipMap (128B)` (RocksDB) |
|---|---:|---:|---:|---:|
| **A** (50R / 50U) | **37.77** [35.67, 38.45] | **17.05** [16.77, 17.23] | 13.27 [12.94, 13.61] | 3.28 [3.24, 3.30] |
| **B** (95R / 5U) | **47.45** [47.26, 47.63] | **29.22** [28.90, 29.46] | 14.62 [14.29, 15.11] | 5.69 [5.56, 5.77] |
| **C** (100% Read) | **48.67** [48.04, 49.03] | **31.43** [30.54, 31.98] | 14.90 [14.57, 15.43] | 6.42 [6.27, 6.51] |
| **D** (95R-Latest / 5I) | **66.07** [65.68, 66.41] | **35.39** [34.91, 35.74] | 15.58 [15.20, 16.16] | 7.83 [7.62, 7.95] |
| **E** (95% Scan / 5I) | 0.94 [0.93, 0.94] | 0.78 [0.77, 0.78] | **1.87** [1.86, 1.87] | 0.40 [0.40, 0.40] |
| **F** (50R / 50 RMW) | **33.36** [33.19, 33.45] | **15.42** [15.27, 15.56] | 13.79 [13.51, 14.22] | 2.89 [2.86, 2.91] |

> **Workload E range scan behavior: sparse keys vs dense clustered inversion.**
> On uniform-random sparse keys (`workload_ycsb`), `BTreeMap` leads `ExpanseBlobMap` **2.26×** on shuffled (1.58 / 0.70 Mops/s; paired ratio 0.442 [0.441, 0.443]) and **2.40×** on sorted (1.87 / 0.78 Mops/s; paired ratio 0.416 [0.415, 0.417]). The cause is structural: uniform-random 64-bit keys yield ~1.43 records per leaf, requiring ~27.6 leaf transitions per 55-record scan where a B-tree walks contiguous leaf arrays.
>
> On dense clustered keys (`benches/ycsb_dense.rs`, workload: `workload_ycsb_dense`), dense clustering amortizes leaf transitions (~322 records per leaf, 0.003 transitions/record), **inverting the result in Expanse's favor**:
> - At $N=1\text{M}$ sorted: `ExpanseBlobMap` leads `BTreeMap` **1.027×** [1.026, 1.029] (2.197 vs 2.138 Mops/s; workload: `workload_ycsb_dense`).
> - At $N=10\text{M}$ sorted: `ExpanseBlobMap` leads `BTreeMap` **1.157×** [1.155, 1.158] (1.723 vs 1.490 Mops/s; workload: `workload_ycsb_dense`).
> - `ExpanseMap (u64)` leads `BTreeMap` across all dense populations (workload: `workload_ycsb_dense`): 4.88–4.90 Mops/s at 100k (1.22×–1.50×), 4.42–4.46 Mops/s at 1M (2.09×–2.89×), and 3.03–3.83 Mops/s at 10M (2.57×–2.98×).

#### Per-operation latency percentiles *(measured: reference host — Intel i9-12900F, commit `21a382f3`; `benches/ycsb.rs`, 200,000 recorded ops/workload across 8 rounds, 64-op window means)*

> **Latency methodology & refutation of historical 38 µs tail.**
> The table below reports percentiles of per-window mean ns/op (64 ops per `Instant` pair; calibrated bracket ~23.5 ns, residual overhead <0.37 ns/op).
> The historical p95–p99.9 figures of ~38–41 µs (e.g. 37,755 ns and 38,224 ns from commit `43b46f38`) **did not reproduce under the corrected harness**:
> under window-mean sampling and valid writes, `ExpanseBlobMap` p95 is **64.5 ns** on Workload A and **72.4 ns** on Workload F, with slow windows (>8× median) counting only ~7–8 out of 3,125 windows per round. The earlier unmeasured hypothesis that arena slab allocation caused 38 µs stalls is formally retracted.

| Workload | Engine | p50 | p95 | p99 | p99.9 | Slow Windows (>8× p50) | Mem (MB) |
|---|---|---:|---:|---:|---:|---:|---:|
| **A** (50R / 50U) | `ExpanseMap (u64)` | 25.9 ns | 28.2 ns | 29.3 ns | 51.3 ns | 0 / 3125 | 2.35 |
| | `ExpanseBlobMap (128B)` | 49.5 ns | 64.5 ns | 74.9 ns | 541.5 ns | 7.2 / 3125 | 30.35 |
| | `BTreeMap (128B)` | 70.7 ns | 77.3 ns | 81.4 ns | 107.1 ns | 0 / 3125 | 18.31 |
| | `SkipMap (128B)` | 323.3 ns | 369.8 ns | 389.7 ns | 428.8 ns | 0 / 3125 | 19.84 |
| **B** (95R / 5U) | `ExpanseMap (u64)` | 21.2 ns | 23.2 ns | 24.2 ns | 45.3 ns | 0 / 3125 | 2.35 |
| | `ExpanseBlobMap (128B)` | 28.3 ns | 40.5 ns | 48.5 ns | 68.2 ns | 1.5 / 3125 | 18.35 |
| | `BTreeMap (128B)` | 63.8 ns | 70.0 ns | 74.6 ns | 117.7 ns | 0 / 3125 | 18.31 |
| | `SkipMap (128B)` | 193.7 ns | 224.0 ns | 238.9 ns | 258.2 ns | 0 / 3125 | 19.84 |
| **C** (100% Read) | `ExpanseMap (u64)` | 20.5 ns | 22.5 ns | 23.5 ns | 83.2 ns | 0.6 / 3125 | 2.35 |
| | `ExpanseBlobMap (128B)` | 26.9 ns | 30.9 ns | 34.0 ns | 79.2 ns | 0.5 / 3125 | 16.35 |
| | `BTreeMap (128B)` | 62.7 ns | 69.4 ns | 74.1 ns | 108.6 ns | 0 / 3125 | 18.31 |
| | `SkipMap (128B)` | 176.0 ns | 198.9 ns | 213.4 ns | 252.5 ns | 0 / 3125 | 19.84 |
| **D** (95R-Latest / 5I) | `ExpanseMap (u64)` | 14.4 ns | 22.6 ns | 27.2 ns | 40.2 ns | 0.1 / 3125 | 2.44 |
| | `ExpanseBlobMap (128B)` | 22.6 ns | 35.7 ns | 45.5 ns | 68.2 ns | 1.5 / 3125 | 18.44 |
| | `BTreeMap (128B)` | 55.6 ns | 62.7 ns | 68.2 ns | 88.0 ns | 0 / 3125 | 20.13 |
| | `SkipMap (128B)` | 147.8 ns | 172.5 ns | 192.7 ns | 402.0 ns | 2.0 / 3125 | 21.80 |
| **E** (95% Scan / 5I) | `ExpanseMap (u64)` | 1,153.3 ns | 1,306.9 ns | 1,371.4 ns | 1,439.1 ns | 0 / 3125 | 2.44 |
| | `ExpanseBlobMap (128B)` | 1,428.3 ns | 1,614.7 ns | 1,697.5 ns | 1,810.6 ns | 0 / 3125 | 18.44 |
| | `BTreeMap (128B)` | 634.8 ns | 719.6 ns | 763.9 ns | 892.0 ns | 0 / 3125 | 20.13 |
| | `SkipMap (128B)` | 3,034.6 ns | 3,372.5 ns | 3,526.2 ns | 3,784.4 ns | 0 / 3125 | 21.80 |
| **F** (50R / 50 RMW) | `ExpanseMap (u64)` | 30.0 ns | 32.8 ns | 34.2 ns | 59.0 ns | 0 / 3125 | 2.35 |
| | `ExpanseBlobMap (128B)` | 56.6 ns | 72.4 ns | 82.2 ns | 549.1 ns | 7.6 / 3125 | 30.35 |
| | `BTreeMap (128B)` | 66.5 ns | 72.7 ns | 77.5 ns | 105.8 ns | 0 / 3125 | 18.31 |
| | `SkipMap (128B)` | 365.0 ns | 417.9 ns | 441.9 ns | 495.3 ns | 0 / 3125 | 19.84 |

---

### 3. Concurrent scaling under write churn: `SyncExpanseMap`

Measured in the concurrent-read-scaling section above (runs [34881026495](https://github.com/orieg/expanse/actions/runs/34881026495) and [34882381735](https://github.com/orieg/expanse/actions/runs/34882381735), commit `76432c5c`). The bare-metal sweep runs workloads 100 and 50, so 95/5 is not a published cell; the harness accepts it for local runs.

Under a 50/50 mix `SyncExpanseMap` goes 28.0 → 40.9 → 62.5 M read + write ops/s across 1→4→16 threads in run 1, C(16) = 2.23 [1.92, 2.43], and 2.23 [1.96, 2.40] in run 2 (`results/baseline_concurrent_mixed.json`; workload: `core_concurrency`). `SyncExpanseMap` admits concurrent writers through optimistic lock coupling (`docs/ARCHITECTURE.md` §4.2), and its u64 keys have no third-party arm in this harness. The single-writer blob, string and bytes arms lose throughput as threads are added, while the concurrent-write structures of their own key types scale: `SkipMap` to 8.54 against `SyncExpanseBlobMap`'s 0.27, `DashMap` to 7.77–7.80 against the string and bytes arms' 0.65. What limits the multi-writer arm is unmeasured.

**Architectural Takeaways (measured, reference host, 21a382f3):** the figures quoted are run 1's; run 2 (`results/baseline_ycsb_run2.json`) lands every one of the 288 paired ratios on the same side of 1.0.
1. **`ExpanseBlobMap` vs RocksDB `SkipMap`**: 4.77×–5.88× the throughput across Workloads A, B, C, D, F on shuffled keys (A: 5.808 [5.693, 5.958], B: 5.883 [5.830, 5.938], C: 5.604 [5.481, 5.703], D: 4.774 [4.755, 4.797], F: 5.634 [5.604, 5.694]) and 4.53×–5.33× on sorted keys (A: 5.203 [5.095, 5.308], B: 5.142 [5.066, 5.266], C: 4.899 [4.775, 5.049], D: 4.526 [4.414, 4.695], F: 5.331 [5.242, 5.413]; workload: `workload_ycsb`, BCa 95% intervals over paired rounds). The earlier "7.6×–11.0×" and "8.14×–11.14×" ranges are retracted.
2. **`ExpanseBlobMap` vs `BTreeMap` on Workload D**: 1.955× [1.937, 1.971] on shuffled keys (34.79 vs 17.79 Mops/s) and 2.275× [2.207, 2.331] on sorted keys (35.39 vs 15.58 Mops/s; workload: `workload_ycsb`, BCa 95% intervals over paired rounds). The earlier uncalibrated ~5.7× and 5.02× criterion medians on non-read-latest D with rejected writes are retracted.
3. **Pure Word Trie (`ExpanseMap`)**: 47.41 [47.22, 47.50] and 48.94 [48.59, 49.15] Mops/s on the read-heavy workloads B and C (shuffled), scaling to 65.83 [65.39, 66.09] Mops/s on D, with ~24.6 B/key memory footprint.
4. **Range-heavy workloads (E)**: On uniform-random sparse keys, `BTreeMap` leads `ExpanseBlobMap` 2.26× (shuffled ratio 0.442 [0.441, 0.443]); the cause is unmeasured. On dense clustered keys (`workload_ycsb_dense`), dense clustering reverses the result: at 1M sorted `ExpanseBlobMap` leads `BTreeMap` 1.027× [1.026, 1.029] (2.197 vs 2.138 Mops/s) and at 10M sorted 1.157× [1.155, 1.158] (1.723 vs 1.490 Mops/s), while `ExpanseMap (u64)` leads across all dense populations (1.22×–2.98×).
