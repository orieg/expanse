...
| [`crates/expanse/benches/concurrency.rs`](../crates/expanse/benches/concurrency.rs) | 1M (keyspace 2M); `SyncExpanseMap32` arm: 4,096 stable + 4,096 churn keys (keyspace 8k, 32-bit) | generator draw order — the population is inserted as drawn, neither sorted nor shuffled; prefill and the concurrent stream both draw from the bounded-keyspace PRNG | Continuous stream; `EXPANSE_BENCH_ROUNDS` interleaved 500 ms windows per thread count (Williams order), all on one prefilled structure per engine and workload | ~50% | Bounded keyspace random stream | `black_box(sink)` | Clean (`run_rounds`): a window starts at a barrier after thread creation and per-thread setup, and its rates divide by its own elapsed time | Symmetric across concurrency primitives | Tables: mean ops/sec over a thread count's windows; `EXPANSE_BENCH_SAMPLES` carries every window for BCa 95% intervals (`docs/benchmarks/concurrency/scripts/mixed_concurrency.py`) | **PENDING** `[unverified: re-measurement with the round and barrier window pending, #568]`: the #375 bounded-keyspace correction still holds. |
...
(0.14×–0.45×). The write-mixed collapse is hypothesised to have one cause: a
tree-level seqlock brackets whole operations for the root snapshot, so under
an active writer readers spin on the odd version or retry — a hypothesis,
