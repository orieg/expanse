...
**Architectural Takeaways (measured, reference host, 21a382f3):**
...
4. **Range-heavy workloads (E)**: On uniform-random sparse keys, `BTreeMap` leads `ExpanseBlobMap` 2.26× (shuffled ratio 0.442 [0.441, 0.443]) due to trie leaf transitions (~1.43 keys/leaf). On dense clustered keys (`workload_ycsb_dense`), dense clustering reverses the result: at 1M sorted `ExpanseBlobMap` leads `BTreeMap` 1.027× [1.026, 1.029] (2.197 vs 2.138 Mops/s) and at 10M sorted 1.157× [1.155, 1.158] (1.723 vs 1.490 Mops/s), while `ExpanseMap (u64)` leads across all dense populations (1.22×–2.98×).
