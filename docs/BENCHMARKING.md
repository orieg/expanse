...
| LLM inference & speculative decoding | [`llm_inference/`](benchmarks/llm_inference/README.md) | `bench_draft_quality`, `bench_datastore_scale`, `bench_llama_lookup`, `bench_prefix_lru` | HuggingFace PromptLookup (fixed 3-gram), CPython `dict`, `numpy.searchsorted`, `collections.OrderedDict`, stock `llama.cpp` | Draft quality: Expanse variable-length LSM wins alpha (+4.0% code, +17.6% JSON, +12.7% summary vs 3-gram); Datastore: 4.1–5.6× lower RAM, ingestion beats batched NumPy at scale (0.3x at 100k, 4.4x at 1M, 13.6x at 5M); C++ parity: 100.0% exact match; Prefix LRU: 9.5–10.1× lower RAM + 2.08M/s rank-eviction |
