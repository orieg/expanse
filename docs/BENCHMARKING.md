...
| [`crates/expanse/examples/remove_retention.rs`](../crates/expanse/examples/remove_retention.rs) | N = 3.2M removed to M = 1M at the headline cell (random @64, λ 48.8 → 15.3 keys per 2-byte expanse); random @64 also at 2M → 1M (λ 30.5 → 15.3), 4M → 1M (λ 61.0 → 15.3), 3.2M → 2M, 3.2M → 320k and 1M → 312.5k; random @62 and @56, sequential, sparse and clustered at 3.2M → 1M; the `model_pins` shapes hold at most 433 keys | generator — the population is inserted in draw order; removal order is the declared per-cell variable (`shuffled`, `sorted`, `range`) | N/A (Memory) | N/A — every removal hits a present key | N/A | `mem_used()` / `mem_held()` accounting, `ExpanseStats::node_bytes` per form, `NodeAlloc::census` per slab class, requested heap bytes from a counting global allocator | Clean: readings taken after the build, after the removals, after `shrink_to_fit()`, on a separate fresh build, and on the rebuilt tree after `clone()` and after the drained tree is dropped; the heap peak spans exactly the `clone()` call | The drained tree, the fresh build and the rebuilt tree hold the identical key set (asserted); `shuffled` and `sorted` remove the identical set | Exact byte and page counts; no interval (deterministic accounting) | Diagnostic census; the Step 0a gate reads the headline cell (`docs/benchmarks/remove_retention/METHODOLOGY.md`). |
