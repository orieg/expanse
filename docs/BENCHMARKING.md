...
| [`crates/expanse/examples/bytes_per_key.rs`](../crates/expanse/examples/bytes_per_key.rs) | 1k to 1M census; the `random` gate also samples 2M | N/A (Memory) | N/A | N/A | `mem_used()` accounting | Clean | Pure 64-bit census | Exact byte count | **PASS** `[verified: RUN (6c63826a)]`: Deterministic memory density census. |
...
| [`crates/expanse/examples/keyspace_density.rs`](../crates/expanse/examples/keyspace_density.rs) | 100k to 2.6M at 64 bits; 100k to 2M at 63 and 62 bits (plus exact-λ 27/40/58 cells); 2M at 58, 57 and 55 bits and 1.2M to 2.7M at 56 bits | N/A (Memory) | N/A | N/A | `mem_used()` accounting | Clean | Same PRNG and seed at every width; the width is the only variable | Exact byte count | **PASS** `[verified: RUN (fcca1c0d)]`: Deterministic density sweep; reproduces the §9.4 sweep of the HOT suite. |
