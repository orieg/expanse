...
| `writer_scaling_ablation_blob_writer_arenas` | wall-clock | Expanse multi-writer scaling diagnostic: default build vs ablation-blob-writer-arenas on the blob arm (Refs #929); outputs ablation_blob_writer_arenas_writer_scaling.json. |
...
- **Population Size**: $N = 100,000$ 64-bit keys in every published cell. The harness also runs 1M and 10M (`YCSB_POPULATIONS=100k,1m,10m`), recorded in every row; those cells are pending their first run (#1005).
- **Key Shape**: uniform-random `u64` keys (`benches/ycsb.rs`, workload `workload_ycsb`) in every published cell. A dense clustered shape — runs of 256 consecutive keys at random bases — is measured by `benches/ycsb_dense.rs` (workload `workload_ycsb_dense`); the two share every line of generator, runner and report code through `benches/ycsb_common/mod.rs`.
- **Insertion Order**: both. Every cell is built once sorted ascending and once Fisher–Yates shuffled, and the row says which; the operation stream is identical across the two.
...
> ⚠️ **Harness methodology disclosure — every cell and every ratio in this section is pending re-run (#1005).**
> The run above predates three harness corrections, each on the measured path (§8.7), and none of them cancels in a ratio by construction (§8.10.3):
>
> 1. **`ExpanseBlobMap` writes were rejected in workloads A, B, D and E.** The operation stream carries 64 random bits as the blob's hot metadata, the arena field is 24 bits, a wider value returns `MetaOverflow` before anything is written, and the runner discarded that `Result`. A uniform 32-bit value fits 24 bits with probability 2²⁴/2³² = 1/256, so 255 of every 256 `Update` and `Insert` ops on that arm were no-ops (verified at the run's head `6462c4ee`: `blobmap.rs:1096`, `benches/ycsb.rs:482`). Workload C has no writes and workload F's read-modify-write metadata stays in range, so those two `ExpanseBlobMap` cells are not affected by this item. The value is now masked and a rejected write panics; the defect is pinned by `test_blob_writes_need_the_metadata_mask_and_land_with_it` and was found by a per-cell work checksum now asserted equal across arms.
> 2. **Every arm's teardown was inside the timed region.** The criterion routine consumed the populated structure and did not return it, and `iter_batched` drops a consumed input before it stops the clock, so each sample also timed the drop of 100,000 entries. How much that added per arm is unmeasured; 100,000 boxed blobs and a slab arena are not freed the same way, so it is not assumed symmetric. Routines now return the structure.
> 3. **Workload D was not read-latest.** Reads were Zipfian over the end of the initial population and never reached a key the run had inserted. They are now Zipfian over recency, in-run inserts first.
>
> The harness also now builds every cell in both insertion orders (sorted and shuffled — `BTreeMap` is order-sensitive, §8.12.4), runs a dense clustered key shape beside this uniform-random one (`benches/ycsb_dense.rs`, workload `workload_ycsb_dense`), and runs at 100k, 1M and 10M. The figures below are criterion medians with no interval; the re-run publishes BCa 95% intervals and paired per-round ratios from `scripts/ycsb_bench.py --suite core`.
...
> `BTreeMap` leads `ExpanseMap` **1.55×** (1.289 / 0.830, `u64` values
> against 128 B blobs) and leads `ExpanseBlobMap` 1.96× (1.289 / 0.657,
> blob against blob); both are criterion medians with no interval,
> pending re-run (#1005). The earlier 4.33× and 3.08× figures — both in
> Expanse's favour — are withdrawn.
>
...
> range-scan result. That contrast comes from leaf-occupancy counts, not
> from a YCSB cell: no dense workload E has been measured by this suite.
> `benches/ycsb_dense.rs` is that cell, pending its first run (#1005).

#### Per-operation latency percentiles *(measured: reference host — Intel i9-12900F, 24 threads, 30 MiB L3, Ubuntu 22.04 / kernel 6.8, commit `43b46f38`; `benches/ycsb.rs` run with `YCSB_LATENCY_REPORT=1`, seed `0x1234_5678_9ABC`, 200,000 recorded ops/workload)* — **pending re-run (#1005)**

> ⚠️ **This table is stale and is pending re-run (#1005).** Its provenance commit `43b46f38` predates the [#385](https://github.com/orieg/expanse/pull/385) scan-bound change and the #470 payload-dereference change, both on the measured path (§8.7), and is not an ancestor of `main`. Its workload E rows cannot describe the current harness: a p50 of 50 ns for a scan that now averages ~55 records sits beside 0.830 Mops/s in the throughput table above, a mean of 1,000 / 0.830 ≈ 1,205 ns per op (workloads differ: the latency rows are the pre-#385 one-record scan, the throughput row is the post-#385 scan).
>
> The timing method is also replaced. Every op here was bracketed with `Instant::now()`/`elapsed()`, calibrated at ~26 ns per op on this host against p50 cells of 27–50 ns, so the low percentiles are mostly bracket. The harness now times windows of 64 ops with one `Instant` pair per window and reports percentiles of the per-window mean, printing the calibrated bracket cost and the residual per op (bracket / 64) on every run. A percentile of window means is not a per-op percentile — one stalled op moves its window's sample by 1/64 of the stall — and the report says so. No published percentile will come from a per-op bracket after the re-run.

| Workload | Engine | p50 | p95 | p99 | p99.9 | Mem (MB) |
|---|---|---:|---:|---:|---:|---:|
| **A** (50R / 50U) | `ExpanseMap (u64)` | 33 ns | 56 ns | 66 ns | 77 ns | 2.35 |
| | `ExpanseBlobMap (128B)` — tail cells pending re-run (#1005), cause unmeasured | 140 ns | 37,755 ns | 39,205 ns | 40,788 ns | 18.35 |
...
| | `ExpanseBlobMap (128B)` — tail cells pending re-run (#1005), cause unmeasured | 134 ns | 38,224 ns | 39,688 ns | 40,651 ns | 18.35 |
| | `BTreeMap (128B)` | 67 ns | 118 ns | 132 ns | 160 ns | 18.31 |
| | `SkipMap (128B)` | 331 ns | 752 ns | 1,433 ns | 2,129 ns | 19.84 |

**`ExpanseBlobMap` tail under write-heavy workloads (A, F) — cells pending re-run (#1005), cause unmeasured.** The table shows p50 ~140 ns with p95–p99.9 at ~38–41 µs on the two 50%-write mixes and nothing comparable on B, C, D or E. Two things are wrong with how that was published. First, the cause given for it — arena slab-chunk allocation or growth stalls — has no counter, allocator census or profile behind it; it was a hypothesis stated as a finding (§8.9.1), and it is withdrawn as an explanation: **the cause of this tail is unmeasured.** Second, the cells disagree with the throughput table: if 5% of ops take at least 37,755 ns, the mean is at least 0.05 × 37,755 ≈ 1,888 ns per op, which is at most ~0.53 Mops/s, against 20.38 Mops/s published for the same engine and workload above (workloads differ: `workload_ycsb` at commit `43b46f38` with a per-op bracket vs `workload_ycsb` at `6462c4ee` with criterion batches), and a ~26 ns bracket does not account for a 38× gap. The re-run reproduces the tail under window sampling, where the harness also counts the windows whose mean exceeds 8× the median, and then either attributes it with a named instrument or retracts the cells. Mem (MB) is `mem_used()` after the 200k-op stream, so the insert-bearing workloads (D, E) read slightly higher than the read-only steady state; the `BTreeMap` and `SkipMap` memory figures are hand-modelled estimates, not measurements.

---

### 3. Concurrent scaling under write churn: `SyncExpanseMap`

Measured in the concurrent-read-scaling section above (runs
[34881026495](https://github.com/orieg/expanse/actions/runs/34881026495) and
[34882381735](https://github.com/orieg/expanse/actions/runs/34882381735), commit
`76432c5c`). The bare-metal sweep runs workloads 100 and 50, so 95/5 is not a
published cell; the harness accepts it for local runs.
...
1. **`ExpanseBlobMap` vs RocksDB `SkipMap`**: 8.14×–11.14× the throughput across Workloads B, C, D, F, derived from the table above (B 21.21 / 2.54 = 8.35×, C 21.26 / 1.98 = 10.74×, D 21.50 / 1.93 = 11.14×, F 14.73 / 1.81 = 8.14×; workload: `workload_ycsb`). Criterion medians with no interval, and pending re-run (#1005) under the disclosure in §2. The "7.6×–11.0×" range published here before matched neither the table nor `docs/DATABASE.md` and is retracted as unsourced.
2. **`ExpanseBlobMap` vs `BTreeMap` on Workload D**: 5.02× the throughput, derived from the table above (21.50 / 4.28; workload: `workload_ycsb`). A criterion median with no interval, and pending re-run (#1005): the D cell was not read-latest and 255 of every 256 of its blob inserts were rejected (§2 disclosure). The "~5.7×" figure and the pair of throughputs published beside it matched no row of the table and are retracted as unsourced, and the clause attributing the lead to appends avoiding B-tree page splits is withdrawn — no counter or profile measured it (§8.9.1).
3. **Pure Word Trie (`ExpanseMap`)**: 23.27 and 23.61 Mops/s on the read-heavy workloads B and C in the table above (workload: `workload_ycsb`; criterion medians, pending re-run (#1005)), with a ~24.6 B/key footprint.
4. **Range-heavy workloads (E) are a measured loss on sparse keys** — `BTreeMap` leads `ExpanseMap` 1.55× (1.289 / 0.830; workload: `workload_ycsb`), a pair whose arms carry different payloads (`u64` values against 128 B blobs); blob against blob, `BTreeMap` leads `ExpanseBlobMap` 1.96× (1.289 / 0.657; workload: `workload_ycsb`). Both are criterion medians with no interval, pending re-run (#1005). The structural reading is the full-`iter()` gap's (`docs/DATABASE.md` §7.1): ~1.43 records per leaf on uniform-random keys means a scan pays a leaf transition roughly every 1.4 records. Whether dense keys reverse the result is not yet measured by this suite: `benches/ycsb_dense.rs` (workload `workload_ycsb_dense`) exists for that cell, and it is pending its first run (#1005).
