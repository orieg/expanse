...
8. **Masstree** ([`kohler/masstree-beta`](https://github.com/kohler/masstree-beta) `1119842`, C++ through FFI, both the single-threaded and the concurrent configuration of its template) — trie-of-B+-trees baseline and the second route to the write-concurrency loss ([#661](https://github.com/orieg/expanse/issues/661), [`docs/benchmarks/masstree_comparison/`](benchmarks/masstree_comparison/README.md)). Its memory census carries two instruments — the shared allocator interposition, quantized to Masstree's 2 MiB pool slab, and Masstree's own node census — and insertion order is published as a sensitivity dimension because a B+-tree's insertion cost, leaf fill and footprint depend on it, and so do the trie's insertion cost and allocator footprint; only the trie's own node census is order-invariant.
