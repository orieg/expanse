...
| [`crates/expanse/examples/keyspace_density.rs`](../crates/expanse/examples/keyspace_density.rs) | 100k to 2M, at 64-, 63- and 62-bit keyspaces | N/A (Memory) | N/A | N/A | `mem_used()` accounting | Clean | Same PRNG and seed at every width; the width is the only variable | Exact byte count | **PASS** `[verified: RUN (fcca1c0d)]`: Deterministic density sweep; reproduces the §9.4 sweep of the HOT suite. |
