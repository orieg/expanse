...
| Full libjudy + ART comparison | landed (`docs/benchmarks/art_comparison/`) | Pure-Rust ART (`blart` 0.5.0) comparison across point lookups, dynamic growth, range scans, and memory census (#387) |
...
| File | Population ($N$) | Probes & Reuse | Hit Rate | Miss Gen Method | Value Dereference | Measured Region | Arm Symmetry | Statistics | Verdict & Notes |
|---|---|---|---|---|---|---|---|---|---|
| [`crates/expanse/benches/art_insert.rs`](../crates/expanse/benches/art_insert.rs) | 10k to 1M | Insertion stream | Dynamic Growth | None | Insertion into container | Clean insertion loop | Symmetric keys and PRNG | Median of interleaved rounds | **PASS** `[verified: CODE READ]`: ART comparison insertion throughput benchmark. |
| [`crates/expanse/benches/art_lookup_hit.rs`](../crates/expanse/benches/art_lookup_hit.rs) | 10k to 1M | Batch stream | 100% Hit | None | `black_box(*val)` | Clean lookup loop | Symmetric keys and PRNG | Median of interleaved rounds | **PASS** `[verified: CODE READ]`: ART comparison point lookup hit benchmark. |
| [`crates/expanse/benches/art_lookup_miss.rs`](../crates/expanse/benches/art_lookup_miss.rs) | 10k to 1M | 50/50 mixed probe stream | 50% Hit / 50% Miss | Rejection sampling | `black_box(*val)` | Clean lookup loop | Symmetric keys and PRNG | Median of interleaved rounds | **PASS** `[verified: CODE READ]`: ART comparison point lookup 50/50 miss benchmark. |
| [`crates/expanse/benches/art_memory.rs`](../crates/expanse/benches/art_memory.rs) | 1k to 1M | N/A (Memory) | N/A | None | Live bytes tracked | Clean GlobalAlloc hook | Symmetric keys and PRNG | Exact byte count | **PASS** `[verified: CODE READ]`: ART comparison memory footprint benchmark. |
| [`crates/expanse/benches/art_scan.rs`](../crates/expanse/benches/art_scan.rs) | 10k to 1M | Range scan stream | Ordered Scan | None | `black_box(*val)` | Clean scan loop | Symmetric keys and PRNG | Median of interleaved rounds | **PASS** `[verified: CODE READ]`: ART comparison range scan benchmark. |
...
| [`crates/expanse/examples/bytes_per_key_32.rs`](../crates/expanse/examples/bytes_per_key_32.rs) | 10k | N/A (Memory) | N/A | N/A | `mem_used()` accounting | Clean | Pure 32-bit census | Exact byte count | **PASS** `[verified: RUN (27019b23)]`: Deterministic 32-bit memory census. |
...
**Run history (§8.7).** PR #599 measured Expanse alone at 64 and 160 MHz; PR #600 added the alternatives and the 400 MHz point; this run adds the Cortex-M4 and the dual-core cells and is the committed artifact for every table below. Each harness change moved the Expanse cells: +1% to +6% from #599 to #600 (vtable plus accounting), and +1.5% to +6% from #600 to this run (both-core build, MPU enabled; CAN dispatch 249 → 264, ingest 1,181 → 1,199). Earlier values stand in their PRs' transcripts and are superseded here, not overwritten.

| fixture (`embedded_memtable.rs` shape, via the C ABI) | 64 MHz off | 64 MHz on | 160 MHz off | 160 MHz on | 400 MHz off | 400 MHz on (ns) |
|---|---:|---:|---:|---:|---:|---:|
| ingest, 2,000 sequential inserts (per insert) | 1,762 | 1,176 | 2,249 | 1,199 | 2,249 | 1,199 (2,993 ns) |
...
|---|---:|---:|---:|---:|
| ingest, 2,000 sequential inserts | 1,199 / 2,993 ns | 186 / 464 ns (6.4× faster) | 71 / 177 ns (16.9× faster) | 27,786 / 69,379 ns (23.2× slower) |
...
|---|---:|---:|---:|---:|---:|
| ingest, 2,000 sequential inserts (per insert) | 1,199 (2,993 ns) | 2,249 | 5,074 (25,339 ns) | 4.2× | 8.5× |
