...
**An emptied string map holds what a new one does** *(measured: Intel Xeon E5-2697 v4, glibc 2.41, commits 7a46aaae and e3bde057; artifact [`results/strmap_drain_retention_e3bde057.txt`](../results/strmap_drain_retention_e3bde057.txt); workload: `example_strmap_drain_retention`)*. `examples/strmap_drain_retention.rs` builds 1,000 groups of R 17-byte keys and drains them group by group: an ordered scan of the group, then its keys removed in order. Before the fix, the drained map reported `mem_used()` 0 and still held 262, 352, 1,548 and 12,165 B/group for R = 10, 100, 1,000 and 10,000 — the counted live heap equals `mem_held()` in every variant, with or without a second `ExpanseMap` and scan buffers beside it, so the retention is the engine's, not the allocator's. RSS after `malloc_trim(0)` stood at 815, 1,155, 3,478 and 24,953 B/group; at R = 10,000 about half of that is the retained pages themselves and the rest is free memory glibc cannot return while those pages sit between it (derived: 24,953 − 12,165). `shrink_to_fit()` at that point brought RSS to 520, 561, 487 and 619 B/group, the same as dropping the map. The map now does that itself when `remove` takes its last key and on `clear`, as the C ABI already does when `JudySLDel` empties an array: drained, live heap and `mem_held()` read 0 and RSS 590, 475, 483 and 651 B/group. That remainder is present after the map is dropped too, so it is the allocator's, not the map's. After the fix, a second map beside it adds 69 B/group of its own live bytes and 115–352 B/group of RSS; scan buffers leave live bytes and `mem_held()` unchanged and move RSS by −98 to +107 B/group, within the spread of single readings. `ExpanseMap` and `ExpanseSet` return them on `clear` but keep them when drained by `remove`, where `shrink_to_fit()` returns them: a release on their last `remove` measured +0.25% on `map_remove` and +0.19% on `sync_set_remove` without attribution and was not adopted *(measured: `instruction-counts` CI job, run [35954949332](https://github.com/orieg/expanse/actions/runs/35954949332) with the release against run [35954172250](https://github.com/orieg/expanse/actions/runs/35954172250) without it; deterministic Callgrind counts)*. `tests/test_strmap_drain_release.rs` and `tests/test_intmap_drain_release.rs` pin both behaviours on counted live bytes.
