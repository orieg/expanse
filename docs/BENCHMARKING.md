...
| [`crates/expanse/examples/writer_scaling.rs`](../crates/expanse/examples/writer_scaling.rs) | prefill 2^20 keys (1M), plus 2^20 fresh keys inserted concurrently by W writers | generator draw order — prefill and fresh-key stream both draw from XorShift64 | none — pure writer scaling (R = 0), insert-only | n/a — no read probes | same-generator rejection sampling against prefill | map arms check stored values against key-derived expectation | barrier release to last-writer join; prefill and teardown outside | symmetric across thread counts; W in {1, 2, 4, 8} on physical P-cores | per-round throughput ops/sec emitted raw, median and BCa 95% CI across rounds | pending measurement |
