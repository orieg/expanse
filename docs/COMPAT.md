...
Status: **Judy1, JudyL, and JudySL families exported** with the shipped `Judy.h`; the gate G1 differential-oracle harness runs in CI against a dlopen'd stock libjudy for all three (randomized op sequences, full-sweep/rank agreement, byte-exact JudySL buffer sweeps). `JudyHS` is declared in the header and lands with `ExpanseBytesMap`. Next gate: G2 (php-judy suite against libexpanse — now unblocked).
