...
| **SVE / SVE2** VL-agnostic scan (AArch64) | additive fast leaf scan on Neoverse/Graviton | **Implemented; instruction effect unmeasured** — runtime detection via `sve_rt` (`is_aarch64_feature_detected!("sve")`) and inline asm (`whilelt`, `ld1b`/`ld1h`/`ld1w`, `cmpeq`/`cmphs`, `brkb`, `cntp`) with zero fn-call overhead; fallback to NEON/portable | absent on Apple Silicon / non-SVE cores. **No instruction delta has been recorded.** #434 merged before #437 made the AArch64 lane gate against the merge base, so its Callgrind run reported `N/A` on every arm. The next PR touching `rust-src` produces the delta; until it is recorded here this row is not a measured win. `HARDWARE.md` carries two SIMD paths that regressed when measured (§1.5 prefetch no-op; the +61% dense `BranchU` mask in [ALGORITHMS.md](ALGORITHMS.md)), so the label matters. |
