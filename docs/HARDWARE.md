...
- **SVE / SVE2 status:** `sve`, `sve2`, `sveaes`, `svesha3`, `svesm4`, `svei8mm`, `svebf16` are **exposed in `/proc/cpuinfo`**. Vector length is 128-bit on the Neoverse-N2 runner — the spec figure, not a census reading; the census does not measure VL. SVE leaf scanning (§6) is implemented with runtime detection via `std::arch::is_aarch64_feature_detected!("sve")` and `#[inline(always)]` inline assembly kernels (`whilelt`, `ld1b`/`ld1h`/`ld1w`, `cmpeq`/`cmphs`, `brkb`, `cntp`), falling back to NEON/portable on Apple Silicon and non-SVE cores. **Those kernels measured as a regression on the AArch64 lane (+7.7% to +9.9% on lookup arms)** — see §6 and #439.
...
*Verdict:* **CI's RV32 target is `riscv32imac-unknown-none-elf` (no Zbb)** and RV64 is
`riscv64gc` (no Zbb); the RFC never mentions the bit-count gap. → On the shipped 32-bit
config, **all 37 popcount/clz/ctz sites in the hot `trie32` path lower to software**.
Note: Zbb `clz`/`ctz` return XLEN on zero input, matching Rust's semantics — no
correctness issue, purely a codegen/perf gap.
...
| **SVE / SVE2** VL-agnostic scan (AArch64) | additive fast leaf scan on Neoverse/Graviton | **Implemented; measured as a REGRESSION on AArch64 (pending dual-pass confirmation)** — runtime detection via `sve_rt` (`is_aarch64_feature_detected!("sve")`) and inline asm (`whilelt`, `ld1b`/`ld1h`/`ld1w`, `cmpeq`/`cmphs`, `brkb`, `cntp`) with zero fn-call overhead; fallback to NEON/portable | absent on Apple Silicon / non-SVE cores. **`smoke_instructions` on the AArch64 lane regressed +7.7% to +9.9% across lookup arms** after #434 landed: `map_get/sequential` 981,056 → 1,071,228 (+9.19%), `map_get/random` 838,196 → 920,821 (+9.86%), `set_contains/random` 828,657 → 892,735 (+7.73%) *(measured: GitHub AArch64 runner Neoverse-N2, jobs 98711354227 → 98737273484)*. Only one of the four commits between those runs touches the lookup path, so attribution is `98d21083` (#434). These are separate runs rather than a dual-pass A/B — #437 now makes the lane gate against the merge base, so any PR touching `rust-src` yields the controlled number (#439). Likely mechanism: linear leaves are short by design (the ladder promotes to bitmap early), so `whilelt`/`brkb`/`cntp` setup is paid per scan over ≤16 elements and never amortised. This would be the third measured SIMD negative here, alongside §1.5's prefetch no-op and the +61% dense `BranchU` mask in [ALGORITHMS.md](ALGORITHMS.md). |
