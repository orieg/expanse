...
- **ThreadSanitizer in nightly CI** (`test-tsan` in `nightly.yml`): runs `cargo test -Zbuild-std --target x86_64-unknown-linux-gnu -p expanse-trie --lib sync:: --lib sync32::` and `cargo test -Zbuild-std --target x86_64-unknown-linux-gnu -p expanse-trie --test linearizability --test test_blobmap --test sync32_stress` with `RUSTFLAGS="-Zsanitizer=thread"` and `TSAN_OPTIONS="suppressions=.github/tsan-suppressions.txt"`. Suppressions are strictly isolated to underlying storage engine node/slot arrays (`map.rs`, `mutate_map.rs`, `bits.rs`, `slot.rs`, `blobmap.rs`, `strmap.rs`, `bytesmap.rs`, `trie32.rs`) where plain optimistic reader loads race benignly with in-place writes, while concurrency coordination logic (`sync.rs`, `sync32.rs`, `occ.rs`, `occ32.rs`, writer mutexes, EBR collector and epoch advancement) remains completely unsuppressed so genuine synchronization defects and data races are caught and alerted.
