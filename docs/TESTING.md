...
- **ThreadSanitizer in nightly CI** (`test-tsan` in `nightly.yml`): runs `cargo test -Zbuild-std --target x86_64-unknown-linux-gnu -p expanse-trie --lib sync::` and `cargo test -Zbuild-std --target x86_64-unknown-linux-gnu -p expanse-trie --test linearizability --test test_blobmap` with `RUSTFLAGS="-Zsanitizer=thread"` to catch data races in concurrent `Sync*` operations, multi-threaded linearizability checking, and blob arena concurrency with automated triage alerting.
