...
| 8 | Integrations & Sanitizers | `test_expanse_memtable` with ASan, UBSan, and TSan in CI; `test_memtable_park_points`, built with `-DEXPANSE_MEMTABLE_PARK_POINTS`, holds one thread at a named point on the read path while a writer runs, in the standard, ASan/UBSan and TSan lanes | Memory corruption, undefined behavior in arenas, and data races in concurrent leaf chaining; interleavings a random stress test reaches only by timing, forced deterministically, such as a split between a `Get`'s validation and its `next_leaf` load |
...
> **Scope caveat, documented in `sync` ([#1086](https://github.com/orieg/expanse/issues/1086)).** The `Sync*` wrappers used to reach two classes of undefined behaviour from safe code with two threads: plain optimistic loads racing a covered writer's plain stores (the seqlock pattern, a data race under the Rust memory model), and references over memory another thread was accessing, which Stacked and Tree Borrows reject. Both are closed by atomic shared accesses and raw-pointer writers ([ARCHITECTURE.md §4.2](ARCHITECTURE.md)), and no required lane observes either class: Miri covers `occ`/`alloc` and the single-threaded engine; loom covers the `occ` protocol primitives below plus the writer-gate entry and quiescence path through `sync`'s own `Shared::enter_writer` and `Shared::quiesce_writers`; the stress tests cover the composition; and the nightly **Miri UB-site census** below runs the threaded workloads under the race detector and both aliasing models, with every entry `clean`. A clean entry is an observation at the manifest's seed range, not a proof. What stays outside is a value stored through a slot that `with_locked_mut` hands the bytes or string map's slot APIs, which is a plain store, and the 32-bit wrappers (`sync32`), which have not been converted and still reach both classes.

The protocol under test is described in [ARCHITECTURE.md §4.1](ARCHITECTURE.md#41-concurrent-reads-occ--sync).

- **Deterministic invariant guards (`assert_bracketed()`)**: Structural invariants must never rely on sampled race timing. `NodeAlloc::assert_bracketed()` guards in-place linear leaf mutations, map immediate value shifts, and path-cache fast paths. In debug builds, any node interior mutation attempted while OCC is active without an enclosing version bracket panics immediately on the first operation, turning intermittent concurrency bugs into deterministic unit test failures (issues #477, #479). `NodeAlloc::enter_bookkeeping()` guards the other unsynchronized-by-design region — the per-tree size-class freelists and the slab-page list, whose updates are load/store pairs rather than CAS because every concurrent allocator defers to the collector's locked freelists first (`defer_to` refuses one that has already carved). Two threads inside it drop a slab page or a freelist link silently, and the page then surfaces as a LeakSanitizer report against whichever test the process happened to end on rather than against the caller that broke the invariant; in debug builds the guard panics at that caller instead. The guard cannot see the whole of the freelist pop, whose load sits outside the region it claims — two threads that read one head and then take the guard one after another never overlap inside it, and each returns the same block to its caller — so the pop also re-reads the head under the guard and fails when it has moved. A source census (`structural_every_freelist_mutation_claims_the_bookkeeping_guard`) pins both the three claim sites and the four stores they cover, so deleting a claim fails, and adding a fifth store fails until a reviewer states where its guard is.
- **Loom model checks** (`RUSTFLAGS="--cfg loom" cargo test -p expanse-trie --release loom_`; CI `loom` job): exhaustively explore the `occ` module's small state machines — the seqlock version protocol (a validated read is never torn — the same construction the per-node versions use), hand-over-hand nested version publication safety (`loom_hand_over_hand_node_bracket_safety`), the EBR pin/advance pairing (the epoch never advances twice past a live pin, so nothing retired under a pin is freed), and the writer gate — including `loom_shared_enter_writer_quiescence`, which drives `sync`'s own `Shared::enter_writer` and `Shared::quiesce_writers` with three writer threads over a real tree (`MAX_WRITER_SLOTS` is 2 under loom, so the table runs out and two writers share an in-flight word), and `loom_diag_entry_probe_excludes_and_drains` (the `diag-entry` feature's `optimistic_probe_on` — two probe threads, each on its own `ProbeCell`, against `Shared::quiesce_writers`; `docs/ARCHITECTURE.md` §4.2). Not advisory: loom's first run found two real ordering bugs — the seqlock needed Boehm's fence construction (release fence after the odd store, acquire fence before the validating re-read; MSPC 2012), and pin/advance is a store-buffer (Dekker) pair needing SeqCst fences between each side's store and load.
- **ThreadSanitizer in nightly CI** (`test-tsan` in `nightly.yml`): runs `cargo test -Zbuild-std --target x86_64-unknown-linux-gnu -p expanse-trie --lib sync::`, `cargo test -Zbuild-std --target x86_64-unknown-linux-gnu -p expanse-trie --lib sync32::`, and `cargo test -Zbuild-std --target x86_64-unknown-linux-gnu -p expanse-trie --test linearizability --test test_blobmap --test sync32_stress` with `RUSTFLAGS="-Zsanitizer=thread"` and `TSAN_OPTIONS="suppressions=.github/tsan-suppressions.txt"`. Because TSan suppresses a report when any frame of either thread's backtrace matches, and all trie/leaf mutations execute in the suppressed storage files (`map.rs`, `mutate_map.rs`, `bits.rs`, `slot.rs`, `blobmap.rs`, `strmap.rs`, `bytesmap.rs`, `trie32.rs`), storage data races—including unbracketed mutations racing readers (#477/#479)—are suppressed by their write-side frames. That defect class is guarded deterministically by `NodeAlloc::assert_bracketed()` debug panics and Loom model checks rather than TSan. TSan residual coverage catches data races where *both* stacks stay outside storage files (e.g. synchronization coordination, test harnesses, language bindings), guarded continuously against suppression rot by the committed `tests/tsan_canary.rs` negative control.
- **Miri UB-site census** (`scripts/miri_ub_sites.py`; nightly `miri-ub-sites` in `nightly.yml`): runs each workload in `sync::miri_ub_sites` — two writers, and a reader against a writer, for every wrapper and, for the map and set, in each root regime, plus the map's ordered reads (`first` / `next_after`, `last` / `prev_before`) against a writer in each root regime — in its own Miri process under three checks: `race` (the race detector alone), `alias-sb` and `alias-tb` (the aliasing model alone, Stacked or Tree Borrows). It compares Miri's report with `.github/miri-ub-sites.json`, where an expected site pins the error text and the innermost frame. A site that stops reproducing fails as a mismatch, so a fix flips its entry to `clean` by an explicit edit and a schedule change cannot; so does a different kind of UB, or UB where an entry records `clean`. The innermost frame is recorded but not gated: which access Miri catches follows its schedule, which any change to the interpreted code can move, so a moved frame is a `moved` notice that asks for the record to be refreshed. A run that neither reports UB nor passes is `could-not-check`, never clean. `map_covered_reader_writer` and `set_covered_reader_writer` force covered writes (tree-shrinking removals) under a concurrent reader; built with `occ-stats`, they print `read_cover_overlaps` (`occ_stats::Stat::ReadCoverOverlaps`), the positive control that a clean aliasing result on them came from runs where the reader actually overlapped a covered write. The census runs only on the toolchain and host target the manifest names, since Miri's schedule for a seed depends on both; anywhere else is `could-not-check`. Under `cargo test` the workloads are ordinary threaded tests that check what they read; under Miri they are `#[cfg_attr(miri, ignore)]`, so `miri-full` skips them. `lint` validates the manifest against the module (`--self-test`).
