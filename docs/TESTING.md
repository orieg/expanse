...
> **Scope caveat, documented in `sync` ([#1086](https://github.com/orieg/expanse/issues/1086)).** The `Sync*` wrappers reach two classes of undefined behaviour from safe code with two threads, and no required lane observes either. *Racing optimistic loads:* between a version sample and a failed validation, readers and optimistic writers make plain loads that race with a covered writer's plain stores. The value is discarded unless validation proves no overlap — the seqlock trade — but the load is still a data race under the Rust memory model. *References over shared state:* the wrappers form `&T` and `&mut T` to the engine, and the engine forms references to node fields (`Bitmap256::test`, `Bitmap256::set`), over bytes another thread is accessing. Stacked and Tree Borrows both reject that, and it is not a seqlock property. Miri covers `occ`/`alloc` and the single-threaded engine; loom covers the `occ` protocol primitives below plus the writer-gate entry and quiescence path through `sync`'s own `Shared::enter_writer` and `Shared::quiesce_writers`; the stress tests cover the composition; and the **Miri UB-site census** below pins every site the threaded workloads reach, so a change to any of them is visible.
