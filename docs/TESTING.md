...
- **ThreadSanitizer in nightly CI** (`test-tsan` in `nightly.yml`): runs `cargo test -Zbuild-std --target x86_64-unknown-linux-gnu -p expanse-trie --lib sync::` with `RUSTFLAGS="-Zsanitizer=thread"` to catch data races in concurrent `Sync*` operations.
