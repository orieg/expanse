...
> **Scope caveat, documented in `sync`.** Between a version sample and a failed validation, readers perform plain loads that race with writer stores. That is the classic seqlock trade: values are discarded unless validation proves no overlap. **The `sync` read path is therefore not Miri- or loom-checkable end-to-end** — Miri would flag the benign-by-protocol races. Miri still covers `occ`/`alloc` and the whole single-threaded engine; loom covers the `occ` protocol primitives below plus the writer-gate entry and quiescence path through `sync`'s own `Shared::enter_writer` and `Shared::quiesce_writers`; the stress tests cover the composition.
