...
Renders the two STM32H747I-DISCO charts from the committed on-target transcript
summary ``docs/benchmarks/stm32h747/results.json`` (produced by
``integrations/stm32h747/harvest.py`` from one VCP transcript):

  * ``docs/assets/bench_stm32h747.svg`` — Expanse across clocks and cache
    states, plus the ISR-reader arm against the critical-section twin;
...
def fixture(data: dict, impl: str, name: str, clk: int, dc: int) -> dict | None:
    for f in data["fixtures"]:
        if f.get("impl", "expanse") == impl and f["name"] == name and f["sysclk"] == clk and f["dcache"] == dc:
            return f
    return None
...
def provenance(data: dict, H: int) -> str:
    ...
    calib = ", ".join(f"{int(k) / 1e6:g} MHz nominal = {mhz(v)} measured" for k, v in sorted(hz.items(), key=lambda kv: int(kv[0])))
...
def render_main(data: dict) -> str:
    ...
    vals = {(n, c, d): fixture(data, "expanse", n, c, d)["min"] for n, _ in FIXTURES for c, d, _, _ in CONFIGS}
    axis_max = 500 * (int(max(vals.values()) / 500) + 1)
