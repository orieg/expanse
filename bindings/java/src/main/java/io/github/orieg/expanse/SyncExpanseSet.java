...
public final class SyncExpanseSet implements AutoCloseable, LongPredicate {
    ...
    public long memHeld() {
        checkOpen();
        try {
            return (long) ExpanseNative.MH_expanse_sync_set_mem_held.invokeExact(handle);
        } catch (Throwable t) {
            throw new RuntimeException(t);
        }
    }
    ...
    /**
     * Returns the freed blocks this set's epoch collector keeps for reuse
    ...
    public long shrinkToFit() {
        checkOpen();
        try {
            return (long) ExpanseNative.MH_expanse_sync_set_shrink_to_fit.invokeExact(handle);
        } catch (Throwable t) {
            throw new RuntimeException(t);
        }
    }
